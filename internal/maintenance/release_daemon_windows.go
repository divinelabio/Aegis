...
func (d *ReleaseDaemon) apply(ctx context.Context, manifest ArtifactManifest, credential string) error {
	...
	state.PreviousEdition, state.PreviousVersion, state.PreviousTarget = state.Edition, state.Version, state.CurrentTarget
...
func (d *ReleaseDaemon) applyNative(ctx context.Context, manifest ArtifactManifest, credential string) (string, error) {
	...
	if err = restartWindowsService(ctx, d.Config.ServiceName); err != nil {
		return "", err
	}
	return target, nil
}
...
func (d *ReleaseDaemon) download(ctx context.Context, manifest ArtifactManifest, credential string) (path string, err error) {
	...
	if "sha256:"+hex.EncodeToString(hash.Sum(nil)) != manifest.Digest {
		os.Remove(path)
...
func validateUpdaterConfig(config UpdaterConfig) error {
	if !strings.HasPrefix(strings.ToLower(config.SocketPath), `\\.\pipe\`) || config.StatePath == "" || config.Mode != "native" && config.Mode != "docker" {
		return errors.New("Windows updater pipe, state path, or mode is invalid")
	}
...
func CallUpdater(ctx context.Context, pipe string, request UpdaterRequest) (UpdaterResponse, error) {
