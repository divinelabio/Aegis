import { renderMetricSkeleton, renderMetricSubSkeleton, renderChartSkeleton, renderBreakdownSkeleton, renderProtectionCardsSkeleton, renderTableSkeleton, renderTextSkeleton } from './skeletons.js';
...
function valueString(value, fallback = 'Unavailable') {
...
function timeRangeLabel(window = selectedWindow) {
    const labels = {
        '15m': 'Last 15 minutes',
        '1h': 'Last hour',
        '24h': 'Last 24 hours',
    ...
    if (amount % 168 === 0)
        return `Last ${amount / 168} week${amount === 168 ? '' : 's'}`;
    if (amount % 24 === 0)
        return `Last ${amount / 24} day${amount === 24 ? '' : 's'}`;
...
function actionLabel(action) {
    ...
    if (normalized === 'ratelimit' || normalized === 'rate_limit')
        return 'Rate limited';
...
function actionTone(action) {
    ...
    if (normalized === 'block')
        return 'danger';
    if (normalized === 'ratelimit' || normalized === 'rate_limit' || normalized === 'challenge')
        return 'warning';
    if (normalized === 'allow')
        return 'success';
    return 'primary';
...
function filterAttributes(key, value, label) {
    if (!key || !value.trim())
        return '';
...
function renderHeaderFilters() {
    ...
    const entries = Object.entries(filters).flatMap(([rawKey, rawValue]) => {
        ...
        return value ? [{ key: rawKey, value, label: filterLabel(rawKey, value) }] : [];
    });
...
function applyFilter(key, rawValue) {
...
function applyLastRange() {
    const amount = Math.max(1, Math.min(999, Math.floor(Number(byId('traffic-dashboard-relative-amount')?.value) || 1)));
    const unit = byId('traffic-dashboard-relative-unit')?.value || 'hours';
    const windows = { minutes: `${amount}m`, hours: `${amount}h`, days: `${amount * 24}h`, weeks: `${amount * 168}h` };
    setWindow(windows[unit] || windows.hours);
}
function createEmptyTrafficDashboard(message = 'Analytics storage is currently unavailable.') {
    return {
        summary: {
            total_requests: 0,
            blocked_requests: 0,
...
function normalizedTrafficAction(value) {
    const normalized = String(value || '').toLowerCase();
    return normalized === 'rate_limit' || normalized === 'rate_limited' ? 'ratelimit' : normalized;
}
function normalizedTrafficReason(value) {
    const normalized = String(value || '').toLowerCase();
    return normalized === 'ddos' ? 'application_flood' : normalized;
}
function eventMatchesTrafficFilters(event) {
    const reason = normalizedTrafficReason(event.rule_type || event.metadata?.reason);
    if (filters.action && normalizedTrafficAction(event.action) !== normalizedTrafficAction(filters.action))
        return false;
    if (filters.reason && !reason.includes(normalizedTrafficReason(filters.reason)) && !normalizedTrafficReason(filters.reason).includes(reason))
        return false;
    if (filters.rule_id) {
    ...
    if (filters.method && event.method?.toUpperCase() !== filters.method.toUpperCase())
        return false;
    if (filters.status && String(event.status_code ?? '') !== filters.status)
        return false;
    return true;
}
function renderBreakdown(items, emptyText, filter) {
...
function trafficCount(point, ...keys) {
    return keys.reduce((total, key) => total + Math.max(0, Number(point.counts?.[key]) || 0), 0);
}
function renderTimeline(points) {
    const visible = (points || []).slice(-24);
    if (!visible.length)
        return '<div class="waf-dashboard-chart-empty">No Traffic Control activity in this window.</div>';
    const totalFor = (point) => Math.max(0, Number(point.counts?.total || point.counts?.requests) || trafficCount(point, 'allow', 'block', 'ratelimit', 'rate_limit', 'challenge', 'observe', 'error'));
...
function renderControlActivity(dashboard) {
...
function renderEvents(events) {
    const body = byId('traffic-dashboard-events');
    if (!body)
        return;
...
function renderEventDetail(event) {
    ...
    const rows = [
        ...
        ...metadata.map(([key, value]) => [key.replace(/[_-]/g, ' ').replace(/\b\w/g, char => char.toUpperCase()), value])
    ];
...
function renderDashboard(dashboard, operations = {}) {
    ...
    if (controlActivity)
        controlActivity.innerHTML = renderControlActivity(dashboard);
    const setBreakdown = (id, items, empty, filter) => {
        const target = byId(id);
        if (target)
            target.innerHTML = renderBreakdown(items, empty, filter);
    };
...
function resetTrafficDashboardSkeletons() {
    ...
    const controlActivity = byId('traffic-dashboard-control-activity');
    if (controlActivity)
        controlActivity.innerHTML = renderProtectionCardsSkeleton(3);
    ['outcomes', 'paths', 'sources-list', 'challenges'].forEach(panel => {
        const p = byId(`traffic-dashboard-${panel}`);
        if (p)
            p.innerHTML = renderBreakdownSkeleton(3);
    });
    const events = byId('traffic-dashboard-events');
    if (events)
        events.innerHTML = renderTableSkeleton(7, 6);
}
async function loadTrafficDashboard() {
    const version = ++requestVersion;
    const refresh = byId('traffic-dashboard-refresh');
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Traffic analytics request failed (${response.status}).`);
        const payload = await response.json();
        const operations = operationsResponse.ok ? await operationsResponse.json() : {};
...
function bindEvents() {
    ...
    document.addEventListener('click', event => {
        if (!(event.target instanceof Element) || !isMounted())
            return;
        const filterTarget = event.target.closest('[data-traffic-dashboard-filter-key][data-traffic-dashboard-filter-value]');
        if (filterTarget?.dataset.trafficDashboardFilterKey && filterTarget.dataset.trafficDashboardFilterValue) {
            event.preventDefault();
            applyFilter(filterTarget.dataset.trafficDashboardFilterKey, filterTarget.dataset.trafficDashboardFilterValue);
            return;
        }
        const actionTarget = event.target.closest('[data-traffic-dashboard-action]');
        if (actionTarget) {
            event.preventDefault();
            ...
            if (action === 'remove-filter')
                removeFilter(actionTarget.dataset.trafficFilterKey);
            if (action === 'toggle-time-panel')
                toggleTimePanel();
            if (action === 'apply-time-preset' && actionTarget.dataset.trafficTimePreset)
                setWindow(actionTarget.dataset.trafficTimePreset);
            if (action === 'apply-last-range')
                applyLastRange();
            if (action === 'refresh')
                void loadTrafficDashboard();
        ...
        const eventButton = event.target.closest('[data-traffic-dashboard-event]');
    ...
    document.addEventListener('keydown', event => {
        if ((event.key !== 'Enter' && event.key !== ' ') || !(event.target instanceof Element) || !isMounted())
            return;
        const target = event.target.closest('[data-traffic-dashboard-filter-key][data-traffic-dashboard-filter-value], [data-traffic-dashboard-action="clear-filters"]');
...
`;
export function initTrafficDashboard() {
    bindEvents();
    renderHeaderTime();
    renderHeaderFilters();
    void loadTrafficDashboard();
}
export function disposeTrafficDashboard() {
    requestVersion += 1;
}
