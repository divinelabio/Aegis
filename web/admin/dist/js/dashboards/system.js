...
import { renderMetricSkeleton, renderMetricSubSkeleton, renderChartSkeleton, renderSystemServicesSkeleton, renderTextSkeleton } from './skeletons.js';
...
export function loadSystemDashboard() {
    ...
    return Promise.all([
        ...
        setText('stat-go-version', sysData.go_version || 'Go runtime');
        // Resource details
...
function updateServicesSummary(health) {
...
function formatLastGCAge(value) {
    const timestamp = metricNumber(value);
    if (!timestamp)
        return 'No completed GC';
...
function updateSystemMetricsUnavailable() {
 ...
 */
function updateMemoryTimeline(memoryMB) {
    const now = new Date();
    memoryHistory.push({
...
function getMemoryTimelineSummary(values) {
 ...
 */
function initMemoryTimelineChart() {
    ...
    const labels = points.map((point, index) => {
        if (index !== 0 && index !== points.length - 1 && index % labelEvery !== 0)
            return '';
        if (index < points.length - 1 && point.time === points[points.length - 1].time)
            return '';
...
function bindMemoryTimelineHover(panel) {
    const chart = panel.querySelector('.system-health-timeline-svg');
    const tooltip = panel.querySelector('.system-health-timeline-tooltip');
    if (!chart || !tooltip)
        return;
    let points = [];
    try {
...
function bindSystemDashboardInteractions() {
    if (systemDashboardInteractionsBound)
        return;
    systemDashboardInteractionsBound = true;
    document.addEventListener('click', event => {
        if (!(event.target instanceof Element))
            return;
        const refresh = event.target.closest('[data-system-dashboard-action="refresh"]');
...
function setSystemDashboardRefreshState(button, refreshing) {
