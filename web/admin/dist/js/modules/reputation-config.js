...
import { escapeTrafficAttr, escapeTrafficHtml } from '../sections/traffic-runtime-helpers.js';
function createDefaultReputationConfig() {
    return {
        enabled: false,
...
export const ReputationConfig = {
    config: createDefaultReputationConfig(),
    trafficConfig: null,
    threatFeedData: null,
    ...
    bindEvents() {
        ...
        AdminEvents.delegateEvent(document, 'change', '#reputation-config-content [data-action="reputation-toggle-enabled"]', (_event, target) => {
            this.config.enabled = target.checked;
            const statusEl = AdminDOM.query('[data-reputation-section-status]');
            const engineEl = AdminDOM.query('[data-reputation-engine-status]');
        ...
        // Cache TTL input
        AdminEvents.delegateEvent(document, 'input', '#reputation-config-content #reputation-cache-ttl', (_event, target) => {
            this.config.cache_ttl = target.value;
            SectionUI.markSaveActionBarDirty('data-action');
        });
        // Datacenter toggle
        AdminEvents.delegateEvent(document, 'change', '#reputation-config-content [data-action="reputation-toggle-datacenter"]', (_event, target) => {
            this.config.block_datacenter = target.checked;
            SectionUI.markSaveActionBarDirty('data-action');
        });
        // Tor toggle
        AdminEvents.delegateEvent(document, 'change', '#reputation-config-content [data-action="reputation-toggle-tor"]', (_event, target) => {
            this.config.block_tor = target.checked;
            SectionUI.markSaveActionBarDirty('data-action');
        });
        // Threat feed category/severity filter
        AdminEvents.delegateEvent(document, 'change', '#reputation-config-content select[data-action="threat-feed-severity-filter"]', (_event, target) => {
            this.threatFeedPagination.severityFilter = target.value;
            this.threatFeedPagination.page = 1;
            this.render();
        });
        // Threat feed search input
        AdminEvents.delegateEvent(document, 'input', '#reputation-config-content input[data-action="threat-feed-search"]', (_event, target) => {
            this.threatFeedPagination.search = target.value;
            this.threatFeedPagination.page = 1;
            this.render();
        });
        // Threat feed page size selector
        AdminEvents.delegateEvent(document, 'change', '#reputation-config-content select[data-action="threat-feed-page-size"]', (_event, target) => {
            this.threatFeedPagination.pageSize = Number.parseInt(target.value, 10) || 25;
            this.threatFeedPagination.page = 1;
            this.render();
        });
        // Threat feed sort click
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="threat-feed-sort"]', (_event, target) => {
            const sortBy = (target.dataset.sortBy || 'score');
        ...
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="threat-feed-prev-page"]', event => {
        ...
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="threat-feed-next-page"]', event => {
            event.preventDefault();
            this.threatFeedPagination.page++;
            this.render();
        });
        // Refresh Threat Feed
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="refresh-threat-feed"]', async (event) => {
            event.preventDefault();
            await this.refreshThreatFeed();
        });
        // Rule modal events
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="reputation-open-add-rule"]', event => {
            event.preventDefault();
            this.openAddRuleModal();
        });
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="reputation-delete-rule"]', (_event, target) => {
            const ruleId = target.dataset.ruleId || '';
            this.config.rules = (this.config.rules || []).filter(r => r.id !== ruleId);
            this.render();
            SectionUI.markSaveActionBarDirty('data-action');
        });
        AdminEvents.delegateEvent(document, 'click', '[data-action="reputation-close-modal"]', event => {
            event.preventDefault();
            this.closeModal();
        });
        AdminEvents.delegateEvent(document, 'click', '[data-action="reputation-add-rule"]', event => {
            event.preventDefault();
            this.submitAddRule();
        });
        // Save and Reset changes
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="save-changes"]', event => {
            event.preventDefault();
            void this.saveConfig();
        });
        AdminEvents.delegateEvent(document, 'click', '#reputation-config-content [data-action="reset-changes"]', event => {
            event.preventDefault();
    ...
    async loadConfig() {
        try {
            ...
            if (response.ok) {
                ...
                const incomingRep = data?.reputation || {};
                this.config = {
    ...
    async loadThreatFeed() {
        try {
            ...
            if (response.ok) {
                this.threatFeedData = (await response.json());
            }
            else {
    ...
    submitAddRule() {
        const targetInput = AdminDOM.getById('rep-rule-target');
        const actionInput = AdminDOM.getById('rep-rule-action');
        const scoreInput = AdminDOM.getById('rep-rule-score');
        const commentInput = AdminDOM.getById('rep-rule-comment');
    ...
    renderThreatTable() {
        ...
        if (searchQuery) {
            filtered = filtered.filter(item => item.indicator.toLowerCase().includes(searchQuery) ||
        ...
        const sorted = [...filtered].sort((a, b) => {
            ...
            if (sortBy === 'score') {
                ...
                const mapSev = (s) => (s === 'CRITICAL' ? 3 : s === 'HIGH' ? 2 : 1);
        ...
        const renderSortHeader = (field, label, thClass = '') => {
