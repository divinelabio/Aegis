...
export function isExclusionExpired(exclusion) {
    if (!exclusion.expires_at)
        return false;
    const expires = new Date(exclusion.expires_at).getTime();
    return !Number.isNaN(expires) && expires < Date.now();
}
export function renderExclusionsTab(exclusions, exclusionQuery, exclusionScope, renderStatusCard) {
    ...
    const filtered = list.filter(ex => {
        if (exclusionQuery) {
            ...
            if (!matchesRule && !matchesPath && !matchesComment)
                return false;
        }
        if (exclusionScope === 'global')
            return !ex.rule_id;
        if (exclusionScope === 'rule')
            return Boolean(ex.rule_id);
        if (exclusionScope === 'active')
            return ex.enabled !== false && !isExclusionExpired(ex);
        if (exclusionScope === 'inactive')
            return ex.enabled === false || isExclusionExpired(ex);
        return true;
    });
    ...
    return `
        ${renderStatusCard('Rule Exclusions', 'Create targeted exceptions when valid traffic is incorrectly detected.', 'filter', expiredCount === 0, '', expiredCount > 0 ? 'Needs Review' : 'Active')}
