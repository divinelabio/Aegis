...
export const defaultWafLeakProtectionConfig = {
    enabled: false,
    mode: 'detect',
...
export function formatWafBytes(bytes) {
    if (bytes < 1024)
        return `${bytes} B`;
    if (bytes < 1048576)
...
export function formatWafMebibytes(bytes) {
    const mib = bytes / 1048576;
    return Number.isInteger(mib) ? `${mib}` : mib.toFixed(1);
}
export function renderLeakProtectionTab(leakConfig, leakProtectionPage, getIcon) {
    const leak = leakConfig || defaultWafLeakProtectionConfig;
    const normalizeAction = (raw, fallback = 'detect') => {
    ...
    const actionSelect = (path, value, label, includeDefault = false) => `
    ...
    const renderMimeGroupCards = () => `
        ...
        const isEnabled = (leak.inspect_content_types || []).some(m => group.mimeTypes.some(gm => gm.toLowerCase() === m.toLowerCase()));
    ...
    const renderCategoryRules = (group) => `
        <div class="operator-control-list">
            ${group.items.map(item => {
        const rulesObj = (leak.categories || {});
    ...
    const activeGroupCount = leakMimeGroups.filter(group => (leak.inspect_content_types || []).some(m => group.mimeTypes.some(gm => gm.toLowerCase() === m.toLowerCase()))).length;
    const rulesMap = (leak.categories || {});
