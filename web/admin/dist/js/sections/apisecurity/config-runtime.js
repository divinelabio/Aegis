...
import { loadAPISecurityConfig, loadAPISecurityDashboard, loadAPISecurityInventory, loadAPISecurityStats, uploadAPISecuritySchema } from './api.js';
...
const API_SECURITY_POLICY_CATALOG = [
    {
        id: 'runtime',
...
const APISecurityConfig = {
    ...
    async loadConfig() {
        try {
            const data = await loadAPISecurityConfig();
            if (!data)
                throw new Error('Failed to load config');
            this.config = normalizeAPISecurityConfig(data.settings || data);
    ...
    async loadStats() {
        try {
            const data = await loadAPISecurityStats();
            if (data)
                this.stats = data;
        }
        catch (e) {
    ...
    async loadOverviewData() {
        ...
        try {
            const [dashboardResponse, inventoryResponse] = await Promise.all([
                ...
                loadAPISecurityInventory({ limit: 100 })
            ]);
    ...
    openAuthorizationControl(id) {
        if (!['function', 'object', 'fields'].includes(id))
            return;
        this.selectedAuthorizationControl = id;
        this.render();
    },
    closeAuthorizationControl() {
        this.selectedAuthorizationControl = '';
        this.render();
    },
    openGraphQLControl(id) {
        if (!['limits', 'schema'].includes(id))
            return;
        this.selectedGraphQLControl = id;
        this.render();
    },
    closeGraphQLControl() {
        this.selectedGraphQLControl = '';
    ...
    focusSetup(id) {
        ...
        this.blockingReviewTarget = null;
        this.policyModeConfirmTarget = null;
        if (['schemas', 'schema'].includes(this.setupFocus))
            this.openWizard('schema');
        else if (this.setupFocus === 'identity')
            this.openWizard('identity');
        else if (this.setupFocus === 'data')
    ...
    async applySchemaWizard() {
        const modeInput = AdminDOM.getById('apisecurity-schema-wizard-mode');
    ...
    applyIdentityWizard() {
        const issuerNameInput = AdminDOM.getById('apisecurity-identity-issuer-name');
        const issuerInput = AdminDOM.getById('apisecurity-identity-issuer');
        const audienceInput = AdminDOM.getById('apisecurity-identity-audiences');
        const jwksInput = AdminDOM.getById('apisecurity-identity-jwks');
        const requireInput = AdminDOM.getById('apisecurity-identity-require-token');
        const modeInput = AdminDOM.getById('apisecurity-identity-mode');
    ...
    applyDataWizard() {
        const modeInput = AdminDOM.getById('apisecurity-data-wizard-mode');
    ...
    requestConfigFieldUpdate(path, value, renderAfterUpdate = false) {
        const normalizedPath = String(path || '').trim();
        if (!normalizedPath)
            return;
        const blockTarget = buildAPISecurityModeFieldConfirmTarget(this.config, normalizedPath, value, this.currentTab, this.selectedPolicyId);
        if (blockTarget) {
    ...
    setPolicyFunctionState(enabledPath, modePath, state) {
        const enabledField = String(enabledPath || '').trim();
        const modeField = String(modePath || '').trim();
        const normalized = String(state || '').trim().toLowerCase();
        if (!enabledField || !modeField || !['disable', 'detect', 'block'].includes(normalized))
            return;
    ...
    setPolicyBooleanState(path, state) {
        const field = String(path || '').trim();
        const normalized = String(state || '').trim().toLowerCase();
        if (!field || !['disable', 'block'].includes(normalized))
            return;
    ...
    applyProfile(key, value) {
        if (value !== 'default' && value !== 'strict' && value !== 'monitor')
            return;
        const profileKey = key;
        const profileValue = value;
        const policyId = getAPISecurityPolicyIdForProfileKey(profileKey);
        if (policyId && deriveAPISecurityProfile(this.config, profileKey) === profileValue)
            return;
    ...
    addNestedListItem(kind, parentIndex, field, value) {
        const item = String(value || '').trim();
        if (!item)
            return;
    ...
    updateNestedListItem(kind, parentIndex, field, itemIndex, value) {
        if (itemIndex < 0)
            return;
        const current = getAPISecurityNestedStringList(this.config, kind, parentIndex, field);
    ...
    removeNestedListItem(kind, parentIndex, field, itemIndex) {
        if (itemIndex < 0)
            return;
        const current = getAPISecurityNestedStringList(this.config, kind, parentIndex, field);
        if (itemIndex >= current.length)
    ...
    updateInventoryFilter(key, value) {
        if (!Object.prototype.hasOwnProperty.call(this.inventoryFilters, key))
            return;
        ...
        if (restoreSearchFocus) {
            window.setTimeout(() => {
                const input = document.querySelector('[data-api-inventory-filter="search"]');
                if (!input)
                    return;
                input.focus();
    ...
    async loadMoreInventory() {
        if (!this.inventoryPage.has_more)
            return;
        ...
        try {
            const response = await loadAPISecurityInventory({ limit, offset });
    ...
    toggleInventoryFilterMenu() {
        this.inventoryFilterMenuOpen = !this.inventoryFilterMenuOpen;
        if (this.inventoryFilterMenuOpen)
            this.inventoryViewMenuOpen = false;
        this.render();
    },
    toggleInventoryViewMenu() {
        this.inventoryViewMenuOpen = !this.inventoryViewMenuOpen;
        if (this.inventoryViewMenuOpen)
            this.inventoryFilterMenuOpen = false;
        this.render();
    },
    toggleInventoryColumn(column) {
        if (!isAPISecurityInventoryColumnId(column))
            return;
    ...
    openScopePicker(path, title, mode, tab) {
        if (!isSupportedAPISecurityScopePath(path))
            return;
    ...
    updateScopePickerFilter(key, value) {
        if (!Object.prototype.hasOwnProperty.call(this.scopePickerFilters, key))
            return;
    ...
    applyScopePickerChoice(choice) {
        if (!this.scopePickerTarget)
            return;
    ...
    openBlockingReview(moduleKey, functionKey, title, modePath, tab) {
        if (!modePath)
            return;
    ...
    closeInventoryEndpoint() {
        this.selectedInventoryEndpointKey = '';
        this.render();
    },
    setInventoryTriageState(key, state) {
        this.updateInventoryMetadataField(key, 'triage_state', state);
    },
    updateInventoryMetadataField(key, field, value) {
    ...
    prefillSchemaFromEndpoint(key) {
        ...
        setTimeout(() => {
            ...
            const textarea = AdminDOM.getById('apisecurity-schema-content');
            if (textarea) {
                textarea.focus();
            }
        }, 120);
    },
    updateBolaRule(index, field, value) {
    ...
    updateAuthIssuer(index, field, value) {
    ...
    updateBFLARule(index, field, value) {
    ...
    updateMassAssignmentPathRule(index, field, value) {
    ...
    removeMassAssignmentPathRule(index) {
        const rules = normalizeMassAssignmentPathRules(this.config.mass_assignment?.path_rules);
        rules.splice(index, 1);
        this.config.mass_assignment = {
            ...(this.config.mass_assignment || {}),
    ...
    async uploadSchema() {
        const endpointSelect = AdminDOM.getById('apisecurity-schema-endpoint');
        const methodInput = AdminDOM.getById('apisecurity-schema-method');
        const pathInput = AdminDOM.getById('apisecurity-schema-path');
        const hostInput = AdminDOM.getById('apisecurity-schema-host');
        const typeInput = AdminDOM.getById('apisecurity-schema-type');
        const contentInput = AdminDOM.getById('apisecurity-schema-content');
    ...
    async saveConfig() {
        ...
        const result = await api.requestResult(API_SECURITY_CONFIG_ENDPOINT, 'PUT', payload);
...
function getDefaultAPISecurityConfig() {
...
function normalizeAPISecurityConfig(input) {
    const cfg = { ...(input || {}) };
    ...
    delete bflaFunctionSettings.role_header;
    ...
    for (const legacyPath of ['user_id_header', 'user_roles_header', 'tenant_header', 'learning_mode', 'learning_days']) {
        delete bolaFunctionSettings[legacyPath];
    }
...
function buildAPISecuritySavePayload(config, sectionEnabled) {
    const normalized = normalizeAPISecurityConfig(config);
    const settings = {};
    SUPPORTED_CONFIG_KEYS.forEach((key) => {
        const value = normalized[key];
        if (value !== undefined) {
            settings[key] = cloneConfigValue(value);
        }
    });
    return {
...
function formatAPISecuritySaveFailure(error) {
    ...
    const detailMessage = details
        ...
        .find((detail) => Boolean(detail) && detail !== message);
...
function getSSRFConfig(config) {
    return config.ssrf || {};
}
function getPayloadAbuseConfig(config) {
    return config.payload_abuse || {};
}
function getGraphQLConfig(config) {
    return config.graphql || {};
}
function getAutomationAbuseConfig(config) {
    return config.automation_abuse || {};
}
function getDataExposureConfig(config) {
    return config.data_exposure || {};
}
function getValidationConfig(config) {
...
function normalizePayloadSensitivity(value) {
    return String(value || '').trim().toLowerCase() === 'strict' ? 'strict' : 'balanced';
}
function normalizeAutomationSensitivity(value) {
    return String(value || '').trim().toLowerCase() === 'strict' ? 'strict' : 'balanced';
}
function normalizeDataExposureSensitivity(value) {
    return String(value || '').trim().toLowerCase() === 'strict' ? 'strict' : 'balanced';
}
function normalizeDataExposureResponseHandling(value) {
...
function normalizeInventoryMetadata(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return {};
    const result = {};
    Object.entries(value).forEach(([key, rawValue]) => {
        const endpointKey = normalizeInventoryEndpointKey(key);
        if (!endpointKey || typeof rawValue !== 'object' || rawValue === null || Array.isArray(rawValue))
            return;
        const raw = rawValue;
...
function normalizeDashboardViews(value) {
    if (!Array.isArray(value))
        return [];
    const seen = new Set();
...
function normalizeDashboardViewFilters(value) {
...
function normalizeMonitoringRules(value) {
    if (!Array.isArray(value))
        return [];
    const seen = new Set();
...
function normalizeSetupWorkflow(value) {
    const raw = (typeof value === 'object' && value !== null && !Array.isArray(value)) ? value : {};
...
function isAPISecuritySetupStep(value) {
...
function normalizeMonitoringRuleType(value) {
    const text = String(value || '').trim().toLowerCase();
...
function slugWorkflowId(value) {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}
function lowerTrim(value) {
    return String(value || '').trim().toLowerCase();
}
function normalizeInventoryEndpointKey(value) {
    return String(value || '').trim().toLowerCase();
}
function getInventoryEndpointKey(endpoint) {
    const id = String(endpoint.id || '').trim();
    if (id)
        return normalizeInventoryEndpointKey(id);
...
function normalizeInventoryEnvironment(value) {
    const text = String(value || '').trim().toLowerCase();
    return text || 'unknown';
}
function normalizeInventoryCriticality(value) {
    const text = String(value || '').trim().toLowerCase();
    return ['low', 'medium', 'high', 'critical'].includes(text) ? text : 'medium';
}
function normalizeInventoryTriageState(value) {
...
function normalizeBolaOwnershipRules(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map(item => {
...
function normalizeAuthTokenIssuers(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map(item => {
...
function normalizeBFLARules(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map(item => {
...
function normalizeMassAssignmentPathRules(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map(item => {
...
function getGraphQLLimitPreset(graphql) {
...
function getGraphQLPresetDescription(preset) {
    if (preset === 'strict')
        return 'Lower depth, aliases, batch, and complexity.';
    if (preset === 'relaxed')
        return 'Higher limits while monitoring onboarding traffic.';
    return 'Balanced production limits with introspection blocked.';
}
function defaultAutomationLoginPaths() {
    return ['/login', '/auth/login', '/api/auth/login', '/sessions', '/token'];
}
function defaultAutomationOTPPaths() {
    return ['/otp', '/mfa', '/2fa', '/verify'];
}
function defaultAutomationMonitoredPaths() {
    return ['*'];
}
function defaultAutomationIdentifierFields() {
...
function normalizeAPISecurityMode(value) {
    const mode = String(value || '').trim().toLowerCase();
    if (mode === 'detect' || mode === 'detection' || mode === 'log' || mode === 'logging')
        return 'detect';
    if (mode === 'block' || mode === 'blocking')
        return 'block';
    return 'block';
}
function revealAPISecurityRule(ruleId) {
    window.requestAnimationFrame(() => {
        const rule = document.querySelector(`[data-api-rule-card="${ruleId}"]`);
        if (!rule)
            return;
        rule.scrollIntoView({ behavior: 'smooth', block: 'center' });
        rule.querySelector('input:not([type="hidden"])')?.focus({ preventScroll: true });
    });
}
function normalizeBolaMode(value) {
    const mode = String(value || '').trim().toLowerCase();
    if (mode === 'block' || mode === 'blocking')
        return 'block';
    if (mode === 'detect' || mode === 'detection' || mode === 'log' || mode === 'logging')
...
function getStatNumber(stats, key, fallbackKey) {
    ...
    if (typeof custom === 'number')
        return custom;
    if (fallbackKey)
        return getStatNumber(stats, fallbackKey);
    return 0;
}
...
function getOverviewCoverage(dashboard, inventory) {
...
function getInventoryEndpointPosture(config, dashboard, inventory) {
    ...
    return inventory.map(endpoint => {
        ...
        const riskReasons = [];
        if (hasSensitiveData && schemaMissing)
            riskReasons.push('Sensitive data with no schema');
        if (hasAuthorizationFindings)
            riskReasons.push('Authorization findings');
        if (isShadow)
            riskReasons.push('Shadow API');
        if (hasRepeatedFindings)
            riskReasons.push('Repeated findings');
        if (hasAutomationFindings)
            riskReasons.push('Automation findings');
        if (isZombie)
            riskReasons.push('Zombie API');
        if (schemaMissing && !isShadow)
            riskReasons.push('Missing schema');
        if (!riskReasons.length && endpoint.last_finding)
            riskReasons.push(labelizeAPIInventoryValue(endpoint.last_finding));
        return {
            endpoint,
            key,
...
function getEndpointRiskScore(flags) {
    let score = 0;
    if (flags.hasSensitiveData && flags.schemaMissing)
        score = Math.max(score, 100);
    if (flags.hasAuthorizationFindings)
        score = Math.max(score, 90);
    if (flags.isShadow)
        score = Math.max(score, 80);
    if (flags.hasRepeatedFindings)
        score = Math.max(score, 70);
    if (flags.hasAutomationFindings)
        score = Math.max(score, 60);
    if (flags.isZombie)
        score = Math.max(score, 50);
    if (flags.schemaMissing)
        score = Math.max(score, 40);
    return score;
}
function getEndpointRecentFindings(dashboard, endpoint) {
    ...
    return (dashboard.events?.events || [])
        ...
        if (path && event.path !== path)
            return false;
        if (method && event.method && String(event.method).toUpperCase() !== method)
            return false;
        if (host && event.host && event.host !== host)
            return false;
        return true;
    })
        .slice(0, 5);
}
function getEndpointFindingCount(dashboard, endpoint, recentFindings) {
    const path = String(endpoint.path || '');
...
function isEndpointStale(endpoint, days) {
...
function getInventoryPostureCounts(posture) {
    return posture.reduce((acc, item) => {
        if (item.schemaMissing)
            acc.missing += 1;
        if (item.isShadow)
            acc.shadow += 1;
        if (item.isZombie)
        ...
        if (item.hasSensitiveData)
            acc.sensitive += 1;
        if (item.hasRepeatedFindings)
            acc.repeated += 1;
        if (item.riskScore > 0 && !['accepted', 'ignored', 'deprecated'].includes(item.metadata.triage_state || ''))
            acc.review += 1;
        return acc;
    }, { sensitive: 0, repeated: 0, missing: 0, shadow: 0, zombie: 0, review: 0 });
}
function renderInventorySummaryMetrics(coverage, counts) {
...
const APISecurityInventoryColumnIds = ['method', 'traffic', 'schema', 'posture', 'last_seen', 'findings'];
function getDefaultAPISecurityInventoryFilters() {
    return { search: '', host: 'all', method: 'all', schema: 'all', risk: 'all', active: 'all' };
}
function getDefaultAPISecurityInventoryColumns() {
    return { method: true, traffic: true, schema: true, posture: true, last_seen: true, findings: true };
}
function normalizeAPISecurityInventoryFilters(value) {
...
function normalizeAPISecurityInventoryColumns(value) {
    const input = typeof value === 'object' && value !== null ? value : {};
    ...
    if (!APISecurityInventoryColumnIds.some(column => next[column]))
        next.traffic = true;
    return next;
}
function loadAPISecurityInventorySavedViews() {
    return readAPISecurityLocalJSON(APISEC_INVENTORY_SAVED_VIEWS_KEY, [])
        .map(normalizeAPISecurityInventorySavedView)
        .filter((view) => Boolean(view))
        .slice(0, 12);
}
function loadAPISecurityInventoryRecentFilters() {
    return readAPISecurityLocalJSON(APISEC_INVENTORY_RECENT_FILTERS_KEY, [])
        .map(normalizeAPISecurityInventoryRecentFilter)
        .filter((filter) => Boolean(filter))
        .slice(0, 8);
}
function readAPISecurityLocalJSON(key, fallback) {
    try {
...
function writeAPISecurityLocalJSON(key, value) {
    try {
...
function normalizeAPISecurityInventorySavedView(value) {
...
function normalizeAPISecurityInventoryRecentFilter(value) {
    if (typeof value !== 'object' || value === null)
        return null;
    const input = value;
    const filters = normalizeAPISecurityInventoryFilters(input.filters);
    if (getActiveInventoryFilterCount(filters) === 0)
        return null;
    return {
...
function saveAPISecurityInventoryView(views, name, filters, columns, groupBy) {
...
function deleteAPISecurityInventoryView(views, id) {
    const next = views.filter(view => view.id !== id);
    writeAPISecurityLocalJSON(APISEC_INVENTORY_SAVED_VIEWS_KEY, next);
    return next;
}
function rememberAPISecurityInventoryRecentFilter(filters, current) {
    const normalized = normalizeAPISecurityInventoryFilters(filters);
    if (getActiveInventoryFilterCount(normalized) === 0)
        return current;
    const now = new Date().toISOString();
...
function formatAPISecurityInventoryViewSummary(filters, groupBy) {
...
function formatAPISecurityInventoryFilterLabel(filters) {
...
function slugifyAPISecurityLocalName(value) {
    return String(value || 'view').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 32) || 'view';
}
function renderInventoryToolbar(inventory, filters, filterMenuOpen, viewMenuOpen, columns, groupBy, savedViews, recentFilters, filteredCount, totalCount) {
...
function renderInventoryViewMenu(columns, groupBy, savedViews, recentFilters, filters) {
    const groupOptions = [
...
function renderInventoryAddFilterMenu(hosts, methods, filters) {
    return `
...
function renderInventorySelectFilter(label, key, value, options) {
    return `
        <label class="bot-overview-field apisecurity-inventory-filter-field">
...
function renderInventoryFilterChips(filters) {
...
function getActiveInventoryFilterCount(filters) {
...
function getInventoryFilterDisplayValue(key, value) {
    if (key === 'search')
        return value;
    return labelizeAPIInventoryValue(value);
}
function getFilteredInventoryPosture(posture, filters) {
...
function renderGroupedInventoryEndpointTables(posture, selectedKey, columns, groupBy) {
    if (!posture.length)
        return renderInventoryEndpointTable(posture, selectedKey, columns);
...
function getAPIMethodTone(method) {
...
function renderInventoryEndpointTable(posture, selectedKey = '', columns = { method: true, traffic: true, schema: true, posture: true, last_seen: true, findings: true }) {
...
function getInventoryGroupValue(item, groupBy) {
    const endpoint = item.endpoint;
    if (groupBy === 'host')
        return endpoint.host || 'Unknown host';
    if (groupBy === 'method')
        return endpoint.method || 'Unknown method';
    if (groupBy === 'schema')
        return labelizeAPIInventoryValue(getEndpointSchemaStatus(endpoint));
    if (groupBy === 'posture')
        return getInventoryPostureLabel(item);
    if (groupBy === 'service')
        return item.metadata.service || 'Unassigned service';
    if (groupBy === 'owner')
        return item.metadata.owner || 'Unassigned owner';
    return 'All endpoints';
}
function getInventoryColumnLabel(column) {
    switch (column) {
        case 'method': return 'Method';
...
function isAPISecurityInventoryColumnId(value) {
    return APISecurityInventoryColumnIds.includes(String(value || ''));
}
function normalizeAPISecurityInventoryGroupBy(value) {
    const normalized = String(value || '').trim();
    return ['none', 'host', 'method', 'schema', 'posture', 'service', 'owner'].includes(normalized)
        ? normalized
        : 'none';
}
function renderInventoryTrafficCell(endpoint) {
    const active = endpoint.is_active !== false;
    return `<strong>${active ? 'Active' : 'Inactive'}</strong>`;
}
function renderInventoryLastSeenCell(endpoint) {
    if (!endpoint.last_seen) {
...
function formatAPISchemaType(type) {
    const raw = String(type || '').trim().toLowerCase();
    if (raw === 'openapi' || raw === 'swagger')
        return 'OpenAPI';
    if (raw === 'graphql')
        return 'GraphQL';
    if (raw === 'grpc' || raw === 'proto' || raw === 'protobuf')
        return 'gRPC';
    if (!raw)
        return '';
    return raw.charAt(0).toUpperCase() + raw.slice(1);
}
function renderInventorySchemaCell(endpoint) {
    const schema = getEndpointSchemaStatus(endpoint);
...
function renderInventoryTableActions(item) {
    const endpoint = item.endpoint;
    const schema = getEndpointSchemaStatus(endpoint);
...
function inventoryPostureMatchesFilter(item, filter) {
...
function getInventoryPostureFilterValue(item) {
    if (item.hasSensitiveData)
        return 'sensitive';
    if (item.isShadow)
        return 'shadow';
...
function getInventoryPostureLabel(item) {
    const value = getInventoryPostureFilterValue(item);
    if (value === 'review')
        return 'Needs review';
    return labelizeAPIInventoryValue(value);
}
function getInventoryPostureTone(item) {
    const value = getInventoryPostureFilterValue(item);
    if (value === 'sensitive')
        return 'danger';
    if (value === 'shadow' || value === 'review')
        return 'warning';
    if (value === 'zombie')
        return 'neutral';
    return 'success';
}
function renderInventoryEndpointDetail(item, config, dashboard, inventory) {
    if (!item)
        return '';
    const endpoint = item.endpoint;
...
function renderInventoryEndpointFact(label, value) {
    return `
...
const API_SECURITY_SETTINGS_CATEGORIES = [
...
const API_SECURITY_POLICY_OWNED_SETTING_CATEGORIES = new Set([
    'runtime',
    'authorization',
    'graphql',
    'abuse',
...
const API_SECURITY_DELEGATED_SETTING_CATEGORIES = new Set([
    'identity_auth_detection'
]);
function getAPISecuritySettingsWorkbenchCategories(config) {
...
function hasAPISecurityIntegrationReportConfig(config) {
    return Array.isArray(config.dashboard_views) || Array.isArray(config.alert_rules);
}
function renderAPISecuritySettingsOverview(categories, icons) {
    const grouped = [
        {
            title: 'Setup',
            description: 'Traffic discovery, inventory, schemas, and identity.',
            ids: ['discovery', 'inventory_classification', 'schemas', 'identity_auth_detection']
        },
        {
            title: 'Operations',
            description: 'Exceptions and reporting configuration.',
            ids: ['exceptions', 'integrations_reports']
        }
    ];
...
function renderAPISecuritySettingsAreaCard(category, icons) {
...
function getAPISecuritySettingsAreaMeta(category, icons) {
...
function renderAPISecuritySettingsCategoryPage(category, config, inventory, icons, schemaPrefillEndpointKey, activeWizard, wizardStep) {
    if (!category)
        return '';
...
function normalizeAPISecuritySettingCategory(value) {
    const text = String(value || '').trim();
    if (API_SECURITY_POLICY_OWNED_SETTING_CATEGORIES.has(text)
        || API_SECURITY_DELEGATED_SETTING_CATEGORIES.has(text))
        return 'discovery';
    return API_SECURITY_SETTINGS_CATEGORIES.some(category => category.id === text)
        ? text
        : 'discovery';
}
function renderAPISecuritySettingsCategoryContent(category, config, inventory, icons, schemaPrefillEndpointKey, activeWizard, wizardStep) {
...
function renderAPISecuritySettingsSearchResults(config, inventory, search) {
...
function renderAPISecurityRegistryRows(settings, config, inventory, compact) {
    if (!settings.length)
        return '';
...
function renderAPISecurityRegistrySettingRow(setting, config, inventory) {
...
function renderAPISecurityRegistryControl(setting, value, inventory) {
...
function renderAPISecuritySettingsExceptions(config, inventory, icons) {
    return `
...
function renderAPISecuritySettingsIntegrationsReports(config, icons) {
...
function getAPISecuritySettingSelectOptions(path, value, fallback) {
...
function formatAPISecuritySettingValue(value) {
    if (Array.isArray(value))
...
function getCriticalityTone(value) {
...
function uniqueInventoryValues(values) {
    return [...new Set(values)].sort((a, b) => a.localeCompare(b));
}
function labelizeAPIInventoryValue(value) {
    return String(value || 'unknown')
        .replace(/[_-]+/g, ' ')
        .replace(/\b\w/g, char => char.toUpperCase());
}
function formatAPISecurityModeField(path) {
...
function renderAPISecurityStringListEditor(title, path, values, options = {}) {
    const normalizedPath = normalizeAPISecurityStringListPath(path);
...
function renderAPISecurityPathListEditor(title, path, values, inventory, options = {}) {
...
function renderAPISecurityChipListEditor(title, path, values, options = {}) {
    return renderAPISecurityStringListEditor(title, path, values, {
        ...options,
...
function getAPISecurityStringListInventoryOptions(inventory, filter) {
...
function renderRuleEndpointPreview(pattern, inventory, methods = []) {
...
function apiPathMatchesPattern(path, pattern) {
...
function renderBolaOwnershipRules(rules, inventory = []) {
    if (rules.length === 0) {
        return `
...
function isBolaRuleReady(rule) {
    return Boolean(rule.name && rule.resource_pattern && rule.owner_id_param);
}
function renderBFLARuleRows(rules, inventory = []) {
    if (rules.length === 0) {
        return `
            <div class="apisecurity-function-empty">
...
function renderBFLACompactValueEditor(title, field, parentIndex, values, options = {}) {
    return renderAuthorizationCompactValueEditor('bfla-rule', title, field, parentIndex, values, options);
}
function renderAuthorizationCompactValueEditor(kind, title, field, parentIndex, values, options = {}) {
    const items = normalizeStringList(values, []);
...
function renderAuthorizationStringValueEditor(title, path, values, options = {}) {
    const items = normalizeStringList(values, []);
    const suggestions = normalizeStringList(options.suggestions || [], []);
    const suggestionKeys = new Set(suggestions.map(value => value.toLowerCase()));
    const customItems = items
...
function renderMassAssignmentPathRuleRows(rules, inventory = []) {
    if (rules.length === 0) {
        return `
            <div class="apisecurity-function-empty">
...
function getAPISecuritySurfaceIdForScopePath(path) {
    if (path === 'discovery.paths')
        return 'inventory';
    if (path.startsWith('graphql.'))
        return 'graphql';
    if (path.startsWith('auth_tokens.') || path.startsWith('bola.'))
        return 'authorization';
    if (path.startsWith('automation_abuse.'))
        return 'abuse';
    if (path.startsWith('data_exposure.'))
        return 'data';
    return 'inventory';
}
function getAPISecurityPolicyCatalogEntry(id) {
    return API_SECURITY_POLICY_CATALOG.find(policy => policy.id === id) || null;
}
function getAPISecurityPolicyState(config, id) {
    if (id === 'runtime') {
...
function formatAPISecurityProfileLabel(profile) {
    if (profile === 'default')
        return 'Standard';
    if (profile === 'strict')
        return 'Strict';
...
function getAPISecurityPolicyCardDescription(id) {
    switch (id) {
...
function getAPISecurityPolicyDisplayTitle(id) {
    switch (id) {
...
function renderAPISecurityPolicyEnabledToggle(id, enabled) {
    return `
...
function buildAPISecurityPolicyViewModel(policy, config, dashboard, inventory) {
    const state = getAPISecurityPolicyState(config, policy.id);
    const surface = getAPISecurityPolicySurfaceSummary(policy.id, config, dashboard, inventory);
    return {
        policy,
...
function renderAPISecurityPolicyEditor(policy, activeTab, config, inventory, icons, setupFocus, schemaPrefillEndpointKey, activeWizard, wizardStep) {
    void activeTab;
...
function doesAPISecurityPolicyOwnSettings(policyId) {
...
function renderAPISecurityPolicyDetailHeader(_policy, _config, _dashboard, _inventory, _icons) {
    return '';
}
function renderAPISecurityPolicyScopeContent(policy, config, inventory, icons) {
    if (policy.id === 'contract') {
        return renderAPISecurityControlSection('Schema attachment', 'Attach OpenAPI or GraphQL contracts using the existing schema upload endpoint.', icons.fileText, renderAPISecuritySchemaUploadForm(inventory, ''));
    }
...
function renderAPISecurityPolicyExceptions(policyId, config, inventory, icons) {
    void icons;
...
function renderAPISecurityPolicyModeConfirm(target, config) {
    if (!target)
        return '';
...
function renderAPISecurityModeFieldConfirm(target, _dashboard) {
    if (!target)
        return '';
...
function getAPISecurityPolicySurfaceSummary(id, config, dashboard, inventory) {
    if (id === 'runtime') {
...
function formatAPISecurityPathSummary(paths, fallback) {
...
function buildAPISecurityModeFieldConfirmTarget(config, path, value, currentTab, selectedPolicyId) {
    if (normalizeAPISecurityMode(value) !== 'block')
        return null;
    const parts = String(path || '').split('.').filter(Boolean);
    if (parts.length !== 2 || parts[1] !== 'mode')
        return null;
    const functionKey = parts[0];
    if (!isAPISecurityFunctionConfigKey(functionKey))
        return null;
    if (getAPISecurityPolicyFunctionMode(config, functionKey) === 'block')
        return null;
...
function getAPISecurityPolicyModeSummary(config, id) {
    const modes = getAPISecurityPolicyFunctionKeys(id).map(key => getAPISecurityPolicyFunctionMode(config, key));
    if (modes.length && modes.every(mode => mode === 'block'))
        return 'Block';
    if (modes.length && modes.every(mode => mode === 'detect'))
        return 'Monitor';
    return 'Mixed';
}
function syncAPISecurityMirroredConfigValue(config, path, value) {
    const [root, ...rest] = String(path || '').split('.').filter(Boolean);
    if (!root || rest.length === 0 || !isAPISecurityFunctionConfigKey(root))
        return;
...
function setAPISecurityConfigValue(config, path, value) {
...
function getAPISecurityNestedStringList(config, kind, parentIndex, field) {
...
function applyAPISecurityNestedStringList(runtime, kind, parentIndex, field, values) {
...
function applyAPISecurityPolicyEnabled(config, id, enabled) {
...
function applyAPISecurityPolicyMode(config, id, mode) {
    const next = normalizeAPISecurityConfig(config);
    getAPISecurityPolicyFunctionKeys(id).forEach(key => {
...
function getAPISecurityPolicyIdForProfileKey(key) {
    const policy = API_SECURITY_POLICY_CATALOG.find(entry => entry.profileKey === key);
    return policy?.id || null;
}
function shouldConfirmAPISecurityProfileBlock(config, policyId, profileKey, profileValue) {
    if (profileValue !== 'strict')
        return false;
...
function renderAPISecurityPolicySetupPanel(policyId, config, inventory, setupFocus, schemaPrefillEndpointKey, activeWizard, wizardStep) {
    if (!activeWizard)
        return '';
    const targetPolicy = getAPISecurityPolicyIdForWizard(activeWizard, policyId);
    if (targetPolicy !== policyId)
        return '';
    return renderAPISecurityWizardDrawer(activeWizard, wizardStep, config, inventory, setupFocus, schemaPrefillEndpointKey);
}
function getAPISecuritySettingsGroupContent(group, config, inventory, icons) {
    switch (group) {
...
function getAPISecurityPolicyRows(id, config, icons) {
    if (id === 'runtime') {
        const injection = getInjectionConfig(config);
        const ssrf = getSSRFConfig(config);
        const payload = getPayloadAbuseConfig(config);
...
function renderAPISecurityPolicyPrimaryConfiguration(id, config, inventory, icons) {
    if (id === 'runtime')
        return renderRuntimePolicyConfiguration(config, icons);
    if (id === 'contract')
        return renderContractPolicyConfiguration(config, icons);
    if (id === 'authorization')
        return renderAuthorizationPolicyConfiguration(config, inventory, icons);
    if (id === 'graphql')
        return renderGraphQLPolicyConfiguration(config, inventory, icons);
...
function renderAPISecurityControlSection(title, description, icon, content, flush = false) {
    return `
...
function renderRuntimePolicyConfiguration(config, icons) {
    const injection = getInjectionConfig(config);
    const payload = getPayloadAbuseConfig(config);
    return `
        ${renderAPISecurityControlSection('Protection modules', 'Enable, monitor, or block the runtime protections that make up this policy.', icons.shield, getAPISecurityPolicyRows('runtime', config, icons).map(renderAPISecurityPolicyRow).join(''), true)}
...
function renderContractPolicyConfiguration(config, icons) {
    return renderAPISecurityControlSection('Contract controls', 'Choose how attached schemas are validated and which contract violations are blocked.', icons.fileText, getAPISecurityPolicyRows('contract', config, icons).map(renderAPISecurityPolicyRow).join(''), true);
}
function renderAuthorizationPolicyConfiguration(config, inventory, icons) {
    const bola = config.bola || {};
    const bfla = config.bfla || {};
    const massAssignment = config.mass_assignment || {};
...
function renderAuthorizationControlList(bfla, bola, massAssignment, icons) {
    const controls = [
...
function renderPolicyStateControl(title, enabled, mode, enabledPath, modePath) {
    const normalizedMode = normalizeAPISecurityMode(mode);
    return `
...
function renderBFLAPolicySettings(config, inventory = []) {
...
function renderObjectAuthorizationPolicySettings(config, inventory = []) {
...
function renderMassAssignmentPolicySettings(config, inventory = []) {
...
function renderGraphQLPolicyConfiguration(config, _inventory, icons) {
    const graphql = getGraphQLConfig(config);
    const selectedControl = APISecurityConfig.selectedGraphQLControl;
    if (!selectedControl)
        return renderGraphQLControlList(graphql, icons);
...
function renderGraphQLScopeTable(paths, showToolbar = true) {
...
function renderGraphQLControlList(graphql, icons) {
...
function renderGraphQLLimitsConfiguration(graphql, icons) {
    const activePreset = getGraphQLLimitPreset(graphql);
...
function renderGraphQLSchemaConfiguration(graphql, icons) {
...
function renderAPISecurityListTableSection(title, description, icon, path, values, options) {
    const normalizedPath = normalizeAPISecurityStringListPath(path);
    const items = normalizeStringList(values, []);
    const suggestions = mergeAPISecurityStringLists(options.suggestions || [], getAPISecurityStringListInventoryOptions(options.inventory || [], options.inventoryFilter));
...
function renderAbusePolicyConfiguration(config, inventory, icons) {
    const automation = getAutomationAbuseConfig(config);
    return `
...
function renderDataExposurePolicyConfiguration(config, inventory, icons) {
    const dataExposure = getDataExposureConfig(config);
    return `
...
function renderAPISecurityWizardDrawer(activeWizard, wizardStep, config, inventory, setupFocus, schemaPrefillEndpointKey) {
    if (!activeWizard)
        return '';
    const title = {
...
function renderAPISecurityWizardSteps(activeWizard, currentStep) {
    const max = getAPISecurityWizardMaxStep(activeWizard);
    if (max <= 1)
        return '<div class="apisecurity-chip-row"><span class="apisecurity-chip is-active">Ready</span></div>';
...
function renderSchemaWizardContent(config, inventory, schemaPrefillEndpointKey, wizardStep) {
    const validation = getValidationConfig(config);
    return `
...
function renderIdentityWizardContent(config, wizardStep) {
    const authTokens = config.auth_tokens || {};
...
function renderDataExposureWizardContent(config, wizardStep) {
    const dataExposure = getDataExposureConfig(config);
...
function renderAPISecuritySchemaUploadForm(inventory, schemaPrefillEndpointKey) {
    const prefillIndex = schemaPrefillEndpointKey
        ? inventory.findIndex(endpoint => {
            const k = getInventoryEndpointKey(endpoint);
            if (k === schemaPrefillEndpointKey)
                return true;
            if (endpoint.id && normalizeInventoryEndpointKey(endpoint.id) === schemaPrefillEndpointKey)
                return true;
...
function renderNoAdditionalSettingsControls() {
...
function renderRuntimeDetectionControls(config, icons) {
    const injection = getInjectionConfig(config);
    const ssrf = getSSRFConfig(config);
    const vectorRows = [
...
function renderContractDetectionControls() {
    return renderNoAdditionalSettingsControls();
}
function renderAuthorizationDetectionControls(config, icons) {
    const bola = config.bola || {};
    const bolaRows = `
...
function renderGraphQLDetectionControls() {
    return renderNoAdditionalSettingsControls();
}
function renderAbuseDetectionControls(config, icons) {
    const automation = getAutomationAbuseConfig(config);
    const detectorRows = `
...
function renderDataExposureDetectionControls(config, icons) {
    const dataExposure = getDataExposureConfig(config);
    const detectorRows = `
...
function renderDiscoverySettingsControls(config, icons, inventory) {
...
function renderAPISecurityToggleSettingRow(title, path, enabled, icon, description = 'Enable or disable this protection.') {
    return SectionUI.renderOperatorControlRow({
        title,
        description,
        icon,
...
function renderAPISecurityPolicyRow(row) {
    const mode = normalizeAPISecurityMode(row.mode || 'detect');
...
function renderAPISecurityScopePicker(target, filters, selectedPaths, inventory, choice = 'selected', selectedHost = '') {
    if (!target)
        return '';
...
function renderScopePickerSelect(label, key, value, options) {
    return `
        <label class="bot-overview-field">
            <span>${escapeSectionHtml(label)}</span>
...
function deriveAPISecurityProfile(config, key) {
    const normalized = normalizeAPISecurityConfig(config);
    if (key === 'runtime') {
        ...
        if (allVectorsOn && ssrfDefaults && injection.sensitivity === 'medium' && normalizePayloadSensitivity(payload.sensitivity) === 'balanced')
            return 'default';
        if (allVectorsOn && ssrfDefaults && injection.sensitivity === 'high' && normalizePayloadSensitivity(payload.sensitivity) === 'strict')
            return 'strict';
    }
    if (key === 'graphql') {
        const graphql = getGraphQLConfig(normalized);
        if (graphql.enabled !== false && normalizeAPISecurityMode(String(graphql.mode || 'detect')) === 'detect' && graphql.block_introspection === false)
            return 'monitor';
        if (graphql.max_depth === 15 && graphql.max_aliases === 25 && graphql.max_batch_size === 5 && graphql.max_complexity === 500 && graphql.block_introspection !== false)
            return 'default';
        if (graphql.max_depth === 10 && graphql.max_aliases === 10 && graphql.max_batch_size === 3 && graphql.max_complexity === 250 && graphql.block_introspection !== false)
            return 'strict';
    }
    if (key === 'contract') {
        ...
        if (enabled && mode === 'detect' && validation.block_unknown_endpoints === false && validation.block_invalid_methods === false && validation.block_invalid_content_types === false)
            return 'monitor';
        if (enabled && mode === 'detect' && methodAndContentTypeOn && validation.block_unknown_endpoints !== true)
            return 'default';
        if (enabled && mode === 'block' && methodAndContentTypeOn && validation.block_unknown_endpoints === true)
            return 'strict';
    }
    if (key === 'authorization') {
        ...
        if (allEnabled && modes.every(mode => mode === 'detect'))
            return 'default';
        if (allEnabled && modes.every(mode => mode === 'block'))
            return 'strict';
    }
    if (key === 'automation') {
...
function applyAPISecurityMonitorProfile(config, key) {
    const next = normalizeAPISecurityConfig(config);
...
function applyAPISecurityProfile(config, key, value) {
    const next = normalizeAPISecurityConfig(config);
    if (value === 'custom')
        return next;
    if (value === 'monitor')
        return applyAPISecurityMonitorProfile(next, key);
    if (key === 'runtime') {
        next.injection = {
            ...(next.injection || {}),
            sqli: true,
...
function normalizeAPISecurityStringListPath(path) {
    const normalizedPath = String(path || '').trim();
    return API_SECURITY_STRING_LIST_PATHS.has(normalizedPath) ? normalizedPath : '';
}
function isSupportedAPISecurityScopePath(path) {
    return API_SECURITY_SCOPE_PICKER_PATHS.has(path);
}
function getAPISecurityStringListAtPath(config, path) {
...
function getAPISecurityFunctionSettingsMirrorPath(path) {
...
function setAPISecurityStringListAtPath(config, path, items) {
    const normalizedPath = normalizeAPISecurityStringListPath(path);
    if (!normalizedPath)
        return normalizeAPISecurityConfig(config);
    const next = normalizeAPISecurityConfig(config);
    ...
    if (mirrorPath)
        setAPISecurityConfigValue(next, mirrorPath, list);
    return normalizeAPISecurityConfig(next);
}
function mergeAPISecurityStringLists(primary, secondary) {
    const seen = new Set();
...
function normalizeAPISecurityWizardId(value) {
    const id = String(value || '').trim();
    if (id === 'schema' || id === 'identity' || id === 'data')
        return id;
    return '';
}
function getAPISecurityWizardMaxStep(wizard) {
    if (wizard === 'schema' || wizard === 'identity')
        return 3;
    if (wizard === 'data')
        return 2;
    return 1;
}
function normalizeAPISecuritySurfaceChoice(value) {
    const choice = String(value || '').trim();
    if (choice === 'all' || choice === 'schema_backed' || choice === 'host' || choice === 'manual')
        return choice;
    return 'selected';
}
function getSurfaceChoicePaths(inventory, choice, host) {
...
function getSelectedScopePathsInInventoryOrder(inventory, selectedPaths) {
    const selected = new Set(selectedPaths);
    const ordered = mergeAPISecurityStringLists(inventory.map(endpoint => endpoint.path || '').filter(path => selected.has(path)), []);
...
function getScopePickerOptions(inventory, read) {
    return mergeAPISecurityStringLists([], inventory.map(read).filter(Boolean)).sort((a, b) => a.localeCompare(b));
}
function getFilteredScopePickerEndpoints(inventory, filters) {
    const search = String(filters.search || '').trim().toLowerCase();
    return inventory.filter(endpoint => {
        ...
        if (search && ![path, method, host, schema, risk].some(value => value.toLowerCase().includes(search)))
            return false;
        if (filters.host && filters.host !== 'all' && host !== filters.host)
            return false;
        if (filters.method && filters.method !== 'all' && method !== filters.method)
            return false;
        if (filters.schema && filters.schema !== 'all' && schema !== filters.schema)
            return false;
        if (filters.risk && filters.risk !== 'all' && risk !== filters.risk)
            return false;
        if (filters.active && filters.active !== 'all' && active !== filters.active)
            return false;
        return true;
    });
}
function parseAPISecurityValue(element) {
...
function parseAPISecurityDatasetValue(value, type) {
    if (type === 'checkbox')
        return value === 'true';
    if (type === 'number') {
...
function openAPISecurityDashboardInvestigation(target) {
...
function consumeAPISecurityConfigFocus(runtime) {
...
function ensureAPISecurityBindings() {
    ...
    AdminEvents.delegateEvent(document, 'click', '[data-api-security-action]', (_event, target) => {
        ...
        switch (action) {
            ...
            case 'set-config-field':
                APISecurityConfig.requestConfigFieldUpdate(target.dataset.apiPath || '', parseAPISecurityDatasetValue(target.dataset.apiValue || '', target.dataset.apiValueType || 'string'), true);
                break;
            ...
            case 'open-scope-picker':
                APISecurityConfig.openScopePicker(target.dataset.apiScopePath || '', target.dataset.apiScopeTitle || '', target.dataset.apiScopeMode || 'include', target.dataset.apiTab || '');
                break;
            ...
            case 'apply-manual-scope-path': {
                const input = AdminDOM.getById('apisecurity-scope-manual-path');
            ...
            case 'open-block-review':
                APISecurityConfig.openBlockingReview(target.dataset.apiModuleKey || '', target.dataset.apiFunctionKey || '', target.dataset.apiReviewTitle || '', target.dataset.apiModePath || '', target.dataset.apiTab || '');
                break;
            ...
            case 'save-inventory-view': {
                const menu = target.closest('.apisecurity-inventory-view-popover');
                const input = menu?.querySelector('[data-api-inventory-view-name]');
            ...
            case 'add-string-list-item': {
                ...
                const editor = target.closest('[data-api-string-list-editor]');
                const input = editor?.querySelector('[data-api-string-list-new]');
            ...
            case 'add-nested-list-item': {
                const editor = target.closest('[data-api-nested-list-editor]');
                const input = editor?.querySelector('[data-api-nested-list-new]');
                APISecurityConfig.addNestedListItem(target.dataset.apiNestedListKind, Number(target.dataset.apiNestedListParentIndex || 0), target.dataset.apiNestedListField || '', input?.value || '');
                break;
            }
            case 'remove-nested-list-item':
                APISecurityConfig.removeNestedListItem(target.dataset.apiNestedListKind, Number(target.dataset.apiNestedListParentIndex || 0), target.dataset.apiNestedListField || '', Number(target.dataset.apiNestedListIndex || 0));
                break;
            case 'toggle-nested-list-value': {
                ...
                const kind = target.dataset.apiNestedListKind;
                if (itemIndex >= 0) {
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-path]', (_event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'change', '#apisecurity-schema-endpoint', (_event, target) => {
        const endpoint = target.value === '' ? null : APISecurityConfig.inventory[Number(target.value)] || null;
        const methodInput = AdminDOM.getById('apisecurity-schema-method');
        const pathInput = AdminDOM.getById('apisecurity-schema-path');
        const hostInput = AdminDOM.getById('apisecurity-schema-host');
        const typeInput = AdminDOM.getById('apisecurity-schema-type');
        if (!methodInput || !pathInput || !hostInput)
            return;
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-policy-function-state]', (_event, target) => {
        APISecurityConfig.setPolicyFunctionState(target.dataset.apiEnabledPath || '', target.dataset.apiModePath || '', target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-policy-boolean-state]', (_event, target) => {
        APISecurityConfig.setPolicyBooleanState(target.dataset.apiStatePath || '', target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-string-list-index]', (_event, target) => {
        APISecurityConfig.updateStringListItem(target.dataset.apiStringListPath || '', Number(target.dataset.apiStringListIndex || 0), target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-string-list-picker]', (_event, target) => {
        if (!target.value)
            return;
        APISecurityConfig.addStringListItem(target.dataset.apiStringListPicker || '', target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-nested-list-picker]', (_event, target) => {
        if (!target.value)
            return;
        APISecurityConfig.addNestedListItem(target.dataset.apiNestedListKind, Number(target.dataset.apiNestedListParentIndex || 0), target.dataset.apiNestedListField || '', target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-nested-list-index]', (_event, target) => {
        APISecurityConfig.updateNestedListItem(target.dataset.apiNestedListKind, Number(target.dataset.apiNestedListParentIndex || 0), target.dataset.apiNestedListField || '', Number(target.dataset.apiNestedListIndex || 0), target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-policy-enabled]', (_event, target) => {
        APISecurityConfig.setPolicyEnabled(target.dataset.apiPolicyEnabled || '', target.checked);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-section-enabled]', (_event, target) => {
        APISecurityConfig.sectionEnabled = target.checked;
        APISecurityConfig.render();
        SectionUI.markSaveActionBarDirty('data-api-security-action');
    });
    AdminEvents.delegateEvent(document, 'keydown', '[data-api-security-action="switch-tab"]', (event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'keydown', '[data-api-security-action="open-policy-detail"]', (event, target) => {
        if (!(event instanceof KeyboardEvent))
            return;
        if (event.key !== 'Enter' && event.key !== ' ')
            return;
    ...
    AdminEvents.delegateEvent(document, 'keydown', '[data-api-security-action="open-settings-category"]', (event, target) => {
        if (!(event instanceof KeyboardEvent))
            return;
        if (event.key !== 'Enter' && event.key !== ' ')
            return;
    ...
    AdminEvents.delegateEvent(document, 'keydown', '[data-api-security-action="open-authorization-control"]', (event, target) => {
        if (!(event instanceof KeyboardEvent))
            return;
        if (event.key !== 'Enter' && event.key !== ' ')
            return;
    ...
    AdminEvents.delegateEvent(document, 'keydown', '[data-api-security-action="open-graphql-control"]', (event, target) => {
        if (!(event instanceof KeyboardEvent))
            return;
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-scope-filter]', (_event, target) => {
        const key = target.dataset.apiScopeFilter;
        if (!key)
            return;
        APISecurityConfig.updateScopePickerFilter(key, target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-surface-host]', (_event, target) => {
        APISecurityConfig.updateScopePickerHost(target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-scope-path-option]', (_event, target) => {
        APISecurityConfig.toggleScopePickerPath(target.dataset.apiScopePathOption || '', target.checked);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-inventory-meta-field]', (_event, target) => {
        APISecurityConfig.updateInventoryMetadataField(target.dataset.apiEndpointKey || '', target.dataset.apiInventoryMetaField || '', parseAPISecurityValue(target));
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-inventory-filter]', (_event, target) => {
        const key = target.dataset.apiInventoryFilter;
        if (!key)
            return;
        APISecurityConfig.updateInventoryFilter(key, target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-inventory-group-by]', (_event, target) => {
        APISecurityConfig.setInventoryGroupBy(target.value);
    });
    AdminEvents.delegateEvent(document, 'input', '[data-api-inventory-filter="search"]', (_event, target) => {
        APISecurityConfig.updateInventoryFilter('search', target.value);
    });
    AdminEvents.delegateEvent(document, 'input', '[data-api-settings-search]', (_event, target) => {
        APISecurityConfig.updateSettingsSearch(target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-api-bola-rule-field]', (_event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-auth-issuer-field]', (_event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-bfla-rule-field]', (_event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-api-mass-rule-field]', (_event, target) => {
...
const APISecurityConfigState = {
    get containerId() { return APISecurityConfig.containerId; },
    set containerId(value) { APISecurityConfig.containerId = value; },
    get currentTab() { return APISecurityConfig.currentTab; },
    set currentTab(value) { APISecurityConfig.currentTab = value; },
    get selectedSettingsCategory() { return APISecurityConfig.selectedSettingsCategory; },
    set selectedSettingsCategory(value) { APISecurityConfig.selectedSettingsCategory = value; },
    get settingsSearch() { return APISecurityConfig.settingsSearch; },
    set settingsSearch(value) { APISecurityConfig.settingsSearch = value; },
    get changedSettings() { return APISecurityConfig.changedSettings; },
    set changedSettings(value) { APISecurityConfig.changedSettings = value; },
    get selectedPolicyEditorTab() { return APISecurityConfig.selectedPolicyEditorTab; },
    set selectedPolicyEditorTab(value) { APISecurityConfig.selectedPolicyEditorTab = value; },
    get config() { return APISecurityConfig.config; },
    set config(value) { APISecurityConfig.config = normalizeAPISecurityConfig(value); },
    get sectionEnabled() { return APISecurityConfig.sectionEnabled; },
    set sectionEnabled(value) { APISecurityConfig.sectionEnabled = value; },
    get stats() { return APISecurityConfig.stats; },
    set stats(value) { APISecurityConfig.stats = value; },
    get dashboard() { return APISecurityConfig.dashboard; },
    set dashboard(value) { APISecurityConfig.dashboard = value; },
    get inventory() { return APISecurityConfig.inventory; },
    set inventory(value) { APISecurityConfig.inventory = value; },
    get overviewLoading() { return APISecurityConfig.overviewLoading; },
    set overviewLoading(value) { APISecurityConfig.overviewLoading = value; },
    get overviewError() { return APISecurityConfig.overviewError; },
    set overviewError(value) { APISecurityConfig.overviewError = value; },
    get tabs() { return APISecurityConfig.tabs; },
    set tabs(value) { APISecurityConfig.tabs = value; },
    get icons() { return APISecurityConfig.icons; },
    set icons(value) { APISecurityConfig.icons = value; }
};
const APISecurityConfigService = {
    init: (containerId) => APISecurityConfig.init(containerId),
    loadConfig: () => APISecurityConfig.loadConfig(),
    loadStats: () => APISecurityConfig.loadStats(),
    loadOverviewData: () => APISecurityConfig.loadOverviewData(),
    saveConfig: () => APISecurityConfig.saveConfig()
};
const APISecurityConfigView = {
    render: () => APISecurityConfig.render(),
    renderTabContent: () => APISecurityConfig.renderTabContent(),
    renderOverview: () => APISecurityConfig.renderOverview(),
    renderInventory: () => APISecurityConfig.renderInventory(),
    renderSettings: () => APISecurityConfig.renderSettings(),
    renderSaveButtons: renderAPISecuritySaveButtons,
    showToast: showSectionToast
};
const APISecurityConfigController = {
    switchTab: (id) => APISecurityConfig.switchTab(id),
    switchSettingsCategory: (id) => APISecurityConfig.switchSettingsCategory(id),
    updateSettingsSearch: (value) => APISecurityConfig.updateSettingsSearch(value),
    switchPolicyEditorTab: (id) => APISecurityConfig.switchPolicyEditorTab(id),
    updateField: (path, value) => APISecurityConfig.updateField(path, value),
    requestConfigFieldUpdate: (path, value, renderAfterUpdate) => APISecurityConfig.requestConfigFieldUpdate(path, value, renderAfterUpdate),
    updateInventoryFilter: (key, value) => APISecurityConfig.updateInventoryFilter(key, value),
    updateBolaRule: (index, field, value) => APISecurityConfig.updateBolaRule(index, field, value),
    addBolaRule: () => APISecurityConfig.addBolaRule(),
    removeBolaRule: (index) => APISecurityConfig.removeBolaRule(index),
    updateAuthIssuer: (index, field, value) => APISecurityConfig.updateAuthIssuer(index, field, value),
    addAuthIssuer: () => APISecurityConfig.addAuthIssuer(),
    removeAuthIssuer: (index) => APISecurityConfig.removeAuthIssuer(index),
    updateBFLARule: (index, field, value) => APISecurityConfig.updateBFLARule(index, field, value),
    addBFLARule: () => APISecurityConfig.addBFLARule(),
    removeBFLARule: (index) => APISecurityConfig.removeBFLARule(index),
    updateMassAssignmentPathRule: (index, field, value) => APISecurityConfig.updateMassAssignmentPathRule(index, field, value),
    addMassAssignmentPathRule: () => APISecurityConfig.addMassAssignmentPathRule(),
    removeMassAssignmentPathRule: (index) => APISecurityConfig.removeMassAssignmentPathRule(index),
    uploadSchema: () => APISecurityConfig.uploadSchema()
};
const apiSecurityServiceMethods = new Set([
    'init',
    'loadConfig',
    'loadStats',
    'loadOverviewData',
...
const apiSecurityViewMethods = new Set([
    'render',
    'renderTabContent',
    'renderOverview',
    'renderInventory',
...
export const APISecurityConfigFacade = new Proxy({}, {
    get(_target, prop) {
        if (prop === 'state')
            return APISecurityConfigState;
        if (typeof prop !== 'string')
            return undefined;
        if (apiSecurityServiceMethods.has(prop)) {
            return APISecurityConfigService[prop];
        }
        if (apiSecurityViewMethods.has(prop)) {
            return APISecurityConfigView[prop];
        }
        if (prop in APISecurityConfigController) {
            return APISecurityConfigController[prop];
        }
        const value = APISecurityConfig[prop];
        return typeof value === 'function' ? value.bind(APISecurityConfig) : value;
    },
    set(_target, prop, value) {
        if (typeof prop === 'string' && prop in APISecurityConfig) {
            APISecurityConfig[prop] = value;
            return true;
        }
        return false;
    }
