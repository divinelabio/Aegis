function renderClassificationPolicyItem(title, desc, fieldPath, checked, isLast = false) {
    return `
...
function renderGoodBotsModal(bots, config, isGoodBotModalOpen, goodBotFilter, deps) {
    if (!isGoodBotModalOpen)
        return '';
    ...
    const filteredBots = bots.filter((b) => b.toLowerCase().includes(normalizedFilter));
...
export function renderAntibotClassificationTab(config, goodBots, goodBotFilter, isGoodBotModalOpen, deps) {
    const c = config.classification || {};
    const totalGoodBots = goodBots.length;
    const rules = c.good_bot_rules || {};
    const areGoodBotsAllowed = c.allow_good_bots !== false;
    const previewLimit = 8;
    const previewBots = goodBots.slice(0, previewLimit).map((bot) => {
        const isDisabled = rules[bot]?.action === 'disabled';
        return `
            ...
            <div class="section-classification-grid">
                ...
                <div class="card section-class-policy-card">
                    ...
                    <div class="section-class-policy-list">
                        ${renderClassificationPolicyItem('Block Known Scrapers', 'Instantly block automated scraping frameworks and reconnaissance tools.', 'classification.block_bad_bots', c.block_bad_bots !== false)}
                        ${renderClassificationPolicyItem('Reverse DNS Verification', 'Verify legitimate search engine crawlers via reverse DNS and forward lookup.', 'classification.verify_search_engines', c.verify_search_engines !== false)}
                        ${renderClassificationPolicyItem('Strict Mode', 'Treat clients with empty or unrecognized User-Agent headers as bots.', 'classification.treat_unknown_as_bot', c.treat_unknown_as_bot === true, true)}
                    </div>
                </div>
            </div>
