...
export function renderAntibotOverviewTab(config, analytics, rules, formatNumber) {
...
function countProtectedSignals(config) {
    const signals = [
...
function renderCompactOperatorIntelligence(config, analytics, formatNumber) {
...
function decisionSnapshotItem(label, value) {
...
function renderCompactRecentEvents(events, expandedId) {
...
function renderCompactEventRows(event, expanded) {
...
function buildTuningRecommendations(config, events) {
...
function challengeTrendBy(events, keyFn) {
...
function exceptionAction(label, key, target) {
...
function quickRuleAction(label, kind, target, action) {
    return quickRuleButton(label, kind, target, action);
}
function tabAction(label, tab) {
...
function renderProtectionModules(config, rules, formatNumber) {
    ...
    return SectionUI.renderOperatorSection('Security functions', `
        ...
        icon: SectionUI.icons[module.icon] || SectionUI.icons.shield,
...
function renderManagedProtectionSettings(config, rules, _formatNumber) {
    const detection = config.detection || {};
    const strictness = normalizeMode(config.strictness || config.mode || detection.mode || 'balanced');
    const responseMode = normalizeMode(config.response_mode || config.enforcement_mode || 'challenge');
...
export function renderAntibotManagedProtectionTab(config, rules, formatNumber) {
...
function quickRuleButton(label, kind, target, action) {
    return `<button type="button" class="btn btn-outline btn-sm"
...
function formatStrictness(mode) {
    const labels = {
        low: 'Low',
        balanced: 'Balanced',
        high: 'High',
...
function formatThresholdLabel(label, value) {
...
function countExceptions(config) {
