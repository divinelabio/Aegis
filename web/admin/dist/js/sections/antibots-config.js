...
import { openSecurityRuleModal } from '../security.js';
...
import { formatAntibotNumber, renderAntibotSaveButton, showAntibotToast } from './antibots-runtime-helpers.js';
import { closeAntibotRuleModal, confirmDeleteAntibotRuleAt, moveAntibotRule, promptDeleteAntibotRule, toggleAntibotRuleStatus } from './antibots-rules-lifecycle.js';
import { readNewAntibotRuleFromForm, selectAntibotRuleAction, toggleAntibotRuleFields } from './antibots-rules-form.js';
...
import { ANTIBOT_GOOD_BOTS, ANTIBOT_MODE_PRESETS, ANTIBOT_TABS, antibotControl, antibotErrorMessage, antibotInput, antibotSelect, antibotValue, asAntibotConfigState, botConfigForSave, asAntibotRules, asAntibotStatsState, createDefaultAntibotAnalyticsState, createDefaultAntibotConfigState, isAntibotModeName, setAntibotField } from './antibots-config-shared.js';
import { parseDatasetBoundValue } from './field-value-parser.js';
function antibotNode(id) {
    const element = antibotControl(id);
    return element instanceof HTMLElement ? element : null;
}
function parseAntibotDatasetValue(target) {
    return parseDatasetBoundValue(target, target.dataset.antibotValueType);
}
function createInitialAntibotLoadError() {
    return null;
}
...
function readAnalyticsFilters(state) {
    return {
...
function normalizeAntibotRuleAction(action) {
    const actionMap = { '0': 'allow', '1': 'block', '2': 'challenge', '5': 'log' };
    const raw = String(action ?? 'block');
    return actionMap[raw] || raw;
}
function normalizeEventRuleAction(action) {
    const value = action.toLowerCase();
    if (value === 'allow' || value === 'challenge' || value === 'log' || value === 'rate_limit')
        return value;
    return 'block';
}
function buildEventSecurityRule(kind, target, action) {
...
function eventRuleExpression(kind, target) {
...
function eventRuleName(kind, target, action) {
...
function toSecurityRuleFromAntibot(rule) {
    return {
        id: rule.id || '',
...
async function quietAntibotGet(endpoint) {
    try {
...
const renderAntibotSectionHeader = (runtime) => runtime.renderSectionHeader.bind(runtime);
const AntibotsConfig = {
    ...
    async loadRules() {
        try {
            const data = await quietAntibotGet('v2/security/bot/rules?section=bot');
    ...
    async loadAnalytics() {
        ...
        try {
            ...
            const [summary, events, timeline, rules, ips, countries, paths, categories, userAgents, decisionSources, decisionStates, challengeTypes, challengeOutcomes, ruleOverrides] = await Promise.all([
                quietAntibotGet(`v2/analytics/summary?${base.toString()}`),
                quietAntibotGet(`v2/analytics/events?${withLimit(base, 100).toString()}`),
                quietAntibotGet(`v2/analytics/timeline?${base.toString()}`),
                quietAntibotGet(`v2/analytics/rules?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-ips?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/geo?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-paths?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-bot-categories?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-user-agents?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-decision-sources?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-decision-states?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-challenge-types?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-challenge-outcomes?${withLimit(base, 8).toString()}`),
                quietAntibotGet(`v2/analytics/top-rule-overrides?${withLimit(base, 8).toString()}`)
            ]);
    ...
    async createRuleFromEvent(kind, target, action) {
        ...
        const created = await api.post('v2/security/bot/rules', rule);
        if (!created) {
    ...
    syncDynamicStyles(root = document) {
        const logoEls = root.querySelectorAll('.section-logo[data-logo-bg]');
        logoEls.forEach((el) => {
            const raw = el.dataset.logoBg ? decodeURIComponent(el.dataset.logoBg) : '';
            if (raw)
                el.style.setProperty('--section-logo-bg', raw);
        });
        const zoneEls = root.querySelectorAll('.section-threshold-zones-bar[data-challenge][data-block]');
        zoneEls.forEach((el) => {
    ...
    switchTab(id) {
        this.currentTab = this.tabs.some((tab) => tab.id === id) ? id : 'overview';
        this.render();
        const targetMap = {
    ...
    // ========== OVERVIEW TAB ==========
    renderOverview() {
        return renderAntibotOverviewTab(this.config, this.analytics, this.rules, formatAntibotNumber);
    },
    renderManagedProtection() {
        return renderAntibotManagedProtectionTab(this.config, this.rules, formatAntibotNumber);
    },
    ...
    isCustomMode() {
        ...
        for (const [modeName, preset] of Object.entries(this.MODE_PRESETS)) {
            ...
            if (matches && current.mode === modeName)
                return false;
        }
        return true;
    },
    ...
    renderClassification() {
        return renderAntibotClassificationTab(this.config, this.goodBots, this.goodBotFilter, this.isGoodBotModalOpen, {
    ...
    toggleGoodBot(botName) {
        if (!this.config.classification)
            this.config.classification = {};
        if (!this.config.classification.good_bot_rules)
            this.config.classification.good_bot_rules = {};
    ...
    renderExceptions() {
        return renderAntibotExceptionsTab(this.config, renderAntibotSectionHeader(this), renderAntibotSaveButton, this.isAddExceptionModalOpen);
    },
    updateField(path, value) {
        setAntibotField(this.config, path, value);
    },
    updateAIRule(botName, action) {
        if (!this.config.ai_crawlers)
            this.config.ai_crawlers = {};
        if (!this.config.ai_crawlers.rules)
            this.config.ai_crawlers.rules = {};
        this.config.ai_crawlers.rules[botName] = { action };
    },
    applyExceptionList(key, value) {
        if (!['ips', 'user_agents', 'paths', 'methods', 'headers'].includes(key))
            return;
    ...
    async addExceptionValue(key, value) {
        const clean = value.trim();
        if (!clean || !['ips', 'user_agents', 'paths', 'methods', 'headers'].includes(key))
            return;
        if (!this.config.whitelist)
            this.config.whitelist = {};
    ...
    async saveExceptionFromModal() {
        const typeEl = document.querySelector('input[name="bot-exception-type"]:checked');
    ...
    async removeExceptionValue(key, value) {
        const clean = value.trim();
        if (!clean || !['ips', 'user_agents', 'paths', 'methods', 'headers'].includes(key))
            return;
        if (!this.config.whitelist)
    ...
    async confirmDeleteRule(idx) {
        ...
        if (!confirmDeleteAntibotRuleAt(this.rules, idx))
            return;
        this.render();
    },
    moveRule(idx, direction) {
        if (!moveAntibotRule(this.rules, idx, direction))
            return;
        this.render();
        this.saveRules();
    },
    toggleRuleStatus(idx) {
        if (!toggleAntibotRuleStatus(this.rules, idx))
            return;
        this.saveRules();
    },
    async saveRules() {
    ...
    async saveConfig(options) {
        ...
        try {
            ...
            if (success) {
                ...
                this.render();
                if (successMessage)
                    showAntibotToast(successMessage, 'success');
                return true;
            }
            else {
    ...
    async applyAdvancedPreset(scope, preset) {
...
const antibotsComposition = createAntibotsComposition(AntibotsConfig, {
    isModeName: (value) => isAntibotModeName(value),
