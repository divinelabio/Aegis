...
function decisionLabel(value) { return value === 'public_bypass' ? 'Public bypass' : value ? value[0].toUpperCase() + value.slice(1) : 'Not published'; }
function policyStatus(value) { return SectionUI.renderStatusPill(value === 'deny_only' ? 'Deny-only' : value.replace(/_/g, ' '), statusTone(value === 'deny_only' ? 'warning' : value)); }
function policyDecisionStatus(value) {
...
function attachmentScope(item) {
    return `<span class="text-mono">${escapeHTML(item.attachment.path)}</span>`;
}
function principalSummary(item) {
    if (item.decision === 'public_bypass')
        return 'Identity not required';
    if (item.principals.any_authenticated)
        return 'Any authenticated identity';
...
export function renderAttachedPolicySummary(state) {
    ...
    if (wait)
        return SectionUI.renderOperatorSection('Attached policies', wait, {
...
export function renderAttachedPolicies(state) {
    const resource = state.policies.appDetail;
    const wait = pending(resource.loading, resource.error, 'policy-app-retry', 'policy attachments');
    if (wait)
        return SectionUI.renderOperatorSection('Attached policies', wait, { subtitle: 'Reusable policies assigned to this application.', className: 'edge-access-v2-app-page-section edge-access-v2-policies-section' });
    const value = resource.value;
    if (!value)
        return SectionUI.renderOperatorSection('Attached policies', SectionUI.renderEmptyState('Policies unavailable', 'This application policy data could not be loaded.', 'shield'), { subtitle: 'Reusable policies assigned to this application.', className: 'edge-access-v2-app-page-section edge-access-v2-policies-section' });
    const rows = value.attachments.map(item => {
        ...
        const rowActions = [
            ...
            { label: 'Detach policy', attrs: `data-edge-access-v2-action="policy-detach-attachment" data-edge-access-v2-attachment-id="${attachmentID}"`, tone: 'danger' }
        ];
        return [
    ...
    const rows = (resource.value || []).map(item => {
        ...
        const rowActions = [
            ...
            ...(!item.policy.archived
                ? [{ label: 'Archive policy', attrs: `data-edge-access-v2-action="policy-archive" ${policyAttrs}`, tone: 'danger' }]
                : [])
        ];
        return [
...
function valueList(values) { return values.join(', '); }
function renderPolicyEditor(state) {
    ...
    const firstAttachment = pendingFirstAttachment || { path: '/*', methods: [], channels: [] };
...
function decisionStatus(result) { return SectionUI.renderStatusPill(result, statusTone(result === 'allow' ? 'active' : result === 'deny' ? 'error' : 'warning')); }
function renderDecisions(state) {
    const resource = state.decisions.list;
    const wait = pending(resource.loading, resource.error, 'decisions-refresh', 'decisions');
    const filter = state.decisions.filter;
    ...
    if (wait)
        return SectionUI.renderOperatorSection('View decisions', `${controls}${wait}`, { subtitle: 'Read-only operational evidence for authorization outcomes.' });
...
export function renderV2Policies(state) {
    if (state.route.policyMode !== 'list')
        return renderPolicyEditor(state);
    return renderLibrary(state);
}
export function renderV2Decisions(state) { return renderDecisions(state); }
