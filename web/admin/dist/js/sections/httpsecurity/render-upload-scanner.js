...
export function renderSelectField(label, path, value, options) {
...
export function renderUploadPrimaryControls(upload) {
    return `
...
export function renderContentSignatureCard(title, desc, path, checked) {
...
export function renderUploadAdvancedControls(upload, content, archive, _quarantine) {
    return `
        <div class="httpsec-upload-advanced-grid">
            <div class="httpsec-upload-advanced-group">
                <div class="httpsec-upload-signatures-grid">
                    ${renderContentSignatureCard('Executable Magic Bytes', 'Validates binary magic bytes to block disguised Windows PE, Linux ELF, and Mac Mach-O binaries masked as images or documents.', 'upload_protection.content_rules.detect_executable_bytes', content.detect_executable_bytes !== false)}
                    ${renderContentSignatureCard('Server Code &amp; Web Shells', 'Scans uploaded assets for embedded PHP, JSP, ASPX, Python, and shell script syntax disguised inside media files.', 'upload_protection.content_rules.detect_server_side_code', content.detect_server_side_code !== false)}
                    ${renderContentSignatureCard('Malicious Macro Documents', 'Inspects OLE2 and OOXML containers for embedded VBA macros, auto-exec triggers, and suspicious automation.', 'upload_protection.content_rules.detect_macro_documents', content.detect_macro_documents !== false)}
                    ${renderContentSignatureCard('Scriptable SVG &amp; Polyglots', 'Inspects SVG vector images for embedded &lt;script&gt; tags, onload event handlers, CDATA vectors, and XSS payloads.', 'upload_protection.content_rules.detect_svg_script', content.detect_svg_script !== false)}
                    ${renderContentSignatureCard('Disguised HTML &amp; Phishing', 'Prevents HTML and client script markup from being uploaded under non-HTML extensions to abuse domain trust.', 'upload_protection.content_rules.detect_html_upload', content.detect_html_upload !== false)}
                    ${renderContentSignatureCard('Active PDF &amp; Embedded Exploits', 'Inspects PDF documents for embedded /JavaScript, /Launch actions, /OpenAction triggers, and hidden binary droppers.', 'upload_protection.content_rules.detect_pdf_exploits', content.detect_pdf_exploits !== false)}
                    ${renderContentSignatureCard('High-Entropy &amp; Packed Payloads', 'Analyzes Shannon entropy to detect UPX-packed binaries, encrypted shellcode stubs, and obfuscated ransomware loaders.', 'upload_protection.content_rules.detect_high_entropy', content.detect_high_entropy !== false)}
                </div>
            </div>
            <div class="httpsec-upload-advanced-group">
