...
export function httpsecSizeToMb(value) {
...
export function encodeHTTPSecurityValue(value) {
    return encodeURIComponent(String(value));
}
export function renderPayloadPresetButton(active, title, attrs) {
    return `
...
export function renderBasicBodyCeiling(view, limit, currentSize, currentSizeMb) {
    const sizePresets = [
    ...
    return `
        <div class="httpsec-payload-limit-editor">
            ...
            <div class="httpsec-payload-limit-controls">
                ...
                <div class="httpsec-payload-limit-presets"><span>Common limits</span><div class="httpsec-request-presets" aria-label="Request body size presets">${sizePresets.map((p) => `<button type="button" data-httpsec-action="set-field" data-httpsec-path="upload_limit.max_body_size" data-httpsec-value="${encodeHTTPSecurityValue(p.value)}" data-httpsec-render="true" class="httpsec-request-preset ${currentSize === p.value ? 'is-active' : ''}">${p.label}</button>`).join('')}</div></div>
            </div>
        </div>`;
}
export function renderPayloadEnforcementProfiles(view, bodyProfile, inspectionProfile, customInspectionSelected, canOpenPayloadExpert) {
    return `
