...
function renderToggleCard(view, title, desc, active, path, icon) {
...
function renderSecurityTxtCard(view) {
...
export function renderHTTPSecurityResponseProtection(runtime) {
    const view = runtime;
    ...
    const referrerLabel = referrerOptions.find((option) => option.val === referrerCurrent)?.label || 'Do not enforce';
    ...
    const modeCard = (active, title, desc, attrs) => `
            ...
            <section class="httpsec-browser-posture">
                ...
                <label class="httpsec-browser-profile-field">
                    ...
                    <select data-httpsec-profile-select="response" aria-label="Browser protection posture">
                        ${Object.entries(HTTPSEC_RESPONSE_PROFILES).map(([id, profile]) => `<option value="${id}" ${responseProfile.id === id ? 'selected' : ''}>${profile.label}</option>`).join('')}
                    </select>
                </label>
            </section>
    ...
    const privacyContent = `
            <div class="httpsec-privacy-workspace">
                <section class="httpsec-privacy-surface httpsec-privacy-disclosure-surface">
                    ...
                    <div class="httpsec-privacy-disclosure-grid">
                        <label class="httpsec-privacy-policy-field">
                            ...
                            <select data-httpsec-path="security_headers.referrer_policy" data-httpsec-value-type="string" data-httpsec-render="true">${referrerOptions.map((o) => `<option value="${o.val}" ${referrerCurrent === o.val ? 'selected' : ''}>${o.label}</option>`).join('')}</select>
                        </label>
                ...
                <section class="httpsec-privacy-surface httpsec-privacy-headers-surface">
                    ...
                    <div class="httpsec-privacy-header-editor">
                        <div class="httpsec-extension-grid">
                            ${hiddenHeaders.map((h) => `<button class="httpsec-extension-chip" type="button" data-httpsec-action="remove-tag" data-httpsec-path="info_hiding.strip_headers" data-httpsec-value="${encodeHTTPSecurityValue(h)}"><span>${escapeHTTPSecurityHtml(h)}</span><span class="httpsec-extension-remove" aria-hidden="true">&times;</span></button>`).join('') || '<div class="httpsec-empty-note">No server identity headers are hidden yet.</div>'}
                        </div>
              ...
              ${['Server', 'X-Powered-By', 'X-AspNet-Version', 'X-AspNetMvc-Version', 'X-Generator'].filter((h) => !hiddenHeaders.includes(h)).length ? `<div class="httpsec-privacy-suggestions"><span>Common headers</span><div class="httpsec-response-chip-row">${['Server', 'X-Powered-By', 'X-AspNet-Version', 'X-AspNetMvc-Version', 'X-Generator'].filter((h) => !hiddenHeaders.includes(h)).slice(0, 5).map((h) => `<button type="button" data-httpsec-action="add-tag" data-httpsec-path="info_hiding.strip_headers" data-httpsec-value="${encodeHTTPSecurityValue(h)}" class="btn btn-outline btn-xs text-mono">${h}</button>`).join('')}</div></div>` : ''}
                    </div>
                </section>
            </div>
        `;
    const cookieContent = `
            ...
            <div class="httpsec-mode-cards httpsec-mode-cards-4">
                ${sameSiteOptions.map((o) => modeCard(sameSiteCurrent === o.val, o.label, o.desc, `data-httpsec-action="set-field" data-httpsec-path="cookie_hardener.force_samesite" data-httpsec-value="${encodeHTTPSecurityValue(o.val)}" data-httpsec-render="true"`)).join('')}
            </div>
        `;
    const responseHeaderContent = `
        ...
        `;
    const responseAreaRow = (id, title, description, icon, enabled) => SectionUI.renderOperatorControlRow({
        title,
        description,
        icon: view.icons[icon] || view.icons.shield,
        enabled,
