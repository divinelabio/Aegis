...
import { httpsecSizeToMb, renderBasicBodyCeiling, renderPayloadEnforcementProfiles } from './render-payload-basic.js';
import { renderBodyExpertThreshold, renderBodyExpertGuards } from './render-body-guard.js';
import { renderUploadPrimaryControls, renderUploadAdvancedControls } from './render-upload-scanner.js';
export function renderHTTPSecurityPayloadProtection(runtime) {
    return renderHTTPSecurityPayloadUploadWorkspace(runtime, 'payload');
}
export function renderHTTPSecurityUploadSecurity(runtime) {
    return renderHTTPSecurityPayloadUploadWorkspace(runtime, 'upload');
}
function renderHTTPSecurityPayloadUploadWorkspace(runtime, workspace) {
    const view = runtime;
    ...
    const payloadProfiles = renderPayloadEnforcementProfiles(view, bodyProfile, inspectionProfile, customInspectionSelected, canOpenPayloadExpert);
