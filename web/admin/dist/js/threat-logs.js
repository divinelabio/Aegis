...
const threatLogsPageSizes = [25, 50, 100];
const threatLogsState = {
    bound: false,
    loading: false,
    refreshing: false,
    filters: {
...
function renderCountryFlagHtml(countryCode, countryName) {
 ...
 */
export async function loadThreatLogsPage() {
 ...
 */
function normalizeThreatLog(item, index) {
    ...
    return {
        ...
        ruleMessages: Array.isArray(item.rule_messages) ? item.rule_messages : [],
        severity,
        category,
        ...
        matchedFields: Array.isArray(item.matched_fields) ? item.matched_fields : undefined,
...
function applyClientFilters() {
    ...
    threatLogsState.filteredLogs = threatLogsState.logs.filter(log => {
        // Action filter
        if (actionFilter === 'blocked' && !log.blocked)
            return false;
        if (actionFilter === 'allowed' && log.blocked)
            return false;
        // Severity filter
        if (sevFilter !== 'all' && log.severity !== sevFilter)
            return false;
        // Category filter
        if (catFilter !== 'all') {
            const normalizedCat = log.category.toLowerCase().replace(/[\s_-]+/g, '');
            const targetCat = catFilter.toLowerCase().replace(/[\s_-]+/g, '');
            if (!normalizedCat.includes(targetCat))
                return false;
        }
        // Text search filter
        if (query) {
 ...
 */
function renderThreatLogsUI(container) {
 ...
 */
export function openThreatInspector(logId) {
 ...
 */
function bindThreatLogsInteractions() {
    if (threatLogsState.bound)
        return;
    threatLogsState.bound = true;
    // Click delegation
    AdminEvents.delegateEvent(document, 'click', '[data-threat-action]', (event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-threat-filter]', (_event, target) => {
        const filterKey = target.dataset.threatFilter;
    ...
    AdminEvents.delegateEvent(document, 'change', '[data-threat-control="page-size"]', (_event, target) => {
        const newSize = Number(target.value);
        if (threatLogsPageSizes.includes(newSize)) {
    ...
    AdminEvents.delegateEvent(document, 'input', '#threat-filter-search', (_event, target) => {
        if (searchTimeout)
 ...
 */
function detectCategoryFromRule(rule) {
    const r = rule.toLowerCase();
    if (r.includes('sql') || r.includes('sqli'))
        return 'SQL Injection';
    if (r.includes('xss') || r.includes('script'))
        return 'Cross-Site Scripting';
    if (r.includes('traversal') || r.includes('path') || r.includes('../'))
        return 'Path Traversal';
    if (r.includes('rce') || r.includes('command') || r.includes('shell'))
        return 'Remote Code Exec';
    if (r.includes('bot') || r.includes('crawler') || r.includes('fingerprint'))
        return 'Bot & Crawler';
    if (r.includes('rate') || r.includes('flood') || r.includes('burst'))
