...
export async function loadProxySettingsPage() {
...
async function loadProxySettings() {
    try {
        ...
        if (response.status === 404) {
            ...
            const legacy = await api.get('config');
...
function legacyProxySettingsResponse(cfg) {
    const infrastructure = cfg.infrastructure || {};
    const trusted = infrastructure.trusted_proxies || {};
...
function hydrateProxyState(response) {
...
function renderProxySettings(container) {
...
function renderProxyFields(isStandalone) {
    if (isStandalone) {
...
function proxySettingRow(title, description, control, tall = false) {
...
function proxyToggleRow(title, description, id, checked) {
...
function bindProxySettingsEvents() {
    if (proxyState.bindingsReady)
        return;
    proxyState.bindingsReady = true;
    AdminEvents.delegateEvent(document, 'input', '#proxy-settings-container [data-proxy-settings-input]', () => {
        readProxyForm();
        markProxySettingsDirty();
        scheduleProxyGuide();
    });
    AdminEvents.delegateEvent(document, 'change', '#proxy-settings-container #proxy-mode, #proxy-settings-container #proxy-provider', () => {
        readProxyForm();
        markProxySettingsDirty();
        const container = AdminDOM.getById('proxy-settings-container');
        if (container)
    ...
    AdminEvents.delegateEvent(document, 'change', '#proxy-settings-container input[type="checkbox"][data-proxy-settings-input]', () => {
        readProxyForm();
        markProxySettingsDirty();
        scheduleProxyGuide();
    });
    AdminEvents.delegateEvent(document, 'click', '#proxy-settings-container [data-proxy-action="copy-guide-config"]', async () => {
        ...
        try {
            ...
            const btn = AdminDOM.query('[data-proxy-action="copy-guide-config"]');
            if (btn) {
    ...
    AdminEvents.delegateEvent(document, 'click', '#proxy-settings-container [data-proxy-settings-action="save"]', event => {
        event.preventDefault();
        void saveProxySettings();
    });
    AdminEvents.delegateEvent(document, 'click', '#proxy-settings-container [data-proxy-settings-action="reset"]', event => {
        event.preventDefault();
        if (!proxyState.dirty || proxyState.saving)
            return;
...
function markProxySettingsDirty() {
    proxyState.dirty = true;
    SectionUI.markSaveActionBarDirty('data-proxy-settings-action');
}
function buildProxySettingsPayload() {
...
async function saveProxySettings() {
    if (!proxyState.loaded || proxyState.saving)
        return;
    readProxyForm();
    ...
    if (proxyState.endpointAvailable) {
        const result = await api.put('infrastructure/proxy-settings', payload);
...
function scheduleProxyGuide() {
    if (proxyState.guideTimer)
        window.clearTimeout(proxyState.guideTimer);
    proxyState.guideTimer = window.setTimeout(() => void renderProxyGuide(), 250);
}
function getGuideFilename(provider) {
    switch (provider) {
...
async function renderProxyGuide() {
    ...
    if (filename)
        filename.textContent = metaFilename;
    try {
        ...
        const guide = await parseJsonSafe(response, { provider });
        if (controller.signal.aborted)
            return;
        const display = compactGuide(provider, guide.title || '', guide.steps || []);
        if (sectionTitle)
            sectionTitle.textContent = `${display.title} Guide`;
    ...
    `).join('');
        ...
        if (codeWrap)
            codeWrap.classList.toggle('hidden', !hasTemplate);
        ...
        if (error instanceof DOMException && error.name === 'AbortError')
            return;
        if (sectionTitle)
            sectionTitle.textContent = 'Configuration Guide';
        steps.innerHTML = '';
        config.textContent = '';
        if (codeWrap)
...
function compactGuide(provider, fallbackTitle, fallbackSteps) {
    const guides = {
        standalone: {
            title: 'Standalone',
            steps: [
