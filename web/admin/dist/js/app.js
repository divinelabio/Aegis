...
import { applyVisualExpression, addSecurityRuleCondition, closeSecurityRuleEditor, initSecurityAnalytics, saveSecurityRule, chooseSecurityRuleType, updateRulePreview, updateRuleEditorState, validateSecurityRule, chooseSecurityRuleField, closeSecurityRuleFieldMenus, insertRuleField, removeSecurityRuleCondition, openSecurityRuleFieldMenu, toggleSecurityRuleFieldMenu, updateSecurityRuleConditionControl, setSecurityRuleRawMode, useExpressionBuilder, useExpressionEditor } from './security.js';
...
function invokeShellAction(action) {
    ...
    if (action === 'remove-security-rule-condition') {
        ...
        if (source instanceof HTMLElement)
            removeSecurityRuleCondition(source.dataset.conditionIndex || '');
        return;
    }
    ...
    if (action === 'choose-security-rule-type') {
        ...
        if (source instanceof HTMLElement)
            chooseSecurityRuleType(source.dataset.ruleType || 'custom');
        return;
    }
    ...
    if (action === 'insert-rule-field') {
        ...
        if (source instanceof HTMLElement)
            insertRuleField(source.dataset.field || '');
        return;
    }
    if (action === 'toggle-security-rule-field-menu') {
        const activeEvent = globalThis.event;
        if (activeEvent?.target instanceof Element)
            toggleSecurityRuleFieldMenu(activeEvent.target);
        return;
    }
    if (action === 'choose-security-rule-field') {
        ...
        const source = activeEvent?.target instanceof Element ? activeEvent.target.closest('[data-field]') : null;
        if (source instanceof HTMLElement)
            chooseSecurityRuleField(source);
        return;
    }
    if (action === 'save-security-rule') {
        ...
        if (source instanceof HTMLElement)
            void saveSecurityRule(source.dataset.ruleId || undefined, source.dataset.saveMode === 'draft');
        return;
    }
...
function bindSidebarQuickSearch() {
    const searchInput = AdminDOM.getById('sidebar-quick-search');
    if (!searchInput)
        return;
    ...
    const handleSearch = () => {
        ...
        if (!navMenu)
            return;
        const domainGroups = navMenu.querySelectorAll('.nav-domain-group');
        const topLinks = navMenu.querySelectorAll('.nav-menu > .nav-link');
        const categoryLabels = navMenu.querySelectorAll('.nav-category-label');
        if (!rawQuery) {
            domainGroups.forEach(group => {
                if (group.classList.contains('hidden'))
                    return;
                group.style.display = '';
                const childLinks = group.querySelectorAll('.nav-sub-link');
        ...
        domainGroups.forEach(group => {
            ...
            const subLinks = group.querySelectorAll('.nav-sub-link');
    ...
    const clearBtn = AdminDOM.getById('sidebar-search-clear');
...
function bindShellInteractions() {
    ...
    appRoot.addEventListener('click', event => {
        ...
        if (actionElement instanceof HTMLElement) {
            ...
            if (action === 'toggle-theme' && actionElement instanceof HTMLInputElement)
                return;
            event.preventDefault();
            if (action)
                invokeShellAction(action);
        }
    ...
    appRoot.addEventListener('click', event => {
        if (!(event.target instanceof Element))
            return;
        if (event.target.closest('.top-account'))
            return;
        closeAccountMenu();
    });
    ...
    window.addEventListener('popstate', () => {
        ...
        if (popPath.startsWith('/admin/modules/')) {
            ...
            if (sub === 'captcha')
                popTarget = 'module_captcha';
            else if (sub === 'challenge')
                popTarget = 'module_challenge';
            else if (sub === 'reputation')
                popTarget = 'module_reputation';
            else
                popTarget = 'modules';
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!target) {
        if (path.startsWith('/admin/modules/')) {
            ...
            if (sub === 'captcha')
                target = 'module_captcha';
            else if (sub === 'challenge')
                target = 'module_challenge';
            else if (sub === 'reputation')
                target = 'module_reputation';
            else if (sub === 'error-pages' || sub === 'error_pages' || sub === 'appearance')
                target = 'module_error_pages';
            else
                target = 'modules';
        }
        else {
    ...
    api.checkSession()
        ...
        if (user) {
            ...
            if (roleEl instanceof HTMLElement)
                roleEl.textContent = 'Active User';
        }
    })
        .catch(error => {
...
function logout() {
    SectionUI.openConfirmModal('Confirm Logout', 'Are you sure you want to log out of your session?', 'Logout', 'var(--primary)', () => {
...
function toggleThemeFromMenu() {
    const input = AdminDOM.getById('admin-theme-switch');
