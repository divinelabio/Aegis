...
function ruleEditorErrorMessage(error, fallback) {
    return error instanceof Error ? error.message : fallback;
}
function getFieldValue(id) {
    const field = AdminDOM.getById(id);
    return field?.value ?? '';
}
let ruleEditorBindingsInitialized = false;
function ensureRuleEditorBindings() {
    if (ruleEditorBindingsInitialized)
        return;
    ruleEditorBindingsInitialized = true;
    AdminEvents.delegateEvent(document, 'click', '[data-rule-editor-action]', (_event, target) => {
        const action = target.dataset.ruleEditorAction;
        const tab = target.dataset.ruleTab;
        ...
        switch (action) {
            ...
            case 'select-rule':
                if (index >= 0)
                    RuleEditor.selectRule(index);
                break;
            case 'duplicate-rule':
                if (index >= 0)
                    RuleEditor.duplicateRule(index);
                break;
            case 'delete-rule':
                if (index >= 0)
            ...
            case 'remove-pattern':
                if (index >= 0)
                    RuleEditor.removePattern(index);
                break;
            case 'run-test':
    ...
    AdminEvents.delegateEvent(document, 'input', '#rule-search', (_event, target) => {
        RuleEditor.filterRules(target.value);
    });
    AdminEvents.delegateEvent(document, 'change', '[data-rule-field]', (_event, target) => {
        const field = target.dataset.ruleField;
        if (!field)
            return;
        RuleEditor.updateField(field, target.value);
    });
    AdminEvents.delegateEvent(document, 'input', '#rule-score-slider', (_event, target) => {
        const scoreInput = AdminDOM.getInput('rule-score');
        if (scoreInput)
            scoreInput.value = target.value;
        RuleEditor.updateField('score', Number.parseInt(target.value, 10));
    });
    AdminEvents.delegateEvent(document, 'change', '#rule-score', (_event, target) => {
        const scoreSlider = AdminDOM.getInput('rule-score-slider');
        if (scoreSlider)
            scoreSlider.value = target.value;
        RuleEditor.updateField('score', Number.parseInt(target.value, 10));
    });
    AdminEvents.delegateEvent(document, 'change', '[data-pattern-index]', (_event, target) => {
        const index = Number.parseInt(target.dataset.patternIndex || '', 10);
        if (Number.isNaN(index))
            return;
        RuleEditor.updatePattern(index, target.value);
    });
    AdminEvents.delegateEvent(document, 'focusout', '[data-pattern-validate]', (_event, target) => {
        const index = Number.parseInt(target.dataset.patternValidate || '', 10);
        if (Number.isNaN(index))
            return;
        void RuleEditor.validatePatternInput(target, index);
    });
    AdminEvents.delegateEvent(document, 'change', '#common-patterns', (_event, target) => {
        RuleEditor.insertCommonPattern(target.value);
        target.value = '';
    });
    AdminEvents.delegateEvent(document, 'input', '#source-code', (_event, target) => {
        RuleEditor.sourceContent = target.value;
        RuleEditor.markUnsaved();
    });
    AdminEvents.delegateEvent(document, 'dragstart', '.pattern-item[draggable="true"]', (event, target) => {
        const index = Number.parseInt(target.dataset.patternIndex || '', 10);
        if (Number.isNaN(index) || !(event instanceof DragEvent))
            return;
        RuleEditor.handleDragStart(event, index);
    });
    AdminEvents.delegateEvent(document, 'dragover', '.pattern-item[draggable="true"]', (event) => {
        if (!(event instanceof DragEvent))
            return;
        RuleEditor.handleDragOver(event);
    });
    AdminEvents.delegateEvent(document, 'drop', '.pattern-item[draggable="true"]', (event, target) => {
        const index = Number.parseInt(target.dataset.patternIndex || '', 10);
        if (Number.isNaN(index) || !(event instanceof DragEvent))
            return;
...
const RuleEditor = {
    // Current state
    filePath: '', // Current file being edited (e.g. 'aegis/sql-injection.yaml')
    fileData: null, // Parsed YAML data
    selectedRuleIndex: -1,
    unsavedChanges: false,
    // State
    tabs: ['visual', 'source', 'test'],
    activeTab: 'visual',
    dragSrcIndex: -1,
    sourceContent: '',
    ...
    async validatePattern(pattern, testInput = '') {
        try {
    ...
    selectRule(index) {
        ...
        if (rulesList)
            rulesList.innerHTML = this.renderRulesList();
        const editorContent = AdminDOM.getById('rule-editor-content');
        if (editorContent)
            editorContent.innerHTML = this.renderRuleDetail();
    },
    ...
    async validatePatternInput(input, index) {
        const statusEl = AdminDOM.getById(`pattern-status-${index}`);
        const result = await this.validatePattern(input.value);
        if (result.valid) {
            input.style.borderColor = 'var(--success)';
            if (statusEl)
                statusEl.innerHTML = '<span class="text-success">✓ Valid regex</span>';
        }
        else {
            input.style.borderColor = 'var(--danger)';
            if (statusEl)
                statusEl.innerHTML = `<span class="text-danger">✗ ${this.escapeHtml(result.error)}</span>`;
        }
    },
    // Add new pattern
    addPattern() {
    ...
    handleDrop(e, dropIndex) {
        e.stopPropagation();
        const target = e.target;
        const patternRow = target?.closest('.pattern-item');
        if (patternRow)
            patternRow.style.opacity = '1';
    ...
    // Delete rule
    deleteRule(index) {
        if (!this.fileData?.rules?.[index])
            return;
        const ruleId = this.fileData.rules[index].id;
        SectionUI.openConfirmModal('Delete Rule', `Are you sure you want to delete rule <strong>"${this.escapeHtml(ruleId)}"</strong>?`, 'Delete', 'var(--danger)', () => { RuleEditor.confirmDeleteRule(index); });
    },
    ...
    duplicateRule(index) {
        ...
        if (!original || !this.fileData?.rules)
            return;
        const duplicate = {
            id: original.id + '-COPY',
    ...
    insertCommonPattern(pattern) {
        if (!pattern || this.selectedRuleIndex < 0)
            return;
        if (!this.fileData?.rules?.[this.selectedRuleIndex])
            return;
...
export function initRuleEditor(filePath) {
