...
let currentSettings = {};
let storageRuntimeStatus = null;
let activeSettingsTab = 'general';
let settingsBindingsInitialized = false;
let upstreamRuntimeAvailability = 'available';
let currentLicense = null;
let licenseLoadError = '';
let activeErrorPageTemplateID = 'aegis';
let errorPageTemplateDrafts = null;
let errorPagesEnabledDraft = null;
let errorPagesThemeDraft = null;
let activeErrorPagePreviewStatus = '403';
...
export async function loadSettingsPage() {
    ...
    try {
        ...
        if (!configResponse.ok)
            throw new Error(`Settings request failed with status ${configResponse.status}`);
        currentSettings = (await configResponse.json());
        clearErrorPageDraftState();
        renderSettingsPage(container);
        if (activeSettingsTab === 'license')
            void loadLicense();
        void Promise.all([loadUpstreamRuntimeSettings(), loadStorageRuntimeStatus()]).then(([upstreamSettings, runtimeStatus]) => {
            ...
            if (upstreamSettings.data)
                currentSettings.upstream = upstreamSettings.data;
            if (activeSettingsTab !== 'database' && activeSettingsTab !== 'upstream')
                return;
...
export function selectSettingsTab(tabId) {
...
function renderSettingsPage(container) {
    ...
    <div class="settings-operator-stack">
      ${SectionUI.renderOperatorSection(`<span id="settings-active-title">${activeTab.name}</span>`, `<div class="settings-content content-fade" id="settings-content" aria-live="polite">
...
function ensureSettingsBindings() {
    if (settingsBindingsInitialized)
        return;
    settingsBindingsInitialized = true;
    AdminEvents.delegateEvent(document, 'click', '[data-settings-tab]', (_event, target) => {
        const tabId = target.dataset.settingsTab;
        if (!tabId)
            return;
        switchSettingsTab(tabId);
    });
    AdminEvents.delegateEvent(document, 'click', '[data-settings-action]', (_event, target) => {
    ...
    AdminEvents.delegateEvent(document, 'click', '[data-import-target]', (_event, target) => {
        const inputId = target.dataset.importTarget;
        if (!inputId)
            return;
        AdminDOM.getInput(inputId)?.click();
    });
    AdminEvents.delegateEvent(document, 'change', 'input[type="file"][data-settings-action="import-config"]', (_event, target) => {
        importConfig(target);
    });
    AdminEvents.delegateEvent(document, 'click', 'button[data-error-page-template]:not([data-settings-action])', (_event, target) => {
        selectErrorPageTemplate(target.dataset.errorPageTemplate || '');
    });
    AdminEvents.delegateEvent(document, 'change', 'select[data-error-page-template]', (_event, target) => {
        selectErrorPageTemplate(target.value);
    });
    AdminEvents.delegateEvent(document, 'click', 'button[data-error-page-preview-status]', (_event, target) => {
        selectErrorPagePreviewStatus(target.dataset.errorPagePreviewStatus || '');
    });
    AdminEvents.delegateEvent(document, 'input', 'textarea[data-error-page-template-content]', () => {
        updateErrorPageTemplatePreview();
    });
    AdminEvents.delegateEvent(document, 'click', 'button[data-error-page-theme]', (_event, target) => {
...
function switchSettingsTab(tabId) {
    ...
    AdminDOM.queryAll('#settings-container [data-settings-tab]').forEach(element => {
    ...
    if (activeTab) {
        ...
        if (desc)
            desc.textContent = activeTab.desc;
    }
    const content = AdminDOM.getById('settings-content');
    if (!content)
        return;
...
function renderErrorPagesSettings() {
...
const BUILT_IN_ERROR_PAGE_TEMPLATES = [
...
const ERROR_PAGE_PREVIEW_RESPONSES = [
    {
        status: '403',
        label: '403 Denied',
        title: 'Access denied',
...
function errorPagePreviewTitle(template, theme = getSelectedErrorPageTheme()) {
...
function normalizeErrorPageTheme(value) {
    return value === 'light' || value === 'dark' || value === 'auto' ? value : 'auto';
}
function errorPageThemeLabel(theme) {
    return theme === 'auto' ? 'Auto' : theme === 'light' ? 'Light' : 'Dark';
}
function getErrorPageTemplateOptions(errorPages) {
...
function renderErrorPageTemplateCreator() {
    return `
...
function renderCustomErrorPageTemplateEditor(template) {
...
function getErrorPageTemplatePreview(template, status, theme = getSelectedErrorPageTheme()) {
    ...
    return errorPageDocumentForStatus(errorPagePresetPages(template.id, theme), status);
}
function errorPageDocumentForStatus(documents, status) {
    switch (status) {
        case '403':
...
function getActiveErrorPageTemplate() {
...
function updateErrorPageTemplatePreview() {
    const preview = AdminDOM.getById('error-pages-preview');
    if (!preview)
        return;
...
function selectErrorPagePreviewStatus(statusValue) {
    if (statusValue !== '403' && statusValue !== '404' && statusValue !== '503')
        return;
    const status = statusValue;
    if (status === activeErrorPagePreviewStatus)
        return;
    activeErrorPagePreviewStatus = status;
    AdminDOM.queryAll('[data-error-page-preview-status]').forEach(button => {
...
function openErrorPageTemplateCreator() {
...
function closeErrorPageTemplateCreator() {
    const creator = AdminDOM.getById('error-page-template-creator');
    if (creator)
        creator.hidden = true;
}
function captureErrorPageTemplateDrafts() {
...
function customErrorPageTemplateStarter() {
    return `<!doctype html>
<html lang="en">
...
function createErrorPageTemplate() {
...
function deleteErrorPageTemplate(templateID) {
    const templates = captureErrorPageTemplateDrafts();
...
function createErrorPageDocuments(render, theme) {
    return {
...
function createCustomErrorPageDocuments(html, theme) {
    return {
...
function renderErrorPageResponseIcon(status, className) {
    switch (status) {
        case '403':
...
function renderErrorPageThemeVariables(theme, dark, light) {
    if (theme === 'light')
        return `:root{color-scheme:light;${light}}`;
    if (theme === 'dark')
        return `:root{color-scheme:dark;${dark}}`;
    return `:root{color-scheme:light dark;${dark}}@media (prefers-color-scheme: light){:root{color-scheme:light;${light}}}`;
}
function renderAegisBlockingErrorPage(status, title, message, theme) {
    return `<!doctype html>
...
function renderMinimalErrorPage(status, title, message, theme) {
    return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>${status} - ${title}</title><style>
...
function renderServiceStatusErrorPage(status, title, message, theme) {
    return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>${status} - ${title}</title><style>
...
</style><style>${renderErrorPageThemeVariables(theme, '--page:#0e1116;--surface:#151a22;--line:#29313c;--text:#f1f4f8;--muted:#a9b3c1;--quiet:#7d8795;--accent:#fb923c', '--page:#f5f6f8;--surface:#fff;--line:#e5e7eb;--text:#181b20;--muted:#69707d;--quiet:#8a919d;--accent:#b45309')}</style></head><body><main class="status-page" aria-labelledby="error-title"><section class="summary"><p class="eyebrow">${getErrorPagePreviewResponse(status).category}</p><h1 id="error-title">${title}</h1><p>${message}</p></section><dl class="status-list"><div class="status-row status-row--state"><dt>Current state</dt><dd>${errorPageServiceState(status)}</dd></div><div class="status-row"><dt>HTTP response</dt><dd>${status}</dd></div><div class="status-row"><dt>Recommended action</dt><dd>${getErrorPagePreviewResponse(status).nextStep}</dd></div></dl><p class="footnote">This status reflects the response to your current request.</p></main></body></html>`;
}
function captureErrorPagesEnabledDraft() {
    const enabled = AdminDOM.getInput('set-error-pages-enabled');
    if (enabled)
        errorPagesEnabledDraft = enabled.checked;
}
function rerenderErrorPagesWorkspace() {
    const content = AdminDOM.getById('settings-content');
    if (!content || activeSettingsTab !== 'appearance')
        return;
    content.innerHTML = renderSettingsTabContent('appearance');
}
function clearErrorPageDraftState() {
...
async function saveErrorPages() {
    ...
    const pages = selected.source === 'custom'
        ...
        : errorPagePresetPages(selected.id, theme);
    await saveSettings({
...
function renderBackupsSettings() {
    return `
        <div class="settings-panel-content">
            <div class="settings-section">
...
function settingsActionIcon(type) {
    const icons = {
...
function isValidPrometheusPath(path) {
...
async function saveAdvancedSettings() {
    ...
    if (token)
        updates['telemetry.prometheus_token'] = token;
    if (getCheckboxValue('set-prometheus-token-clear'))
        updates['telemetry.prometheus_token'] = '';
...
function supportedPostgreSQLSSLModes() {
    return ['disable', 'allow', 'prefer', 'require', 'verify-ca', 'verify-full'];
}
function renderPostgreSQLSSLOptions(selectedMode) {
    return supportedPostgreSQLSSLModes().map(mode => (`<option value="${mode}" ${mode === selectedMode ? 'selected' : ''}>${mode}</option>`)).join('');
}
function supportedAnalyticsModes() {
    return ['required', 'optional', 'disabled'];
}
function currentClickHouseAnalyticsMode() {
    const mode = currentSettings.storage?.analytics?.mode;
    return mode && supportedAnalyticsModes().includes(mode) ? mode : 'disabled';
}
function renderStorageRuntimeStatus(status, unavailableMessage) {
    if (!status)
        return unavailableMessage;
    if (status.restart_required)
        return 'A pending configuration is saved. Restart Aegis to apply it.';
    if (status.available)
        return 'Active connection is healthy.';
    if (status.mode === 'disabled')
        return 'Analytics is disabled in the active settings.';
    if (status.configured && status.active)
        return 'The active connection is currently unavailable.';
    if (status.environment_locked)
        return 'This connection is managed outside Aegis.';
    return 'No active connection is available.';
}
async function testPostgreSQLControlPlaneConnection() {
    ...
    const request = { host, port, database, username, password, ssl_mode: sslMode };
    settingsShowToast('Testing PostgreSQL connection…', 'info');
    try {
        const result = await api.requestResult('storage/control/postgresql/preflight', 'POST', request);
...
async function savePostgreSQLControlPlanePending() {
    const host = getInputValue('set-postgresql-host').trim();
    const port = getNumberValue('set-postgresql-port');
    const database = getInputValue('set-postgresql-database').trim();
    const username = getInputValue('set-postgresql-username').trim();
    ...
    const result = await api.requestResult('storage/control/postgresql/pending', 'POST', request);
...
function clickHouseAnalyticsInput() {
    return {
...
function validateClickHouseAnalyticsInput(input, requirePassword) {
...
async function testClickHouseAnalyticsConnection() {
    ...
    if (!validateClickHouseAnalyticsInput(input, true))
        return;
    ...
    try {
        const result = await api.requestResult('storage/analytics/clickhouse/preflight', 'POST', request);
        if (result.error || result.data?.status !== 'ok' || !result.data.connection) {
...
async function saveClickHouseAnalyticsPending() {
    const input = clickHouseAnalyticsInput();
    if (!validateClickHouseAnalyticsInput(input, false))
        return;
    const request = {
        mode: input.mode,
        host: input.host,
    ...
    const result = await api.requestResult('storage/analytics/clickhouse/pending', 'POST', request);
    if (result.error || result.data?.status !== 'pending' || !result.data.restart_required) {
...
async function saveUpstreamSettings() {
    ...
    const result = await api.requestResult('upstreams/settings', 'POST', settings);
...
function upstreamRuntimeAvailabilityMessage() {
...
async function saveSettings(updates, successMsg) {
    const result = await api.requestResult('config', 'POST', updates);
    if (result.error) {
...
function factoryReset() {
    SectionUI.openConfirmModal('Reset configuration file', 'Replace config.yaml with Aegis defaults?<br><br><strong>This does not delete rules, certificates, users, sessions, or database data.</strong><br>A restart is required before all runtime components use the new configuration.', 'RESET CONFIGURATION', 'var(--danger)', async () => {
        SectionUI.closeModal();
        try {
            const result = await api.requestResult('config/reset', 'POST');
            if (result.error) {
...
async function loadLicense() {
    ...
    licenseLoadError = '';
    renderLicensePanel();
    const snapshot = await api.get('license');
    if (!snapshot) {
...
function renderLicensePanel() {
    const content = AdminDOM.getById('settings-content');
    if (content && activeSettingsTab === 'license')
        content.innerHTML = renderLicenseSettings();
}
function renderLicenseSettings() {
    ...
    if (!currentLicense)
        return SectionUI.renderLoading('Loading licence status...');
...
function renderLicenseOverview(license, statusTone, isCommunityBuild) {
    const facts = [
...
function setLicenseActionsDisabled(isDisabled) {
    AdminDOM.queryAll('[data-license-action]').forEach(button => {
        button.disabled = isDisabled;
    });
    const panel = AdminDOM.query('.settings-panel-content');
    if (panel)
        panel.setAttribute('aria-busy', isDisabled ? 'true' : 'false');
}
...
async function refreshLicense() {
    setLicenseActionsDisabled(true);
    try {
        const snapshot = await api.post('license/refresh');
        if (!snapshot)
            return;
        currentLicense = snapshot;
        renderLicensePanel();
...
function deactivateLicense() {
    SectionUI.openConfirmModal('Deactivate licence', 'Deactivate this installation and release its activation slot?', 'Deactivate', 'var(--danger)', async () => {
        SectionUI.closeModal();
        setLicenseActionsDisabled(true);
        try {
            const snapshot = await api.post('license/deactivate');
            if (!snapshot) {
                await loadLicense();
                return;
            }
