...
function bindAlertInteractions() {
    if (bindAlertInteractions.initialized)
        return;
    bindAlertInteractions.initialized = true;
    AdminEvents.delegateEvent(document, 'click', '[data-action="alerts-mark-all-read"]', event => {
        event.preventDefault();
        event.stopPropagation();
        void markAllAlertsRead();
    });
    AdminEvents.delegateEvent(document, 'click', '[data-action="alerts-open-detail"]', (event, target) => {
        event.preventDefault();
        if (event.target instanceof Element && event.target.closest('.alert-action-btn'))
            return;
        const alertId = target.dataset.alertId;
        if (alertId)
            openAlertDetail(alertId);
    });
    AdminEvents.delegateEvent(document, 'click', '[data-action="alerts-filter"]', (event, target) => {
        event.preventDefault();
        const filter = target.dataset.alertFilter;
        if (filter) {
            alertsState.filter = filter;
            renderAlertDropdown();
        }
    });
    AdminEvents.delegateEvent(document, 'click', '[data-action="alerts-open-route"]', (event, target) => {
        event.preventDefault();
        event.stopPropagation();
    ...
    AdminEvents.delegateEvent(document, 'click', '[data-action="alerts-mark-one-read"]', (event, target) => {
        event.preventDefault();
        event.stopPropagation();
        const alertId = target.dataset.alertId;
        if (alertId)
...
async function persistReadState(body) {
    ...
    if (!response.ok)
        throw new Error(`Alert acknowledgement failed: ${response.status}`);
}
...
function renderAlertFilter(filter, label, count) {
...
function getFilteredAlerts() {
    if (alertsState.filter === 'all')
        return alertsState.alertsCache;
...
function countUnresolved(alerts) {
    return alerts.filter(alert => alert.unresolved !== false).length;
}
function normalizeLevel(level) {
    const normalized = String(level || 'info').toLowerCase();
    if (['critical', 'error', 'danger', 'severe', 'high'].includes(normalized))
        return 'critical';
    if (['warning', 'warn', 'medium', 'degraded'].includes(normalized))
        return 'warning';
    return 'info';
}
...
function inferSource(alert) {
    const text = `${alert.source || ''} ${alert.title || ''} ${alert.message || ''}`.toLowerCase();
    if (text.includes('bot'))
        return 'Bot Protection';
    if (text.includes('waf') || text.includes('rule'))
        return 'WAF';
    if (text.includes('rate') || text.includes('traffic'))
        return 'Traffic Control';
    if (text.includes('http'))
        return 'Application Security';
    if (text.includes('api'))
        return 'API Security';
    if (text.includes('access') || text.includes('auth'))
        return 'Edge Access';
    return 'System';
}
function inferRoute(alert) {
    const source = inferSource(alert);
    if (source === 'WAF')
        return 'waf_config';
    if (source === 'Bot Protection')
        return 'antibots_config';
    if (source === 'Traffic Control')
        return 'traffic_config';
    if (source === 'Edge Access' || source === 'Access')
        return 'access_config';
    if (source === 'API Security')
        return 'apisecurity_config';
    if (source === 'Application Security' || source === 'App Security')
