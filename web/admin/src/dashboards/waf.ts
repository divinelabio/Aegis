...
type WAFWindow = string;

type WAFDashboardFilterKey =
  | 'section'
  | 'action'
  | 'category'
  | 'rule_id'
  ...
  | 'asn'
  | 'request_id';
type WAFDashboardFilter = WAFDashboardFilterKey | 'score_band';
type WAFDashboardFilters = Partial<Record<WAFDashboardFilterKey, string>>;

interface WAFBreakdownFilter {
  key: WAFDashboardFilter;
  value: (item: WAFBreakdownItem) => string;
}

interface WAFSummary {
  total_requests?: number;
  blocked_requests?: number;
  detected_requests?: number;
...
interface WAFEventRow {
  ...
  policy_id?: string;
  rule_source?: string;
  response_inspected?: boolean;
  response_inspection_coverage?: string;
...
interface WAFProtectionSummary {
...
interface WAFInspectionCoverage {
  request_inspected_count?: number;
  response_inspected_count?: number;
  partial_response_count?: number;
  truncated_request_count?: number;
  truncated_response_count?: number;
  truncated_request_rate?: number;
  truncated_response_rate?: number;
  dropped_events?: number;
  freshness_status?: string;
  message?: string;
}

interface WAFPolicyHealth {
  status?: string;
  policy_name?: string;
  rule_source?: string;
...
interface WAFRuleEffectiveness {
  top_blocking_rules?: WAFBreakdownItem[];
  top_detect_only_rules?: WAFBreakdownItem[];
  noisy_rules?: WAFBreakdownItem[];
  matched_fields?: WAFBreakdownItem[];
}

interface SecurityAnalyticsBreakdown {
  key: string;
  label: string;
  items?: WAFBreakdownItem[];
}

interface SecuritySectionAnalytics {
  section: string;
  label: string;
  breakdowns?: SecurityAnalyticsBreakdown[];
}

interface WAFAttackIntelligence {
  categories?: WAFBreakdownItem[];
  severities?: WAFBreakdownItem[];
  anomaly_score_bands?: WAFBreakdownItem[];
  trend?: WAFTimeseriesPoint[];
}

interface WAFTuningRecommendation {
  title?: string;
  detail?: string;
  rule_id?: string;
  count?: number;
...
interface WAFDashboardPayload {
  summary?: WAFSummary;
  timeseries?: WAFTimeseriesPoint[];
  breakdowns?: Partial<Record<string, WAFBreakdownItem[]>>;
  events?: { events?: WAFEventRow[] };
  health?: WAFHealth;
  protection_summary?: WAFProtectionSummary;
  attack_intelligence?: WAFAttackIntelligence;
  rule_effectiveness?: WAFRuleEffectiveness;
  section_analytics?: SecuritySectionAnalytics[];
  warnings?: string[];
}

interface WAFDashboardResponse {
  success?: boolean;
  dashboard?: WAFDashboardPayload;
}

interface WAFEventResponse {
  success?: boolean;
  event?: WAFEventRow;
}

let selectedWindow: WAFWindow = '24h';
let eventsBound = false;
let requestVersion = 0;
let wafDashboardFilters: WAFDashboardFilters = {};
let securitySectionAnalytics: SecuritySectionAnalytics[] = [];
let securityDashboardEvents: WAFEventRow[] = [];
let selectedSecurityAnalyticsSection = '';
let wafEventsPage = 1;
let wafEventsPageSize = 20;
let selectedWAFDrawerTab: 'overview' | 'raw' = 'overview';
...
const formatPercent = (value: number | undefined): string => `${Math.max(0, Number(value) || 0).toFixed(1)}%`;
...
const shortTime = (timestamp: number | undefined): string => {
  if (!timestamp) return '';
  const date = new Date(timestamp);
  if (Number.isNaN(date.getTime())) return '';
...
const timelineTick = (timestamp: number | undefined): string => {
  if (!timestamp) return '';
  const date = new Date(timestamp);
  if (Number.isNaN(date.getTime())) return '';
...
function wafTimeRangeLabel(window = selectedWindow): string {
...
function renderWAFHeaderTime(): void {
...
let selectedFilterBuilderField: string = 'section';

function filterDisplayLabel(key: WAFDashboardFilterKey, value: string): string {
  const labels: Record<WAFDashboardFilterKey, string> = {
    section: 'Section',
    action: 'Decision',
    category: 'Category',
    rule_id: 'Rule ID',
...
function renderFilterOperators(field: string): string {
...
function renderFilterValueControl(field: string): string {
  switch (field) {
    case 'section':
      return `
...
function renderWAFHeaderFilters(): void {
  const target = byId('waf-dashboard-header-filters');
  if (!target) return;
  const entries: Array<{ label: string; key?: WAFDashboardFilterKey; score?: boolean }> = [];
  Object.entries(wafDashboardFilters).forEach(([rawKey, rawValue]) => {
    const key = rawKey as WAFDashboardFilterKey;
    const value = String(rawValue || '').trim();
    if (!value || key === 'min_score' || key === 'max_score') return;
...
function toggleWAFFilterMenu(): void {
  isFilterBuilderOpen = !isFilterBuilderOpen;
  hideWAFTimePanel();
  renderWAFHeaderFilters();
  if (isFilterBuilderOpen) {
    const input = byId<HTMLInputElement | HTMLSelectElement>('waf-filter-value');
    if (input) input.focus();
  }
}

function hideWAFFilterBuilder(): void {
  if (!isFilterBuilderOpen) return;
  isFilterBuilderOpen = false;
  renderWAFHeaderFilters();
}

function applyWAFScoreFilter(rawValue: string, operator: string): void {
  const range = rawValue.match(/^(\d+)(?:\s*[–-]\s*(\d+)|\+)$/);
  if (range) {
    wafDashboardFilters.min_score = range[1];
    if (range[2]) wafDashboardFilters.max_score = range[2];
    else delete wafDashboardFilters.max_score;
    return;
  }
...
function applyWAFFilterBuilder(): void {
  const fieldSelect = byId<HTMLSelectElement>('waf-filter-field');
  const opSelect = byId<HTMLSelectElement>('waf-filter-operator');
  const valInput = byId<HTMLInputElement | HTMLSelectElement>('waf-filter-value');
  if (!fieldSelect || !valInput) return;

  const field = fieldSelect.value as WAFDashboardFilterKey | 'score';
  ...
  if (field === 'score') {
    ...
    wafDashboardFilters[field as WAFDashboardFilterKey] = normalizeDashboardFilterValue(field as WAFDashboardFilterKey, rawValue, operator);
  }
...
function normalizeDashboardFilterValue(key: WAFDashboardFilterKey, value: string, operator = 'equals'): string {
  const firstValue = operator === 'in' ? value.split(',').map(item => item.trim()).filter(Boolean)[0] || value : value;
  if (key === 'action' && firstValue === 'log') return 'detect';
  if (key === 'country' || key === 'method') return firstValue.trim().toUpperCase();
  if (key === 'asn') return firstValue.trim().toUpperCase().startsWith('AS') ? firstValue.trim().toUpperCase() : `AS${firstValue.trim().toUpperCase()}`;
  if (key === 'status_class') return firstValue.trim().toLowerCase();
  return firstValue.trim();
}

function applyWAFDashboardFilter(filter: WAFDashboardFilter, rawValue: string): void {
  const value = rawValue.trim();
  if (!value) return;
  if (filter === 'section' && !isSecuritySectionEntitled(value)) return;
  if (filter === 'score_band') {
    ...
    if (isActive) {
      ...
      if (maximum) wafDashboardFilters.max_score = maximum;
      else delete wafDashboardFilters.max_score;
    }
  } else {
    const normalized = normalizeDashboardFilterValue(filter, value);
    if (wafDashboardFilters[filter] === normalized) delete wafDashboardFilters[filter];
    else wafDashboardFilters = { ...wafDashboardFilters, [filter]: normalized };
  }
  wafEventsPage = 1;
  renderWAFHeaderFilters();
  void loadWAFDashboard();
...
function removeWAFDashboardFilter(key?: WAFDashboardFilterKey, group?: string): void {
  if (group === 'score') {
    delete wafDashboardFilters.min_score;
    delete wafDashboardFilters.max_score;
  } else if (key) {
...
function clearWAFDashboardFilters(): void {
  if (!Object.keys(wafDashboardFilters).length) return;
  wafDashboardFilters = {};
  wafEventsPage = 1;
  renderWAFHeaderFilters();
  void loadWAFDashboard();
...
function applyWAFWindow(window: string): void {
...
function applyWAFLastRange(): void {
  const amount = Math.max(1, Math.min(999, Math.floor(Number(byId<HTMLInputElement>('waf-dashboard-relative-amount')?.value) || 1)));
  const unit = byId<HTMLSelectElement>('waf-dashboard-relative-unit')?.value || 'hours';
...
function createEmptyWAFDashboard(message = ''): WAFDashboardPayload {
  return {
    summary: {
      total_requests: 0,
      blocked_requests: 0,
...
function eventMatchesWAFDashboardFilters(event: WAFEventRow): boolean {
  if (wafDashboardFilters.section && event.section !== wafDashboardFilters.section) return false;
  const action = normalizeDashboardFilterValue('action', String(event.action || ''));
  if (wafDashboardFilters.action && action !== wafDashboardFilters.action) return false;
  if (wafDashboardFilters.category) {
    ...
    if (!matchesCat && !matchesRule) return false;
  }
  if (wafDashboardFilters.rule_id) {
    ...
    if (!matchesId && !matchesIds && !matchesName) return false;
  }
  if (wafDashboardFilters.severity && !event.severities?.some(item => item.toLowerCase() === wafDashboardFilters.severity?.toLowerCase())) return false;
  if (wafDashboardFilters.ip && event.client_ip !== wafDashboardFilters.ip) return false;
  if (wafDashboardFilters.country && event.country?.toUpperCase() !== wafDashboardFilters.country.toUpperCase()) return false;
  ...
  if (wafDashboardFilters.matched_field) {
    ...
    if (!event.matched_fields?.some(item => item.toLowerCase().includes(mf) || mf.includes(item.toLowerCase()))) return false;
  }
  if (wafDashboardFilters.user_agent) {
    ...
    if (!eua.includes(ua) && !ua.includes((event.user_agent_family || '').toLowerCase())) return false;
  }
  if (wafDashboardFilters.method && (event.method || '').toUpperCase() !== wafDashboardFilters.method.toUpperCase()) return false;
  if (wafDashboardFilters.status && String(event.status_code || '') !== wafDashboardFilters.status) return false;
  if (wafDashboardFilters.policy) {
    ...
    if (!epol.includes(pol) && !pol.includes(epol)) return false;
  }
  if (wafDashboardFilters.rule_name) {
    ...
    if (!ern.includes(rn) && !matchesMsg) return false;
  }
  if (wafDashboardFilters.user_agent_family) {
    ...
    if (!euaf.includes(uaf)) return false;
  }
  if (wafDashboardFilters.status_class) {
    ...
    if (cClass !== sc) return false;
  }
  if (wafDashboardFilters.ja3 && event.ja3 && !event.ja3.toLowerCase().includes(wafDashboardFilters.ja3.toLowerCase())) return false;
  if (wafDashboardFilters.ja4 && event.ja4 && !event.ja4.toLowerCase().includes(wafDashboardFilters.ja4.toLowerCase())) return false;
  if (wafDashboardFilters.asn) {
    const fasn = wafDashboardFilters.asn.toUpperCase();
    const easn = (event.asn || '').toUpperCase();
    if (!easn.includes(fasn) && !fasn.includes(easn)) return false;
  }
  if (wafDashboardFilters.request_id && event.request_id && !event.request_id.toLowerCase().includes(wafDashboardFilters.request_id.toLowerCase())) return false;
  const score = Number(event.anomaly_score) || 0;
  if (wafDashboardFilters.min_score && score < Number(wafDashboardFilters.min_score)) return false;
  if (wafDashboardFilters.max_score && score > Number(wafDashboardFilters.max_score)) return false;
  return true;
}
...
function securitySectionLabel(value?: string): string {
  switch (value) {
...
function isSecuritySectionEntitled(section?: string): boolean {
...
function dashboardFilterAttributes(filter: WAFDashboardFilter | undefined, value: string, label: string): string {
...
function securitySectionIcon(section: string): string {
...
function renderBreakdown(items: WAFBreakdownItem[] | undefined, emptyText: string, filter?: WAFBreakdownFilter, limit = 5): string {
...
function renderSecurityAnalyticsCard(title: string, items: WAFBreakdownItem[] | undefined, emptyText: string, filter?: WAFBreakdownFilter): string {
...
function renderSecurityStatRows(items: WAFBreakdownItem[], filter?: WAFBreakdownFilter, emptyText = 'No data'): string {
  if (!items.length) {
...
function renderSecuritySectionAnalytics(items: SecuritySectionAnalytics[] | undefined): string {
  const sources = (items || []).filter(source => isSecuritySectionEntitled(source.section));
  if (!sources.length) return '<div class="waf-dashboard-empty">No protection-section analytics recorded.</div>';
        ...
        <div class="security-events-stat-grid">
          ${breakdowns.map(breakdown => `
            <div class="security-statistics-card">
              ${renderSecurityAnalyticsCard(
                breakdown.label,
                breakdown.items,
                `No ${breakdown.label.toLowerCase()} recorded.`,
                securityAnalyticsFilter(breakdown.key)
              )}
            </div>
          `).join('')}
        </div>
...
function syncSecurityStatisticsShowMore(deck: HTMLElement): void {
  const button = deck.querySelector<HTMLElement>('[data-waf-dashboard-action="toggle-statistics-cards"]');
  if (!button) return;
...
function parseStatisticModalItems(value: string): Array<{ key?: string; label?: string; value?: string; count: number }> {
  try {
    const parsed = JSON.parse(decodeURIComponent(value)) as Array<{ key?: unknown; label?: unknown; value?: unknown; count?: unknown }>;
...
function statisticModalRow(item: { key?: string; label?: string; value?: string; count: number }, filterKey: string, total: number, max: number): string {
...
function selectSecuritySectionAnalytics(section: string, focusTab = false): void {
  if (!isSecuritySectionEntitled(section)) return;
  if (!securitySectionAnalytics.some(source => source.section === section)) return;
  selectedSecurityAnalyticsSection = section;
  wafEventsPage = 1;
  const target = byId('waf-dashboard-section-results');
  if (target) target.innerHTML = renderSecuritySectionAnalytics(securitySectionAnalytics);
  renderSelectedSecurityEvents();
  if (focusTab) {
    document.querySelectorAll<HTMLButtonElement>('[data-waf-dashboard-action="select-section-analytics"]').forEach(tab => {
      if (tab.dataset.wafDashboardSection === section) tab.focus();
    });
  }
}

function securityAnalyticsFilter(key: string): WAFBreakdownFilter | undefined {
  const filters: Record<string, WAFBreakdownFilter> = {
    // Actions & Decisions
...
function buildBezier(coords: Array<{ x: number; y: number }>): string {
  if (!coords.length) return '';
  if (coords.length === 1) return `M ${(coords[0].x - 8).toFixed(1)} ${coords[0].y.toFixed(1)} L ${(coords[0].x + 8).toFixed(1)} ${coords[0].y.toFixed(1)}`;
...
function renderTimeline(points: WAFTimeseriesPoint[] | undefined): string {
  let visible = (points || []).slice(-48);
  if (!visible.length) return '<div class="waf-dashboard-chart-empty">No request activity in this window.</div>';
  ...
  const totalFor = (point: WAFTimeseriesPoint): number => Math.max(0, Number(point.counts?.total || point.counts?.requests || 0));
...
function bindTimelineHover(): void {
  const chart = document.querySelector<SVGSVGElement>('#waf-dashboard-timeline .waf-dashboard-timeline-svg');
  const tooltip = byId('waf-dashboard-timeline-tooltip');
  const shell = byId('waf-dashboard-timeline');
  const crosshair = document.querySelector<SVGLineElement>('#waf-timeline-crosshair');
  if (!chart || !tooltip || !shell) return;
  let points: Array<{ x: number; timestamp: number; total: number; blocked: number; detected: number; errors: number; avgLatency: number; p95Latency: number; avgScore: number }> = [];
  try {
...
function renderRecommendations(items: WAFTuningRecommendation[] | undefined): string {
  const visible = (items || []).slice(0, 3);
  if (!visible.length) return '<div class="waf-dashboard-empty">No tuning priorities right now.</div>';
...
function setWAFDrawerTab(tab: 'overview' | 'raw'): void {
...
function drawerCardRow(label: string, value: string, mono = false): string {
  return `
...
function renderWAFEventCommandBar(event: WAFEventRow): string {
  ...
  const btn = (action: string, label: string, icon: string, attrs: Record<string, string>): string => {
...
function renderEventDetail(event: WAFEventRow): void {
  ...
  if (kicker) kicker.textContent = `${sectionName} event`;
  if (methodBadge) methodBadge.textContent = method;
  if (title) title.textContent = ruleTitle;
  if (sectionSpan) sectionSpan.textContent = sectionName;
  if (timeSpan) timeSpan.textContent = formatTime(event.timestamp);
...
function renderSelectedSecurityEvents(): void {
  const body = byId<HTMLTableSectionElement>('waf-dashboard-events');
  if (!body) return;
  const entitledSources = securitySectionAnalytics.filter(source => isSecuritySectionEntitled(source.section));
  const filterKeys = Object.keys(wafDashboardFilters) as WAFDashboardFilterKey[];
  ...
  if (wafEventsPage > totalPages) wafEventsPage = totalPages;
...
function renderDashboard(dashboard: WAFDashboardPayload): void {
  ...
  if (sectionResults) sectionResults.innerHTML = renderSecuritySectionAnalytics(securitySectionAnalytics);
...
function resetWAFDashboardSkeletons(): void {
  ...
  const protectionSections = byId('waf-dashboard-protection-sections');
  if (protectionSections) protectionSections.innerHTML = renderBreakdownSkeleton(6);
  const sectionResults = byId('waf-dashboard-section-results');
  if (sectionResults) sectionResults.innerHTML = renderProtectionCardsSkeleton(4);
  const events = byId('waf-dashboard-events');
  if (events) events.innerHTML = renderTableSkeleton(8, 6);
}

async function loadWAFDashboard(): Promise<void> {
  const version = ++requestVersion;
  const refresh = byId<HTMLButtonElement>('waf-dashboard-refresh');
...
async function fetchWAFDashboard(query: URLSearchParams): Promise<WAFDashboardResponse | null> {
  ...
  try {
    return await response.json() as WAFDashboardResponse;
  } catch {
    return null;
  }
}
...
async function openEvent(id: string): Promise<void> {
  ...
  try {
    const response = await api.get<WAFEventResponse>(`security/dashboard/events/${encodeURIComponent(id)}`);
    if (!isMounted()) return;
    if (!response?.event) throw new Error('Detection details are unavailable.');
...
function bindEvents(): void {
  ...
  document.addEventListener('click', event => {
    ...
    const filterTarget = event.target.closest<HTMLElement>('[data-waf-dashboard-filter-key][data-waf-dashboard-filter-value]');
    if (filterTarget?.dataset.wafDashboardFilterKey && filterTarget.dataset.wafDashboardFilterValue) {
      event.preventDefault();
      applyWAFDashboardFilter(filterTarget.dataset.wafDashboardFilterKey as WAFDashboardFilter, filterTarget.dataset.wafDashboardFilterValue);
      return;
    }
    const toggleFilterMenu = event.target.closest<HTMLElement>('[data-waf-dashboard-action="toggle-filter-menu"]');
    if (toggleFilterMenu) {
      event.preventDefault();
      toggleWAFFilterMenu();
      return;
    }
    const cancelFilterBuilder = event.target.closest<HTMLElement>('[data-waf-dashboard-action="cancel-filter-builder"]');
    if (cancelFilterBuilder) {
      event.preventDefault();
      hideWAFFilterBuilder();
      return;
    }
    const applyFilterBuilderBtn = event.target.closest<HTMLElement>('[data-waf-dashboard-action="apply-filter-builder"]');
    if (applyFilterBuilderBtn) {
      event.preventDefault();
      applyWAFFilterBuilder();
      return;
    }
    const removeFilter = event.target.closest<HTMLElement>('[data-waf-dashboard-action="remove-filter"]');
    if (removeFilter) {
      event.preventDefault();
      removeWAFDashboardFilter(removeFilter.dataset.wafFilterKey as WAFDashboardFilterKey | undefined, removeFilter.dataset.wafFilterGroup);
      return;
    }
    const clearFilters = event.target.closest<HTMLElement>('[data-waf-dashboard-action="clear-filters"]');
    if (clearFilters) {
      event.preventDefault();
      clearWAFDashboardFilters();
      return;
    }
    const timeTrigger = event.target.closest<HTMLElement>('[data-waf-dashboard-action="toggle-time-panel"]');
    if (timeTrigger) {
      event.preventDefault();
      hideWAFFilterBuilder();
      toggleWAFTimePanel();
    ...
    const preset = event.target.closest<HTMLElement>('[data-waf-dashboard-action="apply-time-preset"]');
    if (preset?.dataset.wafTimePreset) {
      event.preventDefault();
      applyWAFWindow(preset.dataset.wafTimePreset);
      return;
    }
    const customRange = event.target.closest<HTMLElement>('[data-waf-dashboard-action="apply-last-range"]');
    if (customRange) {
      event.preventDefault();
      applyWAFLastRange();
      return;
    }
    const sectionAnalyticsTab = event.target.closest<HTMLButtonElement>('[data-waf-dashboard-action="select-section-analytics"]');
    ...
    const openModal = event.target.closest<HTMLElement>('[data-waf-dashboard-action="open-statistics-modal"]');
    if (openModal) {
      event.preventDefault();
      openSecurityStatisticsModal(openModal);
      return;
    }
    const closeModal = event.target.closest<HTMLElement>('[data-waf-dashboard-action="close-statistics-modal"]');
    if (closeModal) {
      event.preventDefault();
      closeSecurityStatisticsModal();
      return;
    }
    const modalFilter = event.target.closest<HTMLElement>('[data-waf-dashboard-action="apply-modal-filter"]');
    if (modalFilter?.dataset.wafDashboardFilterKey && modalFilter.dataset.wafDashboardFilterValue) {
      event.preventDefault();
      closeSecurityStatisticsModal();
      applyWAFDashboardFilter(modalFilter.dataset.wafDashboardFilterKey as WAFDashboardFilter, modalFilter.dataset.wafDashboardFilterValue);
      return;
    }
    const toggleCards = event.target.closest<HTMLElement>('[data-waf-dashboard-action="toggle-statistics-cards"]');
    if (toggleCards) {
      event.preventDefault();
    ...
    const refresh = event.target.closest<HTMLButtonElement>('[data-waf-dashboard-action="refresh"]');
    if (refresh) {
      event.preventDefault();
      void loadWAFDashboard();
      return;
    }
    const switchTab = event.target.closest<HTMLButtonElement>('[data-waf-dashboard-action="switch-drawer-tab"]');
    if (switchTab?.dataset.wafDrawerTab) {
      event.preventDefault();
      setWAFDrawerTab(switchTab.dataset.wafDrawerTab as 'overview' | 'raw');
      return;
    }
    const copyBtn = event.target.closest<HTMLElement>('[data-waf-dashboard-action="copy-drawer-value"]');
    if (copyBtn?.dataset.wafCopyValue) {
      event.preventDefault();
      void copyToClipboard(copyBtn.dataset.wafCopyValue);
      return;
    }
    const drawerFilter = event.target.closest<HTMLElement>('[data-waf-dashboard-action="apply-drawer-filter"]');
    if (drawerFilter?.dataset.wafFilterKey && drawerFilter.dataset.wafFilterValue) {
      event.preventDefault();
      closeWAFDrawer();
      applyWAFDashboardFilter(drawerFilter.dataset.wafFilterKey as WAFDashboardFilter, drawerFilter.dataset.wafFilterValue);
      return;
    }
    const closeDrawer = event.target.closest<HTMLElement>('[data-waf-dashboard-action="close-drawer"]');
    if (closeDrawer) {
      event.preventDefault();
      closeWAFDrawer();
      return;
    ...
    const prevPage = event.target.closest<HTMLButtonElement>('[data-waf-dashboard-action="previous-events-page"]');
    if (prevPage) {
      event.preventDefault();
      if (wafEventsPage > 1) {
        wafEventsPage -= 1;
    ...
    const nextPage = event.target.closest<HTMLButtonElement>('[data-waf-dashboard-action="next-events-page"]');
    if (nextPage) {
      event.preventDefault();
      wafEventsPage += 1;
      renderSelectedSecurityEvents();
    ...
    const eventRow = event.target.closest<HTMLElement>('[data-waf-dashboard-event]');
  ...
  document.addEventListener('change', event => {
    ...
    if (event.target.id === 'waf-filter-field') {
      const fieldSelect = event.target as HTMLSelectElement;
      selectedFilterBuilderField = fieldSelect.value;
      const opSelect = byId('waf-filter-operator');
      if (opSelect) opSelect.innerHTML = renderFilterOperators(selectedFilterBuilderField);
      const valContainer = byId('waf-filter-value-container');
      if (valContainer) valContainer.innerHTML = renderFilterValueControl(selectedFilterBuilderField);
      const valInput = byId<HTMLInputElement | HTMLSelectElement>('waf-filter-value');
      if (valInput) valInput.focus();
      return;
    }
    const pageSizeSelect = event.target.closest<HTMLSelectElement>('[data-waf-dashboard-action="change-page-size"]');
    if (pageSizeSelect) {
  ...
  document.addEventListener('keydown', event => {
    ...
    if (sectionAnalyticsTab && ['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) {
      const tabs = Array.from(document.querySelectorAll<HTMLButtonElement>('[data-waf-dashboard-action="select-section-analytics"]'));
      ...
      if (!next?.dataset.wafDashboardSection) return;
      event.preventDefault();
      selectSecuritySectionAnalytics(next.dataset.wafDashboardSection, true);
      return;
    }
    const eventRow = event.target.closest<HTMLElement>('.waf-event-row[data-waf-dashboard-event]');
    ...
    const filterTarget = event.target.closest<HTMLElement>('[data-waf-dashboard-filter-key][data-waf-dashboard-filter-value]');
    if (!filterTarget || filterTarget instanceof HTMLButtonElement) return;
    event.preventDefault();
    filterTarget.click();
  });
...
function consumePendingDashboardFilters(): void {
  ...
  if (raw) {
    ...
    try {
      const data = JSON.parse(raw) as { window?: string; filters?: Record<string, string> };
      if (data.window) {
      ...
      if (data.filters && typeof data.filters === 'object') {
        ...
        Object.entries(data.filters).forEach(([k, v]) => {
          ...
          if (!val) return;
          wafDashboardFilters[k as WAFDashboardFilterKey] = normalizeDashboardFilterValue(k as WAFDashboardFilterKey, val);
        });
        wafEventsPage = 1;
      }
    } catch (err) {
  ...
  try {
    ...
    const filterKeys: WAFDashboardFilterKey[] = [
...
export function initWAFDashboard(): void {
...
export function disposeWAFDashboard(): void {
  requestVersion += 1;
  closeWAFDrawer();
  closeSecurityStatisticsModal();
}
