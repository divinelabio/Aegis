...
export type DashboardTab = 'security' | 'system' | 'traffic';
 ...
 */
export function switchDashboardTab(tab: DashboardTab | string = 'security'): void {
  const container = AdminDOM.getById('dashboard-content');
  if (!container) return;

  disposeWAFDashboard();
  disposeTrafficDashboard();
  const normalizedTab: DashboardTab = tab === 'system' || tab === 'traffic' ? tab : 'security';
  ...
  if (normalizedTab === 'system') {
    ...
    if (pageTitle) pageTitle.textContent = 'System Health & Engine Telemetry';
    loadSystemDashboard();
    return;
  }
  ...
  if (normalizedTab === 'traffic') {
    ...
    if (pageTitle) pageTitle.textContent = 'Traffic Control Dashboard';
  ...
  if (pageTitle) pageTitle.textContent = 'Security Dashboard';
  initWAFDashboard();
}
