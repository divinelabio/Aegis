...
const ruleCache = {
  data: new Map<string, CachedRuleData>(),

  set(key: string, value: RulePolicy[], ttl = 300000): void {
    this.data.set(key, {
      value,
  ...
  get(key: string): RulePolicy[] | null {
...
const ruleTestExamples: Record<string, { method: string; uri: string; headers: string; body: string; description: string }> = {
...
function encodePolicyValue(value: string): string {
  return encodeURIComponent(value);
}

function decodePolicyValue(value: string | undefined): string {
  return value ? decodeURIComponent(value) : '';
}

function openRuleEditor(filePath: string): void {
...
function ensureCategoriesBindings(): void {
  if (categoriesBindingsInitialized) return;
  categoriesBindingsInitialized = true;

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '[data-categories-action]', (_event, target) => {
    ...
    switch (action) {
      case 'open-add-policy-modal':
        ...
        break;
      case 'delete-policy':
        if (file) void deletePolicyFile(file);
        break;
      case 'edit-policy':
        if (file) openRuleEditor(file);
        break;
      case 'close-modal':
  ...
  AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '[data-categories-toggle="policy-enabled"]', (_event, target) => {
    const file = decodePolicyValue(target.dataset.policyFile);
    if (!file) return;
...
export async function loadPoliciesTable(): Promise<void> {
  const container = AdminDOM.getById('policies-container');
  if (!container) return;
  ...
  try {
    const data = await api.get<RulesResponse>('rules');
...
function reloadPolicies(): void {
  ruleCache.clear();
  void loadPoliciesTable();
}

async function togglePolicyFile(filename: string, enable: boolean): Promise<void> {
  try {
...
async function deletePolicyFile(filename: string): Promise<void> {
  SectionUI.openConfirmModal(
    'Delete Policy File',
    `Are you sure you want to delete policy <strong>"${filename}"</strong>?<br><br>This action cannot be undone.`,
    'Delete',
    'var(--danger)',
    () => {
...
async function confirmDeletePolicyFile(filename: string): Promise<void> {
...
function openAddPolicyModal(): void {
  const modalHtml = `
...
function categoriesShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}
