...
interface EnterpriseModulesResponse {
  license_tier?: string;
...
interface EnterpriseNavItem {
  ...
  label: string;
  module: keyof Pick<
    EnterpriseModulesResponse,
    'bot_protection' | 'api_security' | 'access_control'
  >;
  permission: string;
  icon: string;
}

const ENTERPRISE_NAV_ITEMS: EnterpriseNavItem[] = [
  {
...
function buildNavItem(item: EnterpriseNavItem): HTMLAnchorElement {
...
async function loadEnterpriseNavigation(): Promise<void> {
  const appSecurityLink = AdminDOM.query<HTMLAnchorElement>(
    '.nav-link[data-nav-target="httpsecurity_config"]'
  );
  if (!appSecurityLink) return;

  try {
    const setupResponse = await fetch('/api/setup/status', { credentials: 'same-origin', headers: { Accept: 'application/json' } });
    if (setupResponse.ok) {
      const setup = (await setupResponse.json()) as SetupStatusResponse;
      if (setup.setup_required && setup.user_count === 0) return;
    }
    ...
    if (!modulesResponse.ok || !sessionResponse.ok) return;

    const modules = (await modulesResponse.json()) as EnterpriseModulesResponse;
    ...
    let insertionPoint: Element = appSecurityLink;
    ENTERPRISE_NAV_ITEMS.forEach(item => {
      if (!modules[item.module] || !permissions.has(item.permission)) return;
      if (AdminDOM.query(`[data-nav-target="${item.target}"]`)) return;
