...
type AnalyticsAggregateItem = Record<string, unknown>;

interface SecurityAnalyticsModel {
  events: SecurityEvent[];
  summary: SecuritySummary;
  timeline: AnalyticsAggregateItem[];
  actionMix: AnalyticsAggregateItem[];
  breakdowns: Record<string, AnalyticsAggregateItem[]>;
  warnings: string[];
  loadedAt: Date;
  filters: SecurityAnalyticsFilters;
  nextCursor?: string;
}

interface TrafficEventsPageResponse {
...
interface SecurityAnalyticsFilters {
  window: string;
  statusClass: string;
  method: string;
  country: string;
...
interface TrafficEventsDashboardResponse {
  dashboard?: {
    summary?: {
    ...
    timeseries?: Array<{ timestamp?: number; counts?: Record<string, number> }>;
    breakdowns?: Record<string, AnalyticsAggregateItem[]>;
    events?: { events?: Array<Record<string, unknown>>; next_cursor?: string };
    warnings?: string[];
    health?: {
...
interface SecurityRuleEditorOptions {
  containerId?: string;
  apiBase?: string;
  defaults?: Partial<SecurityRule>;
  lockType?: boolean;
  hideLockedType?: boolean;
  blankCreate?: boolean;
...
let cachedFields: FieldSpec[] = defaultFields;
let builderMode: 'visual' | 'raw' = 'visual';
let activeEditorOptions: SecurityRuleEditorOptions = {};
let securityAnalyticsController: AbortController | null = null;
let securityAnalyticsRequestId = 0;
let securityAnalyticsEvents: SecurityEvent[] = [];
let securityAnalyticsBound = false;
let securityAnalyticsCustomSince = '';
let securityAnalyticsCustomUntil = '';
let securityDrawerReturnFocus: HTMLElement | null = null;
let securityEventsPage = 1;
let securityEventsCursors = [''];
let securityEventsNextCursor = '';
let securityEventsPageRequest: AbortController | null = null;
...
export function featurePageView(title: string, subtitle: string, endpoint: string): string {
  return `
    <div class="view">
      <div class="panel-row">
        <div>
...
export async function initSecurityAnalytics(): Promise<void> {
  ...
  const refreshButton = AdminDOM.query<HTMLButtonElement>('[data-action="refresh-security-analytics"]');
  if (refreshButton) {
  ...
  try {
    ...
    if (requestId !== securityAnalyticsRequestId) return;
...
function readSecurityAnalyticsFilters(): SecurityAnalyticsFilters {
  return {
...
function buildSecurityAnalyticsParams(filters: SecurityAnalyticsFilters): URLSearchParams {
  ...
  if (filters.userAgent) params.set('user_agent', filters.userAgent);
  if (filters.ja3) params.set('ja3', filters.ja3);
  if (filters.ja4) params.set('ja4', filters.ja4);
  if (filters.asn) params.set('asn', filters.asn);
  if (filters.httpVersion) params.set('http_version', filters.httpVersion);
  if (filters.tlsVersion) params.set('tls_version', filters.tlsVersion);
  if (filters.ipVersion) params.set('ip_version', filters.ipVersion);
  ...
  if (securityAnalyticsCustomUntil) params.set('until', securityAnalyticsCustomUntil);
  return params;
}

function analyticsSince(windowValue: string): Date | null {
...
async function loadSecurityAnalytics(params: URLSearchParams, signal: AbortSignal, filters: SecurityAnalyticsFilters): Promise<SecurityAnalyticsModel> {
  const response = await analyticsGet<TrafficEventsDashboardResponse>(`traffic-events/dashboard?${params.toString()}`, signal);
  const dashboard = response.dashboard;
  if (!dashboard) throw new Error('Traffic analytics are currently unavailable.');
...
function normalizeTrafficEvent(item: Record<string, unknown>): SecurityEvent {
  const seconds = Number(item.timestamp || 0);
  const metadata = item.metadata && typeof item.metadata === 'object' ? item.metadata as Record<string, string> : {};
  return {
...
async function analyticsGet<T>(endpoint: string, parentSignal: AbortSignal): Promise<T> {
...
function eventMatchesSecurityFilters(event: SecurityEvent, filters: SecurityAnalyticsFilters): boolean {
  if (filters.statusClass) {
    const code = Number(event.response_code || 0);
    const sc = filters.statusClass.toLowerCase();
    if (sc === '2xx' && (code < 200 || code >= 300)) return false;
    if (sc === '3xx' && (code < 300 || code >= 400)) return false;
    if (sc === '4xx' && (code < 400 || code >= 500)) return false;
    if (sc === '5xx' && (code < 500 || code >= 600)) return false;
  }
  ...
  if (filters.host) {
    ...
    if (!eh.includes(fh) && !fh.includes(eh)) return false;
  }
  if (filters.route) {
    const fr = filters.route.toLowerCase();
    const er = (event.route_name || '').toLowerCase();
    if (!er.includes(fr) && !fr.includes(er)) return false;
  }
  if (filters.upstream) {
    const fu = filters.upstream.toLowerCase();
    const eu = (event.upstream || '').toLowerCase();
    if (!eu.includes(fu) && !fu.includes(eu)) return false;
  }
  if (filters.requestId && event.request_id !== filters.requestId) return false;
  ...
  if (filters.asn) {
    ...
    if (!ea.includes(fa) && !fa.includes((event.asn || '').toLowerCase())) return false;
  }
  if (filters.statusCode && String(event.response_code) !== String(filters.statusCode)) return false;
  if (filters.httpVersion && (event.http_version || '').toLowerCase() !== filters.httpVersion.toLowerCase()) return false;
  if (filters.tlsVersion && (event.tls_version || '').toLowerCase() !== filters.tlsVersion.toLowerCase()) return false;
  if (filters.ipVersion && (event.ip_version || '').toLowerCase() !== filters.ipVersion.toLowerCase()) return false;
  if (filters.contentType) {
    const fc = filters.contentType.toLowerCase();
    const ec = (event.response_content_type || '').toLowerCase();
    if (!ec.includes(fc) && !fc.includes(ec)) return false;
  }
  if (filters.ja3 && event.ja3 !== filters.ja3) return false;
  if (filters.ja4 && event.ja4 !== filters.ja4) return false;
  return true;
}

function createEmptySecurityAnalyticsModel(filters: SecurityAnalyticsFilters, warnings: string[] = ['Traffic telemetry is currently unavailable.']): SecurityAnalyticsModel {
...
function renderSecurityAnalytics(model: SecurityAnalyticsModel): string {
...
function renderSecurityEventsTableContent(events: SecurityEvent[], total: number, pageSize: string): string {
...
function securityEventsPageSize(value: string): number {
  const pageSize = Number(value);
  return [20, 50, 100, 200].includes(pageSize) ? pageSize : 20;
}

async function changeSecurityEventsPage(direction: 'next' | 'previous'): Promise<void> {
  const previousPage = securityEventsPage;
  const targetPage = direction === 'next' ? previousPage + 1 : previousPage - 1;
  if (targetPage < 1 || (direction === 'next' && !securityEventsNextCursor)) return;
  ...
  try {
    ...
    const response = await analyticsGet<TrafficEventsPageResponse>(`traffic-events/events?${params.toString()}`, controller.signal);
...
function renderSecurityTrafficEmptyNotice(): string {
  return `
...
function renderSecurityMetricStrip(summary: SecuritySummary): string {
...
function securityMetricItem(label: string, value: string, sub: string, tone = ''): string {
  return `
...
function securityEventsTimeRangeLabel(filters: SecurityAnalyticsFilters): string {
...
function activeSecurityTimePreset(filters: SecurityAnalyticsFilters): string {
...
function durationToMilliseconds(amount: number, unit: string): number {
...
function setSecurityAnalyticsTimeRange(sinceIso: string, untilIso = ''): void {
...
function setSecurityEventsHeaderControls(filters: SecurityAnalyticsFilters, activeFilterChips: string): void {
  ...
  if (filterTarget) filterTarget.innerHTML = renderSecurityHeaderFilters(filters, activeFilterChips);
  if (timeTarget) timeTarget.innerHTML = renderSecurityHeaderTime(filters);
}

function renderSecurityHeaderFilters(filters: SecurityAnalyticsFilters, activeFilterChips: string): string {
...
function renderSecurityHeaderTime(filters: SecurityAnalyticsFilters): string {
...
function renderSecurityAnalyticsNotice(message: string, label: string): string {
  if (!message) return '';
  return `
    <div class="security-events-notice" role="status">
...
function renderActiveFilterChips(filters: SecurityAnalyticsFilters): string {
  const chips: string[] = [];
  const push = (label: string, value: string | undefined, key: string): void => {
    if (!value) return;
    chips.push(`
...
function renderSecurityTrafficCard(points: ReturnType<typeof securityTimelineSeries>): string {
  return `
...
function renderSecurityTimelineSvg(points: ReturnType<typeof securityTimelineSeries>): string {
  if (!points.length) return '<div class="security-events-chart-fallback">No timeline data</div>';
...
function securityTimelineSeries(model: SecurityAnalyticsModel): Array<{ bucket: string; label: string; total: number; status2xx: number; status3xx: number; status4xx: number; status5xx: number }> {
  const grouped = new Map<string, { bucket: string; total: number; status2xx: number; status3xx: number; status4xx: number; status5xx: number }>();
  model.timeline.forEach(item => {
    const bucket = String(item.bucket || item.timestamp || item.time || '');
    if (!bucket) return;
    const counts = item.counts && typeof item.counts === 'object' ? item.counts as Record<string, number> : {};
  ...
  const fromTimeline = normalizeSecurityTimelineBuckets(
    Array.from(grouped.values()),
    model.filters.window
  );
  if (fromTimeline.length) return fromTimeline;

  const buckets = new Map<string, { bucket: string; total: number; status2xx: number; status3xx: number; status4xx: number; status5xx: number }>();
  model.events.forEach(event => {
    ...
    row.total += 1;
    if (event.response_code >= 500) row.status5xx += 1;
    else if (event.response_code >= 400) row.status4xx += 1;
    else if (event.response_code >= 300) row.status3xx += 1;
    else if (event.response_code >= 200) row.status2xx += 1;
    buckets.set(bucket, row);
  });
  return normalizeSecurityTimelineBuckets(Array.from(buckets.values()), model.filters.window);
}

function normalizeSecurityTimelineBuckets(items: Array<{ bucket: string; total: number; status2xx: number; status3xx: number; status4xx: number; status5xx: number }>, windowValue: string): Array<{ bucket: string; label: string; total: number; status2xx: number; status3xx: number; status4xx: number; status5xx: number }> {
...
function securityTimelinePointLimit(windowValue: string): number {
  if (windowValue === '1h') return 60;
  if (windowValue === '7d') return 56;
  if (windowValue === '30d') return 60;
  return 48;
}

function snapSecurityTimelineDate(date: Date, windowValue: string): void {
  if (windowValue === '1h') {
    date.setSeconds(0, 0);
...
function formatTimelineBucketLabel(value: string, windowValue: string): string {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  if (windowValue === '1h') return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  if (windowValue === '24h') return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  return date.toLocaleString([], { month: 'short', day: 'numeric' });
}

function renderSecurityStatisticsCards(model: SecurityAnalyticsModel): string {
...
function securityStatisticsVisibleCardCount(): number {
  if (window.innerWidth >= 2560) return 10;
  if (window.innerWidth >= 1800) return 8;
  return 9;
}

function renderSecurityActionMixChartCard(model: SecurityAnalyticsModel): string {
...
function renderSecurityActionMixSvg(items: Array<{ key: string; label: string; count: number; tone: string }>, total: number): string {
  const visible = items.filter(item => item.count > 0);
  if (!total || !visible.length) return '<div class="security-events-chart-fallback">No response data</div>';
  let offset = 0;
...
function renderActionMixLegendRow(item: { key: string; label: string; count: number; tone: string }, total: number): string {
...
function actionMixChartItems(model: SecurityAnalyticsModel): Array<{ key: string; label: string; count: number; tone: string }> {
...
function renderStatisticCard(title: string, items: AnalyticsAggregateItem[], valueKey: string, filterKey: string): string {
...
function renderSecurityStatRows(items: Array<{ value: string; count: number }>, filterKey: string): string {
  if (!items.length) {
...
function normalizeAggregateItems(items: AnalyticsAggregateItem[], valueKey: string): Array<{ value: string; count: number }> {
  return items.map(item => ({ value: String(item[valueKey] || item.label || item.key || item.value || '-'), count: Number(item.count || 0) }));
}

function timelineAggregateItems(items: AnalyticsAggregateItem[]): AnalyticsAggregateItem[] {
...
function aggregateEvents(events: SecurityEvent[], readValue: (event: SecurityEvent) => string, limit = 8): AnalyticsAggregateItem[] {
...
function renderSecurityAnalyticsUnavailable(message: string): string {
  return `
    <div class="security-analytics-shell security-events-explorer security-events-operator-explorer">
...
function renderEventExplorerRows(events: SecurityEvent[]): string {
  if (!events.length) return '<tr><td colspan="7" class="dashboard-empty-cell"><div class="dashboard-empty-state"><div class="dashboard-empty-title">No traffic requests</div><div class="dashboard-empty-detail">Adjust the filters or time range.</div><button type="button" class="btn btn-outline btn-sm" data-security-analytics-action="clear-filters">Clear filters</button></div></td></tr>';
...
function securityTrafficStatusDashboardBadge(status: number): string {
  if (!status) return '<span class="security-events-status">-</span>';
...
function securityActionBadge(action: string): string {
...
function securityStatusBadge(status: number): string {
...
function securityStatusLabel(status: number): string {
...
function ensureSecurityEventDrawerMounted(): HTMLElement | null {
  const existingInBody = AdminDOM.query<HTMLElement>(':scope > #security-event-explorer-drawer', document.body);
  const viewRoot = AdminDOM.getById('view-root');
  const insideView = viewRoot ? AdminDOM.query<HTMLElement>('#security-event-explorer-drawer', viewRoot) : null;
  if (existingInBody && insideView && existingInBody !== insideView) {
    insideView.remove();
    return existingInBody;
  }
  const drawer = existingInBody || insideView || AdminDOM.getById<HTMLElement>('security-event-explorer-drawer');
...
function openSecurityEventExplorerDrawer(id: string): void {
  ...
  if (section) section.textContent = event.route_name || event.upstream || 'Request';
  if (time) time.textContent = formatEventDateTime(event.timestamp);
  body.innerHTML = renderSecurityEventDetail(event);
  body.scrollTop = 0;
  const panels = body.querySelector<HTMLElement>('.security-event-workbench-panels');
  if (panels) panels.scrollTop = 0;
  setSecurityEventDrawerTab('overview');
  drawer.querySelector<HTMLElement>('[data-security-analytics-action="close-drawer"]')?.focus();
}

function closeSecurityEventExplorerDrawer(): void {
...
function renderSecurityEventDetail(event: SecurityEvent): string {
...
function detailCardRow(label: string, value: string, mono = false): string {
  return `
...
function renderEventCommandBar(event: SecurityEvent): string {
  const copyActions: string[] = [];
  const filterActions: string[] = [];
  if (event.client_ip) copyActions.push(drawerActionButton('copy-drawer-value', 'Copy IP', SectionUI.icons.globe, { securityCopyValue: event.client_ip }));
  if (event.path) copyActions.push(drawerActionButton('copy-drawer-value', 'Copy path', SectionUI.icons.route, { securityCopyValue: event.path }));
  if (event.request_id) copyActions.push(drawerActionButton('copy-drawer-value', 'Copy ID', SectionUI.icons.arrowRight, { securityCopyValue: event.request_id }));
  ...
  if (event.client_ip) filterActions.push(drawerActionButton('apply-drawer-filter', 'Filter IP', SectionUI.icons.filter, { securityFilterKey: 'ip', securityFilterValue: event.client_ip }));
  if (event.path) filterActions.push(drawerActionButton('apply-drawer-filter', 'Filter path', SectionUI.icons.filter, { securityFilterKey: 'path', securityFilterValue: event.path }));
  if (event.response_code) filterActions.push(drawerActionButton('apply-drawer-filter', 'Filter status', SectionUI.icons.filter, { securityFilterKey: 'statusCode', securityFilterValue: String(event.response_code) }));
  if (event.route_name) filterActions.push(drawerActionButton('apply-drawer-filter', 'Filter route', SectionUI.icons.filter, { securityFilterKey: 'route', securityFilterValue: event.route_name }));
  if (event.upstream) filterActions.push(drawerActionButton('apply-drawer-filter', 'Filter upstream', SectionUI.icons.filter, { securityFilterKey: 'upstream', securityFilterValue: event.upstream }));
...
function drawerActionButton(action: string, label: string, icon: string, data: Record<string, string> = {}, primary = false): string {
...
function detailRows(rows: Array<[string, string | number | undefined | null]>): string {
  const visibleRows = rows
    .map(([label, value]) => [label, value == null ? '' : String(value).trim()] as const)
    .filter(([, value]) => value !== '');
...
function setSecurityEventDrawerTab(tab: string): void {
  const target = tab === 'raw' ? 'raw' : 'overview';
  document.querySelectorAll<HTMLElement>('[data-security-drawer-tab]').forEach(button => {
    const active = button.dataset.securityDrawerTab === target;
    button.classList.toggle('is-active', active);
    button.setAttribute('aria-selected', String(active));
  });
  document.querySelectorAll<HTMLElement>('[data-security-drawer-panel]').forEach(panel => {
...
function securitySectionRoute(section: string): string {
...
function securityEvidenceRows(event: SecurityEvent, section: string): string {
  const matchedRows = Object.entries(event.matched_fields || {})
    .filter(([key, value]) => shouldShowGenericEvidence(key, value))
    .map(([key, value]) => [labelize(key), value] as [string, string]);
  const coreRows: Array<[string, string | undefined]> = [
...
function shouldShowGenericEvidence(key: string, value: string): boolean {
...
function eventEvidenceValue(event: SecurityEvent, keys: string[]): string {
...
function compactUserAgent(value?: string): string {
  ...
  const browserMap: Record<string, string> = { opr: 'Opera', edg: 'Edge', chrome: 'Chrome', firefox: 'Firefox', safari: 'Safari' };
...
async function copySecurityDrawerValue(value: string): Promise<void> {
...
function openSecurityRawEvent(): void {
  setSecurityEventDrawerTab('raw');
  AdminDOM.getById('security-event-tab-raw')?.focus();
}

function openSecuritySectionRoute(route: string): void {
  const router = (window as Window & { router?: RouterApi }).router;
  if (!route || !router?.views?.[route]) return;
  closeSecurityEventExplorerDrawer();
  router.navigate(route);
}

function applySecurityTimePreset(value: string): void {
  ...
  const unitMap: Record<string, string> = { m: 'minutes', h: 'hours', d: 'days', w: 'weeks' };
...
function applySecurityLastRange(): void {
...
function applySecurityDateInterval(): void {
...
function bindSecurityAnalyticsInteractions(): void {
  if (securityAnalyticsBound) return;
  securityAnalyticsBound = true;
  document.addEventListener('click', event => {
    if (!(event.target instanceof Element)) return;
    const analyticsAction = event.target.closest<HTMLElement>('[data-security-analytics-action]');
    if (analyticsAction) {
    ...
    const eventRow = event.target.closest<HTMLElement>('[data-security-event-id]');
    if (eventRow) {
      event.preventDefault();
  ...
  document.addEventListener('keydown', event => {
    ...
    if (!(event.target instanceof Element)) return;
    const svgAction = event.target.closest<SVGElement>('.security-events-svg-action[data-security-analytics-action]');
    ...
    const eventRow = event.target.closest<HTMLElement>('[data-security-event-id]');
    if (!eventRow || event.target !== eventRow) return;
    event.preventDefault();
    openSecurityEventExplorerDrawer(eventRow.dataset.securityEventId || '');
  });
  ...
  window.addEventListener('resize', () => {
    document.querySelectorAll<HTMLElement>('[data-security-statistics-deck]:not(.is-expanded)').forEach(syncSecurityStatisticsShowMore);
  });
}

function toggleSecurityStatisticsCards(button: HTMLElement): void {
  const deck = button.closest<HTMLElement>('[data-security-statistics-deck]');
  if (!deck) return;
...
function securityStatisticsHiddenCardCount(deck: HTMLElement): number {
  return Math.max(0, deck.querySelectorAll('.security-statistics-card').length - securityStatisticsVisibleCardCount());
}

function syncSecurityStatisticsShowMore(deck: HTMLElement): void {
  const button = deck.querySelector<HTMLElement>('[data-security-analytics-action="toggle-statistics-cards"]');
  if (!button) return;
  const hiddenCardCount = securityStatisticsHiddenCardCount(deck);
  button.hidden = hiddenCardCount === 0;
  if (!button.hidden) button.textContent = `Show ${hiddenCardCount} more breakdowns`;
}

function openSecurityStatisticsModal(source: HTMLElement): void {
...
function closeSecurityStatisticsModal(): void {
  AdminDOM.getById('security-statistics-modal')?.classList.add('hidden');
}

function parseStatisticModalItems(value: string): Array<{ value: string; count: number }> {
  try {
    const parsed = JSON.parse(decodeURIComponent(value)) as Array<{ value?: unknown; count?: unknown }>;
...
function statisticModalRow(item: { value: string; count: number }, filterKey: string, total: number, max: number): string {
...
function clearSecurityAnalyticsFilters(): void {
  securityAnalyticsCustomSince = '';
  securityAnalyticsCustomUntil = '';
  ...
  [
    ...
    const input = AdminDOM.getById<HTMLInputElement>(id);
    if (input) input.value = '';
  });
}

function removeSecurityAnalyticsFilter(key: string): void {
  if (!key) return;
  ...
  const input = inputMap[key] ? AdminDOM.getById<HTMLInputElement>(inputMap[key]) : null;
  if (input) input.value = '';
}

function toggleSecurityFilterMenu(): void {
  ...
  if (isOpen) {
    updateSecurityFilterBuilderForField();
    AdminDOM.getById<HTMLSelectElement>('security-filter-field')?.focus();
  }
}

function hideSecurityFilterBuilder(): void {
...
function toggleSecurityTimePanel(): void {
...
function hideSecurityTimePanel(): void {
...
function setSecurityTimeMode(mode: string): void {
  ...
  document.querySelectorAll<HTMLElement>('.security-time-mode').forEach(button => {
    button.classList.toggle('is-active', button.dataset.securityTimeMode === normalized);
  });
}

function updateSecurityFilterBuilderForField(): void {
  const field = AdminDOM.selectValue('security-filter-field', 'path');
  const operator = AdminDOM.getById<HTMLSelectElement>('security-filter-operator');
  const value = AdminDOM.getById<HTMLInputElement>('security-filter-value');
  ...
  if (value) value.placeholder = placeholders[field] || 'value';
}

function securityFilterOperatorsForField(field: string): Array<{ value: string; label: string; disabled?: boolean }> {
  const equality = [
  ...
  if (field === 'statusCode' || field === 'statusClass' || field === 'asn' || field === 'httpVersion' || field === 'tlsVersion' || field === 'ipVersion' || field === 'contentType') return equality;
  if (field === 'path' || field === 'host' || field === 'route' || field === 'upstream' || field === 'requestId' || field === 'userAgent' || field === 'ja3' || field === 'ja4') return text;
  return equality;
}

function applySecurityFilterBuilder(): void {
  const field = AdminDOM.selectValue('security-filter-field', 'path');
...
function normalizeSecurityFilterValue(field: string, value: string, operator = 'equals'): string {
  const firstValue = operator === 'in' ? value.split(',').map(item => item.trim()).filter(Boolean)[0] || value : value;
  if (field === 'method' || field === 'country') return firstValue.toUpperCase();
  if (field === 'asn') return firstValue.toUpperCase().startsWith('AS') ? firstValue.toUpperCase() : `AS${firstValue}`;
  if (field === 'ipVersion') return firstValue.toLowerCase() === 'ipv6' ? 'IPv6' : 'IPv4';
  if (field === 'contentType') return firstValue.toLowerCase();
  if (field === 'statusClass') return firstValue.toLowerCase();
  return firstValue;
}

function applySecurityAnalyticsFilter(key: string, value: string, toggle = true): void {
  if (!key || !value) return;
  const inputMap: Record<string, string> = {
    country: 'analytics-country-filter',
    ip: 'analytics-ip-filter',
  ...
  if (!inputId) return;
  const input = AdminDOM.getById<HTMLInputElement>(inputId);
  if (input) {
...
function setSelectValue(id: string, value: string): void {
  const select = AdminDOM.getById<HTMLSelectElement>(id);
  if (select) select.value = value;
}

function responseClassItems(summary: SecuritySummary): AnalyticsAggregateItem[] {
  return [
...
function responseClassLabel(value: string): string {
  const labels: Record<string, string> = { '2xx': 'Successful', '3xx': 'Redirects', '4xx': 'Client errors', '5xx': 'Server errors' };
  return `${value} ${labels[value] || 'Other'}`;
}

function responseClassTone(value: string): string {
  const tones: Record<string, string> = { '2xx': 'allow', '3xx': 'log', '4xx': 'challenge', '5xx': 'block' };
  return tones[value] || 'log';
}

function countSecurityAnalyticsFilters(filters: SecurityAnalyticsFilters): number {
  return [
    filters.statusClass,
    filters.method,
    filters.country,
...
function labelForOption(selectId: string, value: string): string {
  const select = AdminDOM.getById<HTMLSelectElement>(selectId);
  return select?.querySelector<HTMLOptionElement>(`option[value="${CSS.escape(value)}"]`)?.textContent || value;
}
...
export function openSecurityRuleModal(rule?: SecurityRule, options: SecurityRuleEditorOptions = {}): void {
  ...
  const defaultRule: SecurityRule = {
  ...
  const initialConditions = blankCreate
    ? [{ joiner: 'and' as const, field: isBotEditor ? '' : defaultFieldForType(current.type), operator: isBotEditor ? '' : defaultOperatorForField(defaultFieldForType(current.type)), value: '' }]
...
export async function saveSecurityRule(id?: string, draft = false): Promise<void> {
  ...
  if (draft) rule.enabled = false;
  const apiBase = activeEditorOptions.apiBase || 'v2/security/bot/rules';
  const validation = await api.post<ValidationResponse>(`${apiBase}/validate`, { expression: rule.expression, rule });
...
export async function closeSecurityRuleEditor(): Promise<void> {
...
export function applyVisualExpression(): void {
  syncExpressionFromConditions(true);
}

export function addSecurityRuleCondition(condition?: Partial<VisualCondition>): void {
...
export function removeSecurityRuleCondition(index: string): void {
  const row = AdminDOM.query(`[data-condition-index="${CSS.escape(index)}"]`);
  if (row) row.remove();
  reindexConditionRows();
  if (!AdminDOM.query('.security-rule-condition-row')) addSecurityRuleCondition();
  syncExpressionFromConditions(true);
}

export function updateSecurityRuleConditionControl(target: Element): void {
  ...
  if (target instanceof HTMLInputElement && target.classList.contains('security-rule-field-search')) {
    ...
    const hiddenField = row.querySelector<HTMLInputElement>('.security-rule-condition-field');
    ...
    if (hiddenField) hiddenField.value = match.name;
    target = hiddenField || target;
  }
  const fieldSelect = row.querySelector<HTMLInputElement | HTMLSelectElement>('.security-rule-condition-field');
  const operatorSelect = row.querySelector<HTMLSelectElement>('.security-rule-condition-operator');
  if (target === fieldSelect && fieldSelect) {
    if (operatorSelect) operatorSelect.innerHTML = operatorOptions(fieldSelect.value, defaultOperatorForField(fieldSelect.value));
    const valueShell = row.querySelector<HTMLElement>('.security-rule-condition-value-shell');
    const nextValue = isBotRuleEditor() ? '' : defaultValueForField(fieldSelect.value, type);
    if (valueShell) valueShell.innerHTML = `Value${renderValueControl(fieldSelect.value, nextValue, row.dataset.conditionIndex || '0', operatorSelect?.value || defaultOperatorForField(fieldSelect.value))}`;
  } else if (target === operatorSelect) {
    const fieldValue = fieldSelect?.value || (isBotRuleEditor() ? '' : defaultFieldForType(type));
    const valueShell = row.querySelector<HTMLElement>('.security-rule-condition-value-shell');
    if (valueShell) valueShell.innerHTML = `Value${renderValueControl(fieldValue, readSingleConditionValue(row), row.dataset.conditionIndex || '0', operatorSelect?.value || defaultOperatorForField(fieldValue))}`;
  }
  updateRulePreview();
...
export function chooseSecurityRuleType(type: string): void {
  const select = AdminDOM.getSelect('security-rule-type');
  if (select) select.value = type;
  const action = AdminDOM.getSelect('security-rule-action');
  if (action) action.value = type === 'rate_limit' ? 'rate_limit' : type === 'custom' ? 'log' : 'block';
  const expression = AdminDOM.getTextarea('security-rule-expression');
  if (expression) expression.value = defaultExpressionForType(type);
  const name = AdminDOM.getInput('security-rule-name');
  if (name && (!name.value || name.value === 'Custom rule')) name.value = defaultNameForType(type);
...
export async function validateSecurityRule(showToast = true): Promise<void> {
  const rule = readRuleFromEditor();
  const apiBase = activeEditorOptions.apiBase || 'v2/security/bot/rules';
  const response = await api.post<ValidationResponse>(`${apiBase}/validate`, { expression: rule.expression, rule });
  if (!response) return;
  renderValidation(response);
  if (showToast) notify(response.valid ? 'No issues found' : 'Rule needs changes', response.valid ? 'success' : 'error');
}

export function updateRulePreview(): void {
  const preview = AdminDOM.getById('security-rule-preview');
  if (preview) preview.textContent = AdminDOM.textareaValue('security-rule-expression');
}

export function setSecurityRuleRawMode(): void {
  builderMode = 'raw';
  updateRuleBuilderModeUI();
}

export function useExpressionEditor(): void {
  builderMode = 'raw';
  updateRuleBuilderModeUI();
}

export function useExpressionBuilder(): void {
  const type = AdminDOM.selectValue('security-rule-type', defaultRuleType);
...
export function updateRuleEditorState(): void {
  const action = AdminDOM.selectValue('security-rule-action', 'log');
  AdminDOM.queryAll('.security-rule-action-panel').forEach(panel => {
    if (panel instanceof HTMLElement) panel.classList.toggle('hidden', panel.dataset.actionPanel !== action);
  });
}

export function insertRuleField(field: string): void {
  const select = AdminDOM.query<HTMLInputElement | HTMLSelectElement>('.security-rule-condition-field');
  if (select) select.value = field;
  const display = select?.closest('.security-rule-condition-row')?.querySelector<HTMLInputElement>('.security-rule-field-search');
  if (display) display.value = fieldLabel(field);
  if (select) updateSecurityRuleConditionControl(select);
}

export function toggleSecurityRuleFieldMenu(target: Element): void {
  ...
  const input = dropdown.querySelector<HTMLInputElement>('.security-rule-field-search');
  if (input) {
...
export function openSecurityRuleFieldMenu(target: Element): void {
  const dropdown = target.closest('.security-rule-field-dropdown');
  if (!(dropdown instanceof HTMLElement)) return;
  const input = dropdown.querySelector<HTMLInputElement>('.security-rule-field-search');
  if (input) filterSecurityRuleFieldOptions(input);
  setSecurityRuleFieldMenuOpen(dropdown, true);
}

export function closeSecurityRuleFieldMenus(): void {
  AdminDOM.queryAll<HTMLElement>('.security-rule-field-dropdown.is-open').forEach(dropdown => {
    setSecurityRuleFieldMenuOpen(dropdown, false);
  });
}

export function chooseSecurityRuleField(source: HTMLElement): void {
  ...
  const input = dropdown.querySelector<HTMLInputElement>('.security-rule-field-search');
  const hidden = dropdown.querySelector<HTMLInputElement>('.security-rule-condition-field');
  if (input) input.value = fieldLabel(field);
  if (hidden) hidden.value = field;
  setSecurityRuleFieldMenuOpen(dropdown, false);
  updateSecurityRuleConditionControl(hidden || source);
}

function readRuleFromEditor(): SecurityRule {
...
function applyBotScopeToExpression(expression: string, type: string): string {
  if (type !== 'bot' || !isBotRuleEditor()) return expression;
  ...
  if (excludeGoodBots) clauses.push('aegis.bot.verified ne true');
  if (excludeAICrawlers) clauses.push('aegis.bot.category ne "ai_crawler"');
  const base = expression.trim();
  if (!clauses.length || !base) return base;
  const normalized = base.toLowerCase();
  const missingClauses = clauses.filter(clause => !normalized.includes(clause.toLowerCase()));
  if (!missingClauses.length) return base;
  return `(${base}) and ${missingClauses.join(' and ')}`;
}

function botScopeExcluded(rule: SecurityRule, scope: 'good' | 'ai'): boolean {
  const legacyValue = (rule as unknown as Record<string, unknown>)[scope === 'good' ? 'apply_to_good_bots' : 'apply_to_ai_crawlers'];
...
function renderActionParams(rule: SecurityRule): string {
...
function renderValidation(response: ValidationResponse): void {
  ...
  const preview = AdminDOM.getById('security-rule-preview');
  if (preview) preview.textContent = response.normalized_expression || AdminDOM.textareaValue('security-rule-expression');
}

function renderConditionRows(conditions: VisualCondition[], type: string): void {
  const container = AdminDOM.getById('security-rule-conditions');
  if (!container) return;
...
function renderConditionRow(condition: VisualCondition, index: number, type: string): string {
  const field = condition.field || (isBotRuleEditor() ? '' : defaultFieldForType(type));
...
function renderFieldControl(type: string, selectedField: string, _index: number): string {
  if (isBotRuleEditor()) {
...
function fieldDropdownOptions(options: FieldSpec[], selectedField: string): string {
...
function setSecurityRuleFieldMenuOpen(dropdown: Element | null, open: boolean): void {
  if (!(dropdown instanceof HTMLElement)) return;
  AdminDOM.queryAll<HTMLElement>('.security-rule-field-dropdown.is-open').forEach(item => {
    if (item !== dropdown) setSecurityRuleFieldMenuOpen(item, false);
  });
...
function filterSecurityRuleFieldOptions(input: HTMLInputElement): void {
  const dropdown = input.closest('.security-rule-field-dropdown');
  if (!(dropdown instanceof HTMLElement)) return;
  const query = input.value.trim().toLowerCase();
  let visibleCount = 0;
  dropdown.querySelectorAll<HTMLElement>('.security-rule-field-option').forEach(option => {
    const haystack = `${option.dataset.label || ''} ${option.dataset.field || ''}`.toLowerCase();
    const visible = !query || haystack.includes(query);
    option.classList.toggle('hidden', !visible);
    if (visible) visibleCount += 1;
  });
  dropdown.querySelectorAll<HTMLElement>('.security-rule-field-group').forEach(group => {
    group.classList.toggle('hidden', !group.querySelector('.security-rule-field-option:not(.hidden)'));
  });
  dropdown.querySelector<HTMLElement>('.security-rule-field-empty')?.classList.toggle('hidden', visibleCount > 0);
}

function isBotRuleEditor(): boolean {
  return (activeEditorOptions.editorClass || '').split(/\s+/).includes('security-rule-builder-bot');
}

function fieldOptions(type: string, selected?: string): string {
...
function selectableFieldOptions(type: string, selected?: string): FieldSpec[] {
  ...
  const fields = cachedFields.filter(field => {
    ...
    if (sectionAllowed || nameAllowed || field.name === selectedField) return true;
...
function dedupeFields(fields: FieldSpec[]): FieldSpec[] {
...
function operatorOptions(field: string, selected?: string): string {
  if (!field) return '<option value="" selected disabled>Select field first</option>';
...
function renderValueControl(field: string, rawValue: string, index: string, selectedOperator?: string): string {
  if (!field) return '<input class="security-rule-condition-value" value="" placeholder="Select a field first" disabled>';
  const spec = fieldSpec(field);
  const inputType = spec?.input_type || spec?.type || 'text';
  const operator = selectedOperator || AdminDOM.query<HTMLSelectElement>(`[data-condition-index="${CSS.escape(index)}"] .security-rule-condition-operator`)?.value || defaultOperatorForField(field);
...
function readSingleConditionValue(row: HTMLElement): string {
  const valueControl = row.querySelector<HTMLInputElement | HTMLSelectElement>('.security-rule-condition-value');
...
function readVisualConditions(): VisualCondition[] {
  return AdminDOM.queryAll<HTMLElement>('.security-rule-condition-row').map((row, index) => {
    const field = row.querySelector<HTMLInputElement | HTMLSelectElement>('.security-rule-condition-field')?.value || (isBotRuleEditor() ? '' : defaultFieldForType(AdminDOM.selectValue('security-rule-type', defaultRuleType)));
    const operator = row.querySelector<HTMLSelectElement>('.security-rule-condition-operator')?.value || (field ? defaultOperatorForField(field) : '');
    const valueControl = row.querySelector<HTMLInputElement | HTMLSelectElement>('.security-rule-condition-value');
    let value = '';
    if (valueControl instanceof HTMLSelectElement && valueControl.multiple) {
    ...
    return {
      joiner: index === 0 ? 'and' : (row.querySelector<HTMLSelectElement>('.security-rule-condition-join')?.value === 'or' ? 'or' : 'and'),
      field,
      operator,
      value
    };
...
function syncExpressionFromConditions(showToast: boolean): void {
  ...
  if (invalid.length) {
    ...
    if (showToast) notify(invalid[0], 'error');
    return;
  }
...
function syncExpressionPreviewFromConditions(): void {
  const textarea = AdminDOM.getTextarea('security-rule-expression');
  if (!textarea) return;
...
function clearRuleValidation(): void {
  const target = AdminDOM.getById('security-rule-validation');
  if (!target) return;
  target.classList.add('hidden');
  target.innerHTML = '';
...
function conditionsToExpression(conditions: VisualCondition[]): string {
  return conditions
...
function expressionToConditions(expression: string, type: string): VisualCondition[] {
...
function readActionParams(): Record<string, string> | undefined {
  const action = AdminDOM.selectValue('security-rule-action', 'log');
  ...
  if (action === 'redirect') return { location: AdminDOM.inputValue('security-rule-param-location', '/') };
  return undefined;
}

function updateRuleBuilderModeUI(): void {
  const builder = AdminDOM.getById('security-rule-builder-panel');
  const editor = AdminDOM.getById('security-rule-editor-panel');
  if (builder) builder.classList.toggle('hidden', builderMode === 'raw');
  if (editor) editor.classList.toggle('hidden', builderMode !== 'raw');
}

function defaultExpressionForType(type: string): string {
...
function defaultFieldForType(type: string): string {
  const defaults: Record<string, string> = {
...
function defaultOperatorForField(field: string): string {
  const spec = fieldSpec(field);
  if (spec?.input_type === 'number' || spec?.type === 'number') return spec.operators.includes('lt') ? 'lt' : 'eq';
  if (field === 'ip.geoip.country' || field === 'ip.src' || field === 'tls.version' || field === 'http.request.method') return 'in';
  if (field.includes('path')) return 'starts_with';
  return spec?.operators?.includes('eq') ? 'eq' : spec?.operators?.[0] || 'contains';
}

function defaultValueForField(field: string, type: string): string {
  const defaults: Record<string, string> = {
...
function defaultPhaseForType(type: string): string {
  ...
  if (type === 'http_security') return 'http_security';
  if (type === 'geo' || type === 'ip_access' || type === 'access') return 'access';
  return 'http_request_firewall_custom';
}

function sectionForType(type: string): string {
  if (type === 'geo' || type === 'ip_access') return 'geo_ip';
  return type;
}

function formatExpressionValue(value: string, operator: string): string {
...
function unformatExpressionValue(value: string): string {
  return value
    .replace(/[{}]/g, '')
    .split(/\s+/)
...
function clientConditionErrors(conditions: VisualCondition[]): string[] {
  const errors: string[] = [];
  for (const condition of conditions) {
    if (!condition.field && !condition.operator && !condition.value.trim()) continue;
    if (!condition.field) {
    ...
    if (condition.field === 'ip.src') {
      for (const value of values) {
        if (!isLikelyIpOrCidr(value)) errors.push(`Invalid IP/CIDR value: ${value}`);
      }
    }
    if (condition.field === 'ip.geoip.country') {
      for (const value of values) {
        if (!/^[A-Za-z]{2}$/.test(value)) errors.push(`Invalid country code: ${value}`);
      }
    }
  }
  return errors;
...
function isLikelyIpOrCidr(value: string): boolean {
  const ipPart = value.split('/')[0];
  const octets = ipPart.split('.');
  if (octets.length !== 4 || !octets.every(octet => /^\d{1,3}$/.test(octet) && Number(octet) >= 0 && Number(octet) <= 255)) return false;
  if (!value.includes('/')) return true;
  const mask = Number(value.split('/')[1]);
  return Number.isInteger(mask) && mask >= 0 && mask <= 32;
}

function renderClientValidation(errors: string[]): void {
  const target = AdminDOM.getById('security-rule-validation');
  if (!target) return;
  target.classList.remove('hidden');
...
function reindexConditionRows(): void {
  AdminDOM.queryAll<HTMLElement>('.security-rule-condition-row').forEach((row, index) => {
    row.dataset.conditionIndex = String(index);
    const remove = row.querySelector<HTMLElement>('[data-action="remove-security-rule-condition"]');
    if (remove) remove.dataset.conditionIndex = String(index);
    const joiner = row.querySelector<HTMLElement>('.security-rule-condition-joiner');
    if (joiner && index === 0) joiner.innerHTML = '<span class="status-pill">When</span>';
    if (joiner && index > 0 && !joiner.querySelector('select')) joiner.innerHTML = '<select class="select-sm security-rule-condition-join"><option value="and">AND</option><option value="or">OR</option></select>';
  });
}
