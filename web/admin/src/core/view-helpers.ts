...
export function setText(elementId: string, text: string | number): void {
...
export function formatNumber(value: number | string | null | undefined): string {
  ...
  if (numeric >= 1_000_000_000) {
    const formatted = numeric / 1_000_000_000;
    return formatted % 1 === 0 ? `${formatted}B` : `${formatted.toFixed(1)}B`;
  }
  if (numeric >= 1_000_000) {
    const formatted = numeric / 1_000_000;
    return formatted % 1 === 0 ? `${formatted}M` : `${formatted.toFixed(1)}M`;
  }
  if (numeric >= 1_000) {
    const formatted = numeric / 1_000;
    return formatted % 1 === 0 ? `${formatted}K` : `${formatted.toFixed(1)}K`;
  }
  return Math.round(numeric).toString();
}

export function dashboardEmptyRow(columns: number, title: string, detail = ''): string {
