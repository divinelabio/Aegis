...
export interface EChartTimelinePoint {
  bucket: string;
  label: string;
  total: number;
  mitigated?: number;
...
export interface EChartActionMixItem {
  key: string;
  label: string;
  count: number;
}

export interface EChartDecisionTimelinePoint {
  bucket: string;
  label: string;
...
interface EChartElement extends HTMLElement {
  _aegisEChart?: EChartsInstanceLike;
}
...
const chartRegistry = new Set<EChartElement>();
...
export function renderEventTimelineChart(
  elementId: string,
  points: EChartTimelinePoint[],
  options: { onBucketClick?: (bucket: string) => void } = {}
): void {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      valueFormatter: (value: unknown) => formatChartNumber(Number(value || 0))
    },
    legend: {
      top: 0,
      right: 0,
  ...
  if (options.onBucketClick) {
    chart.on('click', params => {
      ...
      if (bucket) options.onBucketClick?.(bucket);
    });
  }
}

export function disposeEChart(elementId: string): void {
  const element = chartElementById(elementId);
  if (!element?._aegisEChart) return;
...
export function renderActionMixStackedBarChart(
  elementId: string,
  items: EChartActionMixItem[],
  options: { onActionClick?: (action: string) => void } = {}
): void {
  const element = chartElementById(elementId);
  if (!element) return;
  ...
  if (options.onActionClick) {
    chart.on('click', params => {
      ...
      if (key) options.onActionClick?.(key);
    });
  }
}

function createEChart(element: EChartElement): EChartsInstanceLike | null {
...
function timelineSeries(name: string, data: number[], color: string): EChartsOption {
  return {
    name,
    type: 'line',
    stack: 'events',
...
function chartElementById(id: string): EChartElement | null {
  const element = AdminDOM.getById(id);
  return element instanceof HTMLElement ? element as EChartElement : null;
}

function readChartTheme(): Record<string, string> {
...
function renderChartFallback(element: HTMLElement, points: EChartTimelinePoint[]): void {
...
function renderChartMessage(element: HTMLElement, message: string): void {
