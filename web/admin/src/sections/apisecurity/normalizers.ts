import type { APISecurityState } from './types.js';

export function cloneAPISecurityValue<T>(value: T): T {
    if (value === undefined) return value;
    return JSON.parse(JSON.stringify(value)) as T;
}


export function normalizeAPISecurityMode(value: unknown, fallback: 'detect' | 'block' = 'detect'): 'detect' | 'block' {
    return String(value || '').trim().toLowerCase() === 'block' ? 'block' : fallback;
}


export function normalizeAPISecurityConfigShape(input: APISecurityState | null | undefined): APISecurityState {
...
function removeAPISecurityLegacyIdentitySettings(value: unknown, keys: string[]): void {
