...
import { renderCustomRulesList, renderRuleEditor } from './waf/custom.js';
import { renderExclusionsTab, isExclusionExpired } from './waf/exclusions.js';

type WAFTabId = 'overview' | 'leak_protection' | 'managed' | 'custom' | 'exclusions';
type WAFRulesView = 'main' | 'managed' | 'editor';
type WAFExclusionScope = 'all' | 'global' | 'rule' | 'active' | 'inactive';
type WAFManagedRuleStatusFilter = 'all' | 'enabled' | 'disabled' | 'warning' | 'error';
type WAFLeakProtectionPage = '' | 'advanced' | 'credentials' | 'debug' | 'files' | 'custom_detectors' | 'allowlist';
...
interface WAFAttackStat {
    type: string;
    count: number;
    [key: string]: unknown;
}

interface WAFConfigHistoryPoint {
    timestamp?: string;
...
interface WAFConfigDashboardStats {
    total_requests?: number;
    blocked_requests?: number;
...
interface WAFCoreConfig {
    mode?: 'off' | 'detection' | 'blocking' | string;
	failure_mode?: 'fail_open' | 'fail_closed' | 'service_unavailable' | string;
    active_profile_id?: string;
    active_policy_id?: string;
    rule_strictness?: number;
    anomaly_threshold?: number;
    response_inspection?: boolean;
...
interface WAFLeakProtectionConfig {
    enabled?: boolean;
    mode?: 'allow' | 'detect' | 'redact' | 'block' | string;
    max_body_size?: number;
    inspect_content_types?: string[];
    skip_content_types?: string[];
    redaction_enabled?: boolean;
    default_action?: 'allow' | 'detect' | 'redact' | 'block' | string;
    block_status_code?: number;
    replacement_body?: string;
    rules?: Record<string, string>;
    custom_patterns?: WAFLeakCustomPattern[];
    allowlist?: WAFLeakAllowlistEntry[];
    [key: string]: unknown;
}

interface WAFLeakCustomPattern {
    id: string;
    name: string;
    pattern: string;
    category?: string;
    severity?: string;
    action?: 'allow' | 'detect' | 'redact' | 'block' | string;
    enabled?: boolean;
    confidence?: number;
    [key: string]: unknown;
}

interface WAFLeakAllowlistEntry {
    id: string;
    enabled?: boolean;
    category?: string;
    host?: string;
...
interface WAFRulesResponse {
    rules?: WAFCustomRule[];
    files?: Array<string | WAFManagedRuleFile>;
    [key: string]: unknown;
}
...
interface WAFExclusionsResponse {
    exclusions?: WAFExclusion[];
    exclusion?: WAFExclusion;
    [key: string]: unknown;
}

interface WAFConfigDashboardResponse {
    dashboard?: WAFConfigDashboardStats;
    [key: string]: unknown;
}

interface WAFConfigResponse {
    config?: WAFCoreConfig;
    health?: WAFHealthResponse;
    [key: string]: unknown;
}

interface WAFHealthResponse {
    engine_initialized?: boolean;
    enabled?: boolean;
    mode?: string;
    rules?: {
        crs_files_loaded?: number;
		crs_rules_loaded?: number;
		crs_directives_loaded?: number;
...
interface WAFRuleDetailResponse {
    rule?: WAFCustomRule;
    valid?: boolean;
    validation_errors?: string[];
}

interface WAFSectionApiErrorResponse {
    error?: string;
    message?: string;
    detail?: string;
    [key: string]: unknown;
...
function wafCfgErrorMessage(error: unknown, fallback: string): string {
    return error instanceof Error ? error.message : fallback;
}

const defaultWafLeakProtectionConfig: Required<Pick<WAFLeakProtectionConfig,
    'enabled' | 'mode' | 'max_body_size' | 'inspect_content_types' | 'skip_content_types' |
    'redaction_enabled' | 'default_action' | 'block_status_code' | 'replacement_body' | 'rules' |
    'custom_patterns' | 'allowlist'
>> = {
    enabled: true,
    mode: 'detect',
...
function getWafLeakProtectionConfig(config: WAFCoreConfig): WAFLeakProtectionConfig {
...
function formatWafBytes(value: unknown): string {
    const bytes = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(bytes) || bytes <= 0) return 'Not configured';
...
function formatWafMebibytes(value: unknown): string {
    const bytes = typeof value === 'number' ? value : Number(value);
...
async function wafCfgParseJson<T>(response: Response): Promise<T | null> {
    try {
...
function wafCfgApiErrorMessage(data: WAFSectionApiErrorResponse | null, fallback: string): string {
...
function wafCfgGetElementById<T extends HTMLElement = HTMLElement>(id: string): T | null {
    const element = globalThis['document'].getElementById(id);
    return element instanceof HTMLElement ? (element as T) : null;
}

function wafCfgGetControlById(id: string): HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null {
...
function wafCfgGetControlValue(id: string, fallback = ''): string {
    return wafCfgGetControlById(id)?.value ?? fallback;
}

function wafCfgGetCheckboxValue(id: string): boolean {
    const checkbox = wafCfgGetElementById<HTMLInputElement>(id);
...
function wafCanWrite(): boolean {
    return api.hasPermission('waf:write');
}

function applyWafPermissionState(root: HTMLElement): void {
    if (wafCanWrite()) {
    ...
    const controls = root.querySelectorAll<HTMLElement>(
        '[data-waf-field], [data-waf-mode-toggle="true"], [data-waf-action], [data-waf-mime-input]'
    );
...
function parseWafDatasetValue(target: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): unknown {
...
function ensureWafBindings(): void {
    if (wafBindingsInitialized) return;
    wafBindingsInitialized = true;

    document.addEventListener('click', (event) => {
        if (!(event.target instanceof Element)) return;
        const fieldTarget = event.target.closest<HTMLElement>('[data-waf-field]');
        if (
            fieldTarget &&
        ...
        const target = event.target.closest<HTMLElement>('[data-waf-action]');
        if (!target) return;
        ...
        switch (action) {
            ...
            case 'open-crs-editor':
                if (value) void WAFConfig.openCRSEditor(value);
                break;
            ...
            case 'close-modal-by-id':
                if (value) wafCfgGetElementById(value)?.remove();
                break;
            case 'close-self-if-overlay':
                if (event.target === target) target.remove();
                break;
    ...
    document.addEventListener('change', (event) => {
        ...
        if (target.dataset.wafState) {
            (WAFConfig as unknown as Record<string, unknown>)[target.dataset.wafState] = target.value;
            WAFConfig.render();
            return;
        }
        ...
        if (target.classList.contains('section-condition-transform') && target instanceof HTMLInputElement) {
            if (target.parentElement) target.parentElement.classList.toggle('active', target.checked);
            WAFConfig.updatePreview();
        }
    });
    ...
    document.addEventListener('keydown', (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement) || target.dataset.wafMimeInput !== 'true' || event.key !== 'Enter') return;
        event.preventDefault();
        if (!wafCanWrite()) {
            showSectionToast('WAF configuration is read-only for your role', 'warning');
            return;
    ...
    document.addEventListener('input', (event) => {
        ...
        if (stateKey) {
            (WAFConfig as unknown as Record<string, unknown>)[stateKey] = target.value;
            WAFConfig.render();
            return;
        }
...
type WAFConfigRuntime = {
    ...
    crsGroups: Record<string, WAFManagedRuleFile[]>;
    profiles: WAFProfile[];
    exclusions: WAFExclusion[];
    exclusionQuery: string;
    exclusionScope: WAFExclusionScope;
    managedRuleQuery: string;
    managedRuleStatusFilter: WAFManagedRuleStatusFilter;
    isLoading: boolean;
    loadError: string | null;
    rulesView: WAFRulesView;
    editingRule: WAFCustomRule | null;
    refreshInterval?: number;
    statsAbortController?: AbortController;
    statsRequest?: Promise<void>;
    visibilityHandler?: () => void;
    tabs: WAFTab[];
    getIcon(name: string): string;
    renderSectionHeader(icon: string, title: string, desc: string): string;
    renderWafModuleStatusCard(title: string, description: string, icon: string, enabled: boolean, actions?: string, statusLabel?: string): string;
    init(): Promise<void>;
    startRefresh(): void;
    loadConfig(): Promise<void>;
    loadStats(silent?: boolean): Promise<void>;
    loadRules(): Promise<void>;
    loadProfiles(): Promise<void>;
    loadExclusions(): Promise<void>;
    render(): void;
    switchTab(id: WAFTabId): void;
    toggleAdvancedSettings(): void;
    renderTabContent(): string;
    renderOverview(): string;
    renderLeakProtection(): string;
    renderManagedRules(): string;
    renderReloadHealth(): string;
    renderOverviewStat(label: string, value: number | string, color: string): string;
    getParanoiaDescription(level: number): string;
    renderTrafficChart(history: WAFConfigHistoryPoint[] | undefined): string;
    getAttackColor(type: string): string;
    renderFeatureCard(tabKey: string, title: string, subtitle: string, iconKey: string, isActive: boolean): string;
    updateDashboardUI(): void;
    renderModeOption(value: string, title: string, subtitle: string, iconName: string): string;
    renderParanoiaCard(level: number, title: string, tone: string, subtitle: string, isRecommended?: boolean): string;
    getAttackCount(type: string): number;
    renderRules(): string;
    renderRulesMain(): string;
    renderManagedRulesFolder(): string;
    formatManagedRuleName(fileName: string): string;
    managedRuleDescription(file: WAFManagedRuleFile): string;
    managedRuleStatus(file: WAFManagedRuleFile): { label: string; tone: string };
    managedRulePolicyGroup(file: WAFManagedRuleFile): { key: string; label: string; description: string; tone: string };
    managedPolicyGroupStatus(files: WAFManagedRuleFile[]): { label: string; tone: string };
    renderManagedPolicyGroup(group: { key: string; label: string; description: string; tone: string; files: WAFManagedRuleFile[] }, index: number): string;
    renderManagedPolicyRuleRow(file: WAFManagedRuleFile): string;
    renderCustomRulesList(): string;
    openCRSEditor(filename: string): Promise<void>;
    bulkToggleCRS(enable: boolean): Promise<void>;
    openRuleEditor(ruleId?: string): Promise<void>;
    renderRuleEditor(): string;
    renderVisualBuilder(rule: WAFCustomRule | null): string;
    renderEmptyConditionsState(): string;
    addRuleCondition(zone?: string, field?: string, pattern?: string, negate?: boolean, operator?: string, transforms?: string[]): void;
    renderConditionRow(zone?: string, field?: string, pattern?: string, negate?: boolean, operator?: string, transforms?: string[]): string;
    populateVisualRuleConditions(rule?: WAFCustomRule | null): void;
    renderConditionRowsFromTargets(targets?: WAFRuleTarget[]): string;
    toggleConditionField(element: Element): void;
    updatePreview(): void;
    submitCustomRule(isEdit?: boolean): Promise<void>;
    processCRSRules(files: WAFManagedRuleFile[]): Record<string, WAFManagedRuleFile[]>;
    toggleCustomRule(id: string, enabled: boolean): Promise<void>;
    toggleCRSFile(filename: string, enabled: boolean): Promise<void>;
    deleteRule(id: string): Promise<void>;
    renderExclusions(): string;
    showExclusionModal(): void;
    addExclusion(): Promise<void>;
    deleteExclusion(id: string): Promise<void>;
    toggleExclusionEnabled(id: string, enabled: boolean): Promise<void>;
    matchesExclusionFilter(ex: WAFExclusion): boolean;
    isExclusionExpired(ex: WAFExclusion): boolean;
    showCRSEditorModal(): void;
    renderConditionRowFromTarget(target: WAFRuleTarget): string;
    getFilteredExclusions(exclusions: WAFExclusion[]): WAFExclusion[];
    countExpiredExclusions(exclusions: WAFExclusion[]): number;
    getExclusionsWithoutId(id: string): WAFExclusion[];
    loadRulesAndRender(): Promise<void>;
    renderProtectionProfiles(): string;
    applyProfile(id: string): void;
    resolveProfileNumber(profile: WAFProfile, keys: string[], fallback: number): number;
    applyProfileDefaults(profile: WAFProfile): void;
    stageLeakConfig(config: WAFLeakProtectionConfig): void;
    addLeakPattern(): void;
    removeLeakPattern(id: string): void;
    addLeakAllowlist(): void;
    removeLeakAllowlist(id: string): void;
    addLeakMime(scope: string): void;
    removeLeakMime(scope: string, mimeType: string): void;
    toggleLeakMimeGroup(groupId: string, enabled: boolean): void;
    updateField(field: string, value: unknown): void;
    saveConfig(): Promise<void>;
    lastEnabledMode: 'detection' | 'blocking';
};

const WAFConfig: WAFConfigRuntime = {
    currentTab: 'overview',
    leakProtectionPage: '',
    config: {} as WAFCoreConfig,
    health: {} as WAFHealthResponse,
    stats: {} as WAFConfigDashboardStats,
    rules: [], // Custom rules
    ...
    get tabs(): WAFTab[] {
        ...
        const tabs: WAFTab[] = [
            ...
            ...(hasDLP ? [{ id: 'leak_protection', name: 'Leak Protection', icon: 'eye' } as WAFTab] : []),
    ...
    renderSectionHeader(_icon: string, _title: string, _desc: string) {
        return '';
    },

    renderWafModuleStatusCard(_title: string, _description: string, _icon: string, _enabled: boolean, _actions = '', _statusLabel?: string) {
        return '';
    },
    ...
    startRefresh() {
        ...
        this.refreshInterval = window.setInterval(refreshIfActive, 30_000);
    },

    // Data Loading
    async loadConfig() {
        try {
            const data = await api.get<WAFConfigResponse>('sections/waf_core/config');
    ...
    async loadStats(silent = false) {
        ...
        const request = (async (): Promise<void> => {
            try {
                const data = await api.get<WAFConfigDashboardResponse>(
                    'sections/waf_core/dashboard?window=24h',
                    { signal: controller.signal }
                );
                if (controller.signal.aborted || !data) return;
                this.stats = data.dashboard || {};
                if (!silent) this.render();
                else this.updateDashboardUI();
            } catch (e) {
                if (!silent && !controller.signal.aborted) console.error('Failed to load stats', e);
            }
        })();
        ...
        try {
            ...
            if (this.statsRequest === request) this.statsRequest = undefined;
            if (this.statsAbortController === controller) this.statsAbortController = undefined;
        }
    },
    ...
    async loadRules() {
        try {
            const [customData, crsData] = await Promise.all([
                api.get<WAFRulesResponse>('sections/waf_core/rules/custom'),
                api.get<WAFRulesResponse>('sections/waf_core/rules/crs')
            ]);

            this.rules = customData?.rules || [];
            // Backend now returns objects {name, enabled}, but legacy might return strings
            this.crsRules = (crsData?.files || []).map((f: string | WAFManagedRuleFile) => {
    ...
    async loadProfiles() {
        try {
            const data = await api.get<WAFProfilesResponse>('sections/waf_core/profiles');
            this.profiles = data?.profiles || [];
        } catch (e) {
            try {
                const data = await api.get<WAFProfilesResponse>('sections/waf_core/policies');
    ...
    async loadExclusions() {
        try {
            const data = await api.get<WAFExclusionsResponse>('sections/waf_core/exclusions');
    ...
    render() {
        ...
        const consoleMetaMap: Record<WAFTabId, WAFConsoleMeta> = {
            overview: {
                title: 'Overview',
    ...
    switchTab(id) {
        ...
        this.currentTab = id;
        if (id !== 'leak_protection') this.leakProtectionPage = '';
        if (id === 'custom' && this.rulesView === 'managed') this.rulesView = 'main';
        this.render();
        if (id === 'overview') void this.loadStats(true);
        ...
        if (target) {
            AdminDOM.queryAll<HTMLElement>('.nav-link').forEach((el: HTMLElement) => el.classList.remove('active'));
            const link = AdminDOM.query<HTMLElement>(`.nav-link[data-nav-target="${target}"]`);
    ...
    renderOverview() {
        return renderOverviewTab(
            ...
            this.stats,
            this.health,
            (e) => this.isExclusionExpired(e),
            (name) => this.getIcon(name)
        );
    },
    ...
    renderLeakProtection() {
        return renderLeakProtectionTab(
    ...
    renderTrafficChart(history) {
        ...
        const getY = (val: number) => height - padding - (val / maxReq) * (height - 2 * padding);
    ...
    getAttackCount(type) {
        if (!this.stats.attack_types) return 0;
    ...
    updateDashboardUI() {
        const container = wafCfgGetElementById('waf-config-content');
        if (!container || this.currentTab !== 'overview') return;
        ...
        const update = (key: string, value: string, note?: string): void => {
            const valueNode = container.querySelector<HTMLElement>(`[data-waf-kpi-value="${key}"]`);
            if (valueNode) valueNode.textContent = value;
            const noteNode = container.querySelector<HTMLElement>(`[data-waf-kpi-note="${key}"]`);
            if (!noteNode) return;
            if (note !== undefined) noteNode.textContent = note;
        };
    ...
    formatManagedRuleName(fileName: string) {
        ...
        const normalized = String(fileName || '')
            ...
            .trim()
            .toLowerCase();
        if (!normalized) return 'Managed Protection';
        return normalized
            .split(/\s+/)
    ...
    managedRuleDescription(file: WAFManagedRuleFile) {
        ...
        if (category === 'response') return `${friendlyName} checks responses for sensitive data and information leaks.`;
        if (category === 'request') return `${friendlyName} checks incoming requests using ${groupText}.`;
        return `${friendlyName} provides built-in protection for your application.`;
    },

    managedRuleStatus(file: WAFManagedRuleFile) {
        if (file.enabled === false) return { label: 'Disabled', tone: 'muted' };
    ...
    managedRulePolicyGroup(file: WAFManagedRuleFile) {
    ...
    managedPolicyGroupStatus(files: WAFManagedRuleFile[]) {
        if (files.some(file => String(file.status || '').toLowerCase() === 'error')) return { label: 'Error', tone: 'danger' };
        if (files.some(file => String(file.status || '').toLowerCase() === 'warning')) return { label: 'Needs review', tone: 'warning' };
        if (files.every(file => file.enabled === false)) return { label: 'Disabled', tone: 'muted' };
        if (files.some(file => file.enabled === false)) return { label: 'Partially enabled', tone: 'warning' };
        return { label: 'Enabled', tone: 'success' };
    },

    renderManagedPolicyRuleRow(file: WAFManagedRuleFile) {
    ...
    renderManagedPolicyGroup(group: { key: string; label: string; description: string; tone: string; files: WAFManagedRuleFile[] }, _index: number) {
        return `
    ...
    renderManagedRulesFolder() {
        ...
        const policyGroups = Array.from(files.reduce((groups, file) => {
            ...
            return groups;
        }, new Map<string, { key: string; label: string; description: string; tone: string; files: WAFManagedRuleFile[] }>()).values())
    ...
    async openCRSEditor(filename) {
        ...
        try {
            ...
            const activeTextarea = wafCfgGetElementById<HTMLTextAreaElement>('crs-editor-textarea');
            if (activeTextarea) {
                if (resp.ok) {
                    activeTextarea.value = await resp.text();
                } else {
                    const data = await wafCfgParseJson<WAFSectionApiErrorResponse>(resp);
    ...
    async bulkToggleCRS(enable) {
        ...
        const result = await api.requestResult<{ changed?: string[]; skipped?: string[] }>(
            'sections/waf_core/rules/crs/bulk',
            'POST',
            { action: enable ? 'enable' : 'disable', files }
        );
        if (result.error) {
            const details = Array.isArray(result.error.details)
                ? result.error.details
                    .map(item => {
                        const failure = item as { file?: string; error?: string };
        ...
        this.crsRules.forEach(file => {
            if (changed.has(file.name) || skipped.has(file.name)) file.enabled = enable;
        });
        this.crsGroups = this.processCRSRules(this.crsRules);
    ...
    async openRuleEditor(ruleId) {
        if (ruleId) {
            const data = await api.get<WAFRuleDetailResponse>('sections/waf_core/rules/custom/' + encodeURIComponent(ruleId));
            if (!data?.rule) {
    ...
    addRuleCondition(zone = 'headers', field = '', pattern = '', negate = false, operator = '@rx', transforms: string[] = []) {
        const container = wafCfgGetElementById('cr-conditions');
        if (!container) return;
        ...
        const row = temp.firstElementChild as HTMLElement | null;
        if (!row) return;

        container.appendChild(row);
        const operatorField = row.querySelector('.section-condition-operator');
        if (operatorField) this.toggleConditionField(operatorField); // Initialize field visibility
        this.updatePreview();
    },
    ...
    populateVisualRuleConditions(rule = null) {
        if (rule && rule.targets && rule.targets.length > 0) {
            rule.targets.forEach((target) => {
                if (!target.patterns || target.patterns.length === 0) return;
    ...
    toggleConditionField(element) {
        ...
        if (!row) return;
        const zoneSelect = row.querySelector('.section-condition-zone') as HTMLSelectElement | null;
        const operatorSelect = row.querySelector('.section-condition-operator') as HTMLSelectElement | null;
        const fieldInput = row.querySelector('.section-condition-field') as HTMLInputElement | null;
        const patternInput = row.querySelector('.section-condition-pattern') as HTMLInputElement | null;
        ...
        if (!zoneSelect || !operatorSelect) return;

        const needsField = ['headers', 'args', 'cookies'].includes(zoneSelect.value);
        if (fieldWrap) fieldWrap.classList.toggle('is-hidden', !needsField);
    ...
    updatePreview() {
        ...
        const conditions: WAFPreviewCondition[] = [];
        document.querySelectorAll<HTMLElement>('#cr-conditions .section-condition-row').forEach((row) => {
            const zone = (row.querySelector('.section-condition-zone') as HTMLSelectElement | null)?.value || '';
            const op = (row.querySelector('.section-condition-operator') as HTMLSelectElement | null)?.value || '@rx';
            const field = (row.querySelector('.section-condition-field') as HTMLInputElement | null)?.value || '';
            const pattern = (row.querySelector('.section-condition-pattern') as HTMLInputElement | null)?.value || '';
            const negate = (row.querySelector('.section-condition-negate') as HTMLInputElement | null)?.checked || false;
            const transforms = Array.from(row.querySelectorAll<HTMLInputElement>('.section-condition-transform:checked')).map((cb) => cb.value);

            let zoneName = zone.toUpperCase();
            if (zoneName === 'HEADERS') zoneName = 'REQUEST_HEADERS';
            if (zoneName === 'BODY') zoneName = 'REQUEST_BODY';
            if (zoneName === 'URI') zoneName = 'REQUEST_URI';
            if (zoneName === 'PATH') zoneName = 'URL_PATH';
            if (zoneName === 'METHOD') zoneName = 'REQUEST_METHOD';
            if (zoneName === 'CLIENT_IP') zoneName = 'REMOTE_ADDR';
            if (field) zoneName += ':' + field;
    ...
    async submitCustomRule(isEdit = false) {
        ...
        const targets: WAFRuleTarget[] = [];
        document.querySelectorAll<HTMLElement>('#cr-conditions .section-condition-row').forEach((row) => {
            const zone = (row.querySelector('.section-condition-zone') as HTMLSelectElement | null)?.value || '';
            const operator = (row.querySelector('.section-condition-operator') as HTMLSelectElement | null)?.value || '@rx';
            const field = (row.querySelector('.section-condition-field') as HTMLInputElement | null)?.value.trim() || '';
            const pattern = (row.querySelector('.section-condition-pattern') as HTMLInputElement | null)?.value.trim() || '';
            const negate = (row.querySelector('.section-condition-negate') as HTMLInputElement | null)?.checked || false;
            const transforms = Array.from(row.querySelectorAll<HTMLInputElement>('.section-condition-transform:checked')).map((o) => o.value);
        ...
        const result = await api.requestResult(
            'sections/waf_core/rules/custom' + (isEdit ? '/' + encodeURIComponent(id) : ''),
            isEdit ? 'PUT' : 'POST',
            payload
        );
        if (result.error) {
    ...
    processCRSRules(files) {
        const groups: Record<string, WAFManagedRuleFile[]> = {};
        files.forEach(f => {
    ...
    async toggleCustomRule(id, enabled) {
        ...
        const result = await api.requestResult<WAFRuleDetailResponse>(
            'sections/waf_core/rules/custom/' + encodeURIComponent(id) + '/enabled',
            'PATCH',
            { enabled }
        );
        if (result.error) {
        ...
        if (result.data?.rule) Object.assign(rule, result.data.rule);
    ...
    async toggleCRSFile(filename, enabled) {
        ...
        const result = await api.requestResult(
            'sections/waf_core/rules/crs?file=' + encodeURIComponent(filename) + '&action=' + action,
            'POST'
        );
        if (result.error) {
    ...
    renderExclusions() {
        ...
        const tableRows = filtered.map(ex => {
            const scopeParts = [];
            if (ex.paths && ex.paths.length) scopeParts.push(...ex.paths.map(p => `Path: ${p}`));
            if (ex.params && ex.params.length) scopeParts.push(...ex.params.map(p => `Param: ${p}`));
            if (ex.ips && ex.ips.length) scopeParts.push(...ex.ips.map(ip => `IP: ${ip}`));
        ...
        return `
            ${this.renderWafModuleStatusCard(
                'Rule Exclusions',
    ...
    async addExclusion() {
        ...
        const exclusion: WAFExclusionPayload = {
            reason: reason || '',
            enabled
        };
        if (ruleId) exclusion.rule_id = ruleId;
        if (paths.length) exclusion.paths = paths;
        if (params.length) exclusion.params = params;
        if (ips.length) exclusion.ips = ips;
        if (expiresRaw) exclusion.expires_at = new Date(expiresRaw).toISOString();

        try {
            const data = await api.post<WAFExclusionsResponse>('sections/waf_core/exclusions', exclusion);
            if (!data) throw new Error('The exclusion was not saved');
            const createdExclusion = data && typeof data === 'object' && 'exclusion' in data
                ? data.exclusion
                : undefined;
            if (createdExclusion) this.exclusions.push(createdExclusion);
            else await this.loadExclusions();
            const modalContainer = wafCfgGetElementById('exclusion-modal-container');
            if (modalContainer) modalContainer.innerHTML = '';
    ...
    async toggleExclusionEnabled(id, enabled) {
        ...
        const result = await api.requestResult<WAFExclusionsResponse>(
            'sections/waf_core/exclusions/' + encodeURIComponent(id),
            'PUT',
            payload
        );
        if (result.error) {
    ...
    matchesExclusionFilter(ex) {
        ...
        if (scope === 'rule' && isGlobal) return false;
        if (scope === 'active' && !enabled) return false;
        if (scope === 'inactive' && enabled) return false;
    ...
    renderConditionRowFromTarget(target) {
        return this.renderConditionRow(
            target.zone,
            target.field,
            target.patterns ? target.patterns[0] : '',
            target.negate,
            target.operator,
            target.transforms
        );
    ...
    countExpiredExclusions(exclusions) {
        let count = 0;
        for (const exclusion of exclusions) {
            if (this.isExclusionExpired(exclusion)) count++;
        }
        return count;
    },
    ...
    getExclusionsWithoutId(id) {
        ...
        for (const exclusion of this.exclusions) {
            if (exclusion.id !== id) exclusions.push(exclusion);
        }
        return exclusions;
    },

    async loadRulesAndRender(): Promise<void> {
        await this.loadRules();
        this.render();
    },
    ...
    renderProtectionProfiles() {
        ...
        return this.profiles.map(p => {
            ...
            if ((p.paranoia_level || 1) >= 4) levelTone = 'danger';
            else if ((p.paranoia_level || 1) >= 3) levelTone = 'warning';
    ...
    addLeakMime(scope) {
        if (scope !== 'inspect' && scope !== 'skip') return;
    ...
    removeLeakMime(scope, mimeType) {
        if ((scope !== 'inspect' && scope !== 'skip') || !mimeType) return;
        const leak = getWafLeakProtectionConfig(this.config);
        const field = scope === 'inspect' ? 'inspect_content_types' : 'skip_content_types';
    ...
    applyProfileDefaults(profile) {
        ...
        this.config.rule_strictness = this.resolveProfileNumber(
            profile,
            ['paranoia_level', 'rule_strictness', 'strictness', 'paranoiaLevel'],
            this.config.rule_strictness || 1
        );
        this.config.anomaly_threshold = this.resolveProfileNumber(
            profile,
            ['anomaly_threshold', 'threshold', 'anomalyThreshold', 'score_threshold'],
            this.config.anomaly_threshold || 5
        );
...
const wafPopulateVisualRuleConditions = (): void => {
...
const WAFConfigState = {
    get currentTab(): WAFTabId { return WAFConfig.currentTab; },
    set currentTab(value: WAFTabId) { WAFConfig.currentTab = value; },
    get config(): WAFCoreConfig { return WAFConfig.config; },
    set config(value: WAFCoreConfig) { WAFConfig.config = value; },
    get health(): WAFHealthResponse { return WAFConfig.health; },
    set health(value: WAFHealthResponse) { WAFConfig.health = value; },
    get stats(): WAFConfigDashboardStats { return WAFConfig.stats; },
    set stats(value: WAFConfigDashboardStats) { WAFConfig.stats = value; },
    get rules(): WAFCustomRule[] { return WAFConfig.rules; },
    set rules(value: WAFCustomRule[]) { WAFConfig.rules = value; },
    get crsRules(): WAFManagedRuleFile[] { return WAFConfig.crsRules; },
    set crsRules(value: WAFManagedRuleFile[]) { WAFConfig.crsRules = value; },
    get crsGroups(): Record<string, WAFManagedRuleFile[]> { return WAFConfig.crsGroups; },
    set crsGroups(value: Record<string, WAFManagedRuleFile[]>) { WAFConfig.crsGroups = value; },
    get profiles(): WAFProfile[] { return WAFConfig.profiles; },
    set profiles(value: WAFProfile[]) { WAFConfig.profiles = value; },
    get exclusions(): WAFExclusion[] { return WAFConfig.exclusions; },
    set exclusions(value: WAFExclusion[]) { WAFConfig.exclusions = value; },
    get exclusionQuery(): string { return WAFConfig.exclusionQuery; },
    set exclusionQuery(value: string) { WAFConfig.exclusionQuery = value; },
    get exclusionScope(): WAFExclusionScope { return WAFConfig.exclusionScope; },
    set exclusionScope(value: WAFExclusionScope) { WAFConfig.exclusionScope = value; },
    get isLoading(): boolean { return WAFConfig.isLoading; },
    set isLoading(value: boolean) { WAFConfig.isLoading = value; },
    get loadError(): string | null { return WAFConfig.loadError; },
    set loadError(value: string | null) { WAFConfig.loadError = value; },
    get rulesView(): WAFRulesView { return WAFConfig.rulesView; },
    set rulesView(value: WAFRulesView) { WAFConfig.rulesView = value; },
    get editingRule(): WAFCustomRule | null { return WAFConfig.editingRule; },
    set editingRule(value: WAFCustomRule | null) { WAFConfig.editingRule = value; },
    get refreshInterval(): number | undefined { return WAFConfig.refreshInterval; },
    set refreshInterval(value: number | undefined) { WAFConfig.refreshInterval = value; }
};

const WAFConfigService = {
    init: (): Promise<void> => WAFConfig.init(),
    startRefresh: (): void => WAFConfig.startRefresh(),
    loadConfig: (): Promise<void> => WAFConfig.loadConfig(),
    loadStats: (silent?: boolean): Promise<void> => WAFConfig.loadStats(silent),
    loadRules: (): Promise<void> => WAFConfig.loadRules(),
    loadProfiles: (): Promise<void> => WAFConfig.loadProfiles(),
    loadExclusions: (): Promise<void> => WAFConfig.loadExclusions(),
    saveConfig: (): Promise<void> => WAFConfig.saveConfig()
};

const WAFConfigView = {
    getIcon: WAFConfig.getIcon.bind(WAFConfig),
    renderSectionHeader: (icon: string, title: string, desc: string): string => WAFConfig.renderSectionHeader(icon, title, desc),
    render: (): void => WAFConfig.render(),
    renderTabContent: (): string => WAFConfig.renderTabContent(),
    renderOverview: (): string => WAFConfig.renderOverview(),
    renderManagedRules: (): string => WAFConfig.renderManagedRules(),
    renderReloadHealth: (): string => WAFConfig.renderReloadHealth(),
    renderRules: (): string => WAFConfig.renderRules(),
    renderExclusions: (): string => WAFConfig.renderExclusions(),
    renderProtectionProfiles: (): string => WAFConfig.renderProtectionProfiles(),
    renderRuleEditor: (): string => WAFConfig.renderRuleEditor(),
    renderManagedRulesFolder: (): string => WAFConfig.renderManagedRulesFolder(),
    renderCustomRulesList: (): string => WAFConfig.renderCustomRulesList(),
    renderRulesMain: (): string => WAFConfig.renderRulesMain(),
    renderTrafficChart: (history: WAFConfigHistoryPoint[] | undefined): string => WAFConfig.renderTrafficChart(history),
    updateDashboardUI: (): void => WAFConfig.updateDashboardUI(),
    showToast: showSectionToast
};

const WAFConfigController = {
    switchTab: (id: WAFTabId): void => WAFConfig.switchTab(id),
    toggleAdvancedSettings: (): void => WAFConfig.toggleAdvancedSettings(),
    openCRSEditor: (filename: string): Promise<void> => WAFConfig.openCRSEditor(filename),
    bulkToggleCRS: (enable: boolean): Promise<void> => WAFConfig.bulkToggleCRS(enable),
    openRuleEditor: (ruleId?: string): Promise<void> => WAFConfig.openRuleEditor(ruleId),
    addRuleCondition: (zone?: string, field?: string, pattern?: string, negate?: boolean, operator?: string, transforms?: string[]): void =>
        WAFConfig.addRuleCondition(zone, field, pattern, negate, operator, transforms),
    toggleConditionField: (element: Element): void => WAFConfig.toggleConditionField(element),
    updatePreview: (): void => WAFConfig.updatePreview(),
    submitCustomRule: (isEdit?: boolean): Promise<void> => WAFConfig.submitCustomRule(isEdit),
    toggleCustomRule: (id: string, enabled: boolean): Promise<void> => WAFConfig.toggleCustomRule(id, enabled),
    toggleCRSFile: (filename: string, enabled: boolean): Promise<void> => WAFConfig.toggleCRSFile(filename, enabled),
    deleteRule: (id: string): Promise<void> => WAFConfig.deleteRule(id),
    showExclusionModal: (): void => WAFConfig.showExclusionModal(),
    addExclusion: (): Promise<void> => WAFConfig.addExclusion(),
    deleteExclusion: (id: string): Promise<void> => WAFConfig.deleteExclusion(id),
    toggleExclusionEnabled: (id: string, enabled: boolean): Promise<void> => WAFConfig.toggleExclusionEnabled(id, enabled),
    applyProfile: (profileId: string): void => WAFConfig.applyProfile(profileId),
    updateField: (key: string, value: unknown): void => WAFConfig.updateField(key, value)
};

const wafServiceMethods = new Set<keyof typeof WAFConfigService>([
    'init',
    'startRefresh',
    'loadConfig',
    'loadStats',
...
const wafViewMethods = new Set<keyof typeof WAFConfigView>([
    'getIcon',
    'renderSectionHeader',
    'render',
    'renderTabContent',
...
export const WAFConfigFacade = new Proxy({} as WAFConfigRuntime & { state: typeof WAFConfigState }, {
    get(_target, prop: string | symbol): unknown {
        if (prop === 'state') return WAFConfigState;
        if (typeof prop !== 'string') return undefined;

        if (wafServiceMethods.has(prop as keyof typeof WAFConfigService)) {
            return WAFConfigService[prop as keyof typeof WAFConfigService];
        }
        if (wafViewMethods.has(prop as keyof typeof WAFConfigView)) {
            return WAFConfigView[prop as keyof typeof WAFConfigView];
        }
        if (prop in WAFConfigController) {
            return WAFConfigController[prop as keyof typeof WAFConfigController];
        }

        const value = WAFConfig[prop as keyof WAFConfigRuntime];
    ...
    set(_target, prop: string | symbol, value: unknown): boolean {
        if (typeof prop === 'string' && prop in WAFConfig) {
            (WAFConfig as Record<string, unknown>)[prop] = value;
            return true;
        }
        return false;
    }
