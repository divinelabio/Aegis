...
type AntibotDatasetField = HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement;

type AntibotsBindingsDeps = {
    switchTab(id: string): void;
    applyModePreset(modeName: string): void;
    ...
    parseDatasetValue(target: AntibotDatasetField): unknown;
    getNode(id: string): HTMLElement | null;
};

export function createEnsureAntibotBindings(deps: AntibotsBindingsDeps): () => void {
    ...
    return () => {
        ...
        document.addEventListener('click', (event) => {
            ...
            const target = event.target.closest<HTMLElement>('[data-antibot-action]');
            if (!target) return;
            ...
            switch (action) {
                ...
                case 'set-field': {
                    ...
                    if (target.dataset.antibotRender === 'true') deps.render();
                    break;
                }
                case 'move-rule': {
                    const idx = Number.parseInt(target.dataset.antibotIndex || '', 10);
                    const direction = Number.parseInt(value, 10);
                    if (Number.isFinite(idx) && Number.isFinite(direction)) deps.moveRule(idx, direction);
                    break;
                }
                case 'edit-rule': {
                    const idx = Number.parseInt(target.dataset.antibotIndex || '', 10);
                    if (Number.isFinite(idx)) deps.editRule(idx);
                    break;
                }
                case 'toggle-rule-status': {
                    const idx = Number.parseInt(target.dataset.antibotIndex || '', 10);
                    if (Number.isFinite(idx)) deps.toggleRuleStatus(idx);
                    break;
                }
                case 'delete-rule': {
                    const idx = Number.parseInt(target.dataset.antibotIndex || '', 10);
                    if (Number.isFinite(idx)) deps.deleteRule(idx);
                    break;
                }
                ...
                case 'close-self-if-overlay':
                    if (event.target === target) {
                        ...
                        if (closeAction === 'close-rule-modal') deps.closeRuleModal();
                        if (closeAction === 'close-good-bots-modal') deps.closeGoodBotsModal();
                        if (closeAction === 'close-exception-modal') deps.closeExceptionModal();
                    }
                    break;
                case 'save-new-rule':
                    deps.saveNewRule();
				...
				case 'save-config':
					void Promise.resolve(deps.saveConfig()).then(saved => {
						if (saved) SectionUI.markSaveActionBarClean('data-antibot-action');
					});
					break;
				case 'reset-config':
                ...
                case 'create-rule-from-event':
                    void deps.createRuleFromEvent(
                        target.dataset.antibotRuleKind || '',
                        target.dataset.antibotRuleTarget || '',
                        target.dataset.antibotRuleAction || 'block'
                    );
                    break;
                case 'apply-advanced-preset':
                    void deps.applyAdvancedPreset(
                        target.dataset.antibotScope || '',
                        value
                    );
                    break;
                ...
                case 'add-exception-value':
                    void deps.addExceptionValue(
                        target.dataset.antibotExceptionKey || '',
                        target.dataset.antibotExceptionValue || ''
                    );
                    break;
                default:
                    break;
        ...
        document.addEventListener('keydown', (event) => {
            ...
            if (!(event.target instanceof Element)) return;
            const target = event.target.closest<HTMLElement>('[data-antibot-action="switch-tab"][role="button"]');
            if (!target) return;
