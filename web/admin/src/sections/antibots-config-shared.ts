...
export type AntibotModeName = 'strict' | 'balanced' | 'permissive' | 'custom';

export type AntibotPresetDefinition = {
    detection: {
        enabled: boolean;
    ...
    classification: {
        treat_unknown_as_bot: boolean;
        allow_good_bots: boolean;
        block_bad_bots: boolean;
        verify_search_engines: boolean;
    };
};

export type AntibotRuleCondition = {
    field?: string;
    operator?: string;
...
export type AntibotRule = {
    ...
    expression?: string;
    mode?: string;
    conditions?: AntibotRuleCondition[];
    [key: string]: unknown;
};

export type AntibotStatsState = {
    custom?: {
...
export type AntibotAnalyticsEvent = {
    id: string;
    timestamp: string;
    rule_id?: string;
    rule_name?: string;
...
export type AntibotAnalyticsSummary = {
    total: number;
...
export type AntibotAnalyticsItem = Record<string, string | number | null | undefined>;

export type AntibotAnalyticsFilters = {
    range: string;
    action: string;
    rule: string;
    country: string;
...
export type AntibotAnalyticsState = {
    loading: boolean;
    error: string | null;
    filters: AntibotAnalyticsFilters;
    summary: AntibotAnalyticsSummary;
    events: AntibotAnalyticsEvent[];
    timeline: AntibotAnalyticsItem[];
    topRules: AntibotAnalyticsItem[];
    topIps: AntibotAnalyticsItem[];
    topCountries: AntibotAnalyticsItem[];
    topPaths: AntibotAnalyticsItem[];
    topCategories: AntibotAnalyticsItem[];
    topUserAgents: AntibotAnalyticsItem[];
    topDecisionSources: AntibotAnalyticsItem[];
    topDecisionStates: AntibotAnalyticsItem[];
    topChallengeTypes: AntibotAnalyticsItem[];
    topChallengeOutcomes: AntibotAnalyticsItem[];
    topRuleOverrides: AntibotAnalyticsItem[];
    expandedEventId: string | null;
};

export type AntibotFingerprintIdentity = {
    hash: string;
    tenant_id?: string;
    first_seen?: string;
    last_seen?: string;
...
export type AntibotDetectionConfig = {
    enabled?: boolean;
    mode?: AntibotModeName;
    block_threshold?: number;
    challenge_threshold?: number;
    dynamic_challenge?: boolean;
    sensitivity?: number;
    custom?: AntibotPresetDefinition;
    [key: string]: unknown;
};

export type AntibotManagedThresholds = {
    observe?: number;
    challenge?: number;
    block?: number;
...
export type AntibotChallengeLadder = {
    enabled?: boolean;
    js_challenge_enabled?: boolean;
    managed_challenge_enabled?: boolean;
...
export type AntibotScoringThresholds = {
    observe?: number;
    challenge?: number;
    block?: number;
    [key: string]: unknown;
...
export type AntibotChallengesConfig = {
...
export type AntibotTLSConfig = {
    enabled?: boolean;
    ja3_enabled?: boolean;
    ja4_enabled?: boolean;
    header_order_enabled?: boolean;
    known_bot_ja3_hashes?: string[];
    known_bot_ja4_hashes?: string[];
    mismatch_action?: string;
    [key: string]: unknown;
};

export type AntibotAntiEvasionConfig = {
    fp_rotation_detection?: boolean;
    credential_stuffing?: boolean;
    session_anomaly?: boolean;
    rate_pattern_analysis?: boolean;
    max_fps_per_ip?: number;
    [key: string]: unknown;
};

export type AntibotClassificationConfig = {
    treat_unknown_as_bot?: boolean;
    allow_good_bots?: boolean;
    block_bad_bots?: boolean;
    verify_search_engines?: boolean;
    good_bot_rules?: Record<string, AntibotOverrideRule>;
    [key: string]: unknown;
};

export type AntibotOverrideRule = {
    action: string;
    verify_dns?: boolean;
};

export type AntibotAICrawlersConfig = {
    enabled?: boolean;
    rules?: Record<string, AntibotOverrideRule>;
    [key: string]: unknown;
};

export type AntibotWhitelistConfig = {
    ips?: string[];
    user_agents?: string[];
    paths?: string[];
    methods?: string[];
...
export type AntibotConfigState = {
    enabled?: boolean;
    mode?: string;
    strictness?: string;
    response_mode?: string;
    expert_settings_enabled?: boolean;
    enforcement_mode?: string;
    risk_profile?: string;
    auto_thresholds_enabled?: boolean;
    scoring_profile?: string;
    signal_weights?: Record<string, number>;
    managed_thresholds?: AntibotManagedThresholds;
    challenge_ladder?: AntibotChallengeLadder;
    challenges?: AntibotChallengesConfig;
    tls?: AntibotTLSConfig;
    anti_evasion?: AntibotAntiEvasionConfig;
    default_action?: string;
    hmac_secret?: string;
    detection?: AntibotDetectionConfig;
    classification?: AntibotClassificationConfig;
    ai_crawlers?: AntibotAICrawlersConfig;
    whitelist?: AntibotWhitelistConfig;
    [key: string]: unknown;
};

export type AntibotTab = {
    id: string;
    name: string;
    icon: string;
};

export type AntibotPresetMode = Exclude<AntibotModeName, 'custom'>;

export const ANTIBOT_GOOD_BOTS: string[] = [
...
export const ANTIBOT_TABS: AntibotTab[] = [
...
export const ANTIBOT_ICONS: Record<string, string> = {
...
export const ANTIBOT_MODE_PRESETS: Record<AntibotPresetMode, AntibotPresetDefinition> = {
    strict: {
...
export function createDefaultAntibotConfigState(): AntibotConfigState {
    return {
        enabled: true,
        mode: 'balanced',
        strictness: 'balanced',
...
export function createDefaultAntibotAnalyticsState(): AntibotAnalyticsState {
    return {
        loading: false,
        error: null,
        filters: {
...
export function isAntibotRecord(value: unknown): value is Record<string, unknown> {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export function asAntibotRecord(value: unknown): Record<string, unknown> {
    return isAntibotRecord(value) ? value : {};
}

export function asAntibotOverrideRecord(value: unknown): Record<string, AntibotOverrideRule> {
    const record = asAntibotRecord(value);
    const out: Record<string, AntibotOverrideRule> = {};
...
export function asAntibotStringArray(value: unknown): string[] {
    return Array.isArray(value)
        ? value.map((entry) => String(entry).trim()).filter(Boolean)
        : [];
}

export function asAntibotNumberRecord(value: unknown): Record<string, number> {
    const record = asAntibotRecord(value);
...
export function isAntibotModeName(value: unknown): value is AntibotModeName {
    return value === 'strict' || value === 'balanced' || value === 'permissive' || value === 'custom';
}

export function asAntibotPresetDefinition(value: unknown): AntibotPresetDefinition | undefined {
    if (!isAntibotRecord(value)) return undefined;
...
export function asAntibotRuleCondition(value: unknown): AntibotRuleCondition | null {
    if (!isAntibotRecord(value)) return null;
...
export function asAntibotRuleConditions(value: unknown): AntibotRuleCondition[] {
    return Array.isArray(value) ? value.map(asAntibotRuleCondition).filter((item): item is AntibotRuleCondition => Boolean(item)) : [];
}

export function asAntibotRule(value: unknown): AntibotRule | null {
    if (!isAntibotRecord(value)) return null;
...
export function asAntibotRules(value: unknown): AntibotRule[] {
    return Array.isArray(value) ? value.map(asAntibotRule).filter((rule): rule is AntibotRule => Boolean(rule)) : [];
}

export function asAntibotStatsState(value: unknown): AntibotStatsState {
    const record = asAntibotRecord(value);
...
export function asAntibotFingerprintIdentities(value: unknown): AntibotFingerprintIdentity[] {
    if (!Array.isArray(value)) return [];
    return value.map((item) => {
        const record = asAntibotRecord(item);
        const ips = Array.isArray(record.ips) ? record.ips.filter((ip): ip is string => typeof ip === 'string') : [];
        return {
...
function strictnessFromLegacy(mode: unknown, risk: unknown): string {
    if (risk === 'permissive' || mode === 'permissive') return 'low';
    if (risk === 'aggressive' || mode === 'strict') return 'high';
    return 'balanced';
}

function normalizeResponseMode(value: unknown, fallback: string): string {
    return value === 'log' || value === 'challenge' || value === 'block' ? value : fallback;
}

function normalizeEnforcementMode(value: unknown, fallback: string): string {
    return value === 'managed' || value === 'hybrid' || value === 'observe' || value === 'preset' ? value : fallback;
}

export function asAntibotConfigState(value: unknown): AntibotConfigState {
    const record = asAntibotRecord(value);
...
export function botConfigForSave(config: AntibotConfigState): AntibotConfigState {
	const {
		hmac_secret_configured: _hmacSecretConfigured,
		revision: _revision,
		sdk_version: _sdkVersion,
		sdk_obfuscate: _sdkObfuscate,
		network: _network,
...
export function antibotErrorMessage(error: unknown, fallback: string): string {
    return error instanceof Error ? error.message : fallback;
}

export function antibotControl(id: string): HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null {
    return AdminDOM.getById(id) as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null;
}

export function antibotInput(id: string): HTMLInputElement | null {
    const control = antibotControl(id);
    return control instanceof HTMLInputElement ? control : null;
}

export function antibotSelect(id: string): HTMLSelectElement | null {
    const control = antibotControl(id);
    return control instanceof HTMLSelectElement ? control : null;
}


export function antibotValue(id: string): string {
    return antibotControl(id)?.value ?? '';
}

export function setAntibotField(config: AntibotConfigState, path: string, value: unknown): void {
