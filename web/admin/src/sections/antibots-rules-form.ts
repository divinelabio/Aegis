...
export function toggleAntibotRuleFields(args: {
    ...
    if (!valInput || !valBool || !valCat) return;
...
export function selectAntibotRuleAction(
    action: string,
    args: {
        getNode: (id: string) => HTMLElement | null;
        getInput: (id: string) => HTMLInputElement | null;
...
type RuleReadResult =
    | { ok: true; rule: AntibotRule }
    | { ok: false; error: 'missing-key' | 'missing-fields' };

export function readNewAntibotRuleFromForm(getValue: (id: string) => string): RuleReadResult {
    ...
    const actionMap: Record<string, string> = { allow: 'allow', block: 'block', challenge: 'challenge', log: 'log' };
...
function fieldToExpressionField(field: string): string {
    if (field.startsWith('header:')) return `http.request.headers[${JSON.stringify(field.slice(7))}]`;
    if (field.startsWith('cookie:')) return `http.cookie[${JSON.stringify(field.slice(7))}]`;
    if (field.startsWith('query:')) return `http.request.query[${JSON.stringify(field.slice(6))}]`;
...
function operatorToExpressionOperator(op: string): string {
    const map: Record<string, string> = {
        neq: 'ne',
        regex: 'matches'
    };
