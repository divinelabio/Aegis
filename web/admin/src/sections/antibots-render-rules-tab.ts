import type { AntibotRule, AntibotRuleCondition } from './antibots-config-shared.js';
...
function getRuleConditionIcon(field: string): string {
...
function getRuleActionIcon(action: string): string {
    if (action === 'block') {
...
function normalizeRuleAction(action: AntibotRule['action']): string {
...
function renderBotScopePills(rule: AntibotRule): string {
    const pills: string[] = [];
    const applyGood = (rule as Record<string, unknown>).apply_to_good_bots;
    const applyAI   = (rule as Record<string, unknown>).apply_to_ai_crawlers;
...
export function renderAntibotRulesTab(rules: AntibotRule[], escapeHTML: EscapeHtmlFn, renderSectionHeader: RenderSectionHeaderFn): string {
    ...
    const tableRows = rules.map((rule: AntibotRule, idx: number) => {
        const isFirst = idx === 0;
        ...
        const matchChips = `
            <div class="section-rule-conditions bot-rules-table-matches">
                ...
                ` : ''}
                ${(rule.conditions || []).map((c: AntibotRuleCondition) => {
        ...
        const actions = `<div class="operator-control-actions bot-rules-table-actions">${SectionUI.renderActionMenu({
            ...
            items: [
                ...
                { label: 'Delete rule', attrs: `data-antibot-action="delete-rule" data-antibot-index="${idx}"`, tone: 'danger' as const }
            ],
