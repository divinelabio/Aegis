...
import { queryAll, resolveElement, type ElementTarget } from '../core/dom.js';
...
let pendingActionMenuTrigger: HTMLElement | null = null;
...
function actionMenuItems(menu: HTMLElement): HTMLButtonElement[] {
  return Array.from(menu.querySelectorAll<HTMLButtonElement>('[role="menuitem"]:not([disabled])'));
}

function closeActionMenus(except?: HTMLElement | null, restoreFocus = false): void {
  queryAll<HTMLElement>(actionMenuSelector).forEach(menu => {
    if (menu === except || menu.hidden) return;
    menu.hidden = true;
    const trigger = document.querySelector<HTMLElement>(`${actionMenuTriggerSelector}[aria-controls="${escapeSectionAttr(menu.id)}"]`);
    trigger?.setAttribute('aria-expanded', 'false');
    if (restoreFocus) trigger?.focus();
  });
}

function positionActionMenu(trigger: HTMLElement, menu: HTMLElement): void {
  const gap = 6;
...
function openActionMenu(trigger: HTMLElement, focusFirst = false): void {
  ...
  if (focusFirst) actionMenuItems(menu)[0]?.focus();
}

function modalFocusableElements(panel: HTMLElement): HTMLElement[] {
  return Array.from(panel.querySelectorAll<HTMLElement>(modalFocusableSelector)).filter(element => !element.hasAttribute('hidden') && element.getClientRects().length > 0);
}

type SaveActionBarOptions = {
  actionAttr: string;
  resetAction: string;
  saveAction: string;
  resetLabel?: string;
...
function setSaveActionBarDirty(actionAttr: string | null, dirty: boolean): void {
  ...
  queryAll<HTMLElement>(selector).forEach(bar => {
...
function markSaveActionBarDirty(actionAttr: string): void {
  setSaveActionBarDirty(actionAttr, true);
}

function markSaveActionBarClean(actionAttr?: string): void {
  setSaveActionBarDirty(actionAttr || null, false);
}

function getWindowObject(path: string): Record<string, unknown> | null {
...
function runWindowAction(expression: string): void {
...
function bindSectionUIEvents(): void {
  if (sectionUIEventsBound) return;
  sectionUIEventsBound = true;

  document.addEventListener('change', event => {
    const target = event.target;
    if (!(target instanceof HTMLInputElement)) return;
    if (target.dataset.sectionValueType !== 'checkbox') return;

    const configName = target.dataset.sectionConfigName;
    const field = target.dataset.sectionField;
    if (!configName || !field) return;
  ...
  document.addEventListener('click', event => {
    ...
    const actionMenuTrigger = event.target.closest<HTMLElement>(actionMenuTriggerSelector);
    if (actionMenuTrigger) {
      ...
      if (shouldOpen) {
        ...
        window.requestAnimationFrame(() => {
          if (pendingActionMenuTrigger !== actionMenuTrigger || !actionMenuTrigger.isConnected) return;
    ...
    const actionMenuItem = event.target.closest<HTMLElement>('[role="menuitem"]');
    if (actionMenuItem) {
      pendingActionMenuTrigger = null;
      closeActionMenus();
    ...
    const target = event.target.closest<HTMLElement>('[data-section-action]');
    if (!target) return;

    const action = target.dataset.sectionAction || '';
    if (action === 'close-on-backdrop') {
      if (event.target === target) SectionUI.closeModal();
      return;
    }
  ...
  document.addEventListener('keydown', event => {
    ...
    const trigger = event.target.closest<HTMLElement>(actionMenuTriggerSelector);
    ...
    const menu = event.target.closest<HTMLElement>(actionMenuSelector);
    if (!menu) return;
...
export const SectionUI: SectionUIApi = {
  icons: {
  ...
  renderModuleHeader(icon: string, title: string, desc: string, toggleField: string | null, enabled: boolean, configName: string): string {
    bindSectionUIEvents();
  ...
  renderSaveActionBar(options: SaveActionBarOptions): string {
    bindSectionUIEvents();
  ...
  renderLoading(message = 'Loading configuration...'): string {
    return `
  ...
  getElement(target: ElementTarget): HTMLElement | null {
    return resolveElement(target);
  },

  setHidden(target: ElementTarget, hidden = true): HTMLElement | null {
  ...
  toggleHidden(target: ElementTarget): boolean {
    const element = this.getElement(target);
  ...
  renderPageHeader(title: string, subtitle = '', actions = ''): string {
    return `
  ...
  renderConfigPageHero(title: string, subtitle = '', kicker = '', actions = ''): string {
    return `
  ...
  renderConfigConsoleHeader(options): string {
  ...
  renderConfigSectionNav(tabs, activeId, action, actionAttr = 'data-waf-action', valueAttr = 'data-waf-value'): string {
    return `
  ...
  renderMetricStrip(metrics): string {
    return `
  ...
  renderOperatorFrame(options): string {
  ...
  renderResourceWorkspace(options): string {
    bindSectionUIEvents();
    const tabs = options.tabs || [];
    const tabNav = tabs.length
      ? this.renderLineTabs(tabs, options.activeTab || tabs[0].id, {
  ...
  renderLineTabs(tabs, activeId, options): string {
  ...
  renderOperatorMetricStrip(metrics, className = ''): string {
    return `
  ...
  renderOperatorBackButton(options): string {
  ...
  renderOperatorSection(title, content, options = {}): string {
    return `
  ...
  renderOperatorControlRow(options): string {
  ...
  renderStickyActionBar(content, className = ''): string {
    return `<div class="operator-sticky-actions ${className}">${content}</div>`;
  },

  renderConfigPanel(title: string, content: string, options = {}): string {
    const className = options.className || '';
    return `
  ...
  renderStatusPill(label: string, tone = 'neutral'): string {
    return `<span class="config-status-pill tone-${tone}">${label}</span>`;
  },

  renderActionMenu(options): string {
    bindSectionUIEvents();
    const className = options.className ? ` ${options.className}` : '';
    return `
  ...
  renderEnterpriseTable(options): string {
    const empty = `
        <tr>
  ...
  renderEmptyState(title: string, message: string, icon = 'info', action = ''): string {
  ...
  showModal(content: string, options: ModalOptions = {}): void {
    ...
    if (activeElement instanceof HTMLElement && !container.contains(activeElement)) this.modalFocusReturnTarget = activeElement;
    ...
    const title = panel.querySelector<HTMLElement>('h1, h2, h3, h4, h5, h6');
    if (title) {
      if (!title.id) title.id = 'section-modal-title';
      panel.setAttribute('aria-labelledby', title.id);
    } else panel.setAttribute('aria-label', 'Dialog');

    this.modalKeyHandler = (event: KeyboardEvent): void => {
      if (event.key === 'Escape') {
        this.closeModal();
        return;
      }
    ...
    window.setTimeout(() => {
      if (!container?.isConnected || !panel.isConnected) return;
  ...
  openConfirmModal(title: string, message: string, btnText: string, btnColor: string, callback: MaybeAsyncVoidFn | string): void {
