...
function action(label: string, name: string, attributes = '', primary = false): string {
...
function renderRuntimeControls(state: EdgeAccessV2ShellState): string {
  const resource = state.runtime.settings;
  const wait = pending(resource.loading, resource.error, 'runtime-retry', 'runtime settings');
  if (wait) return SectionUI.renderOperatorSection('Runtime settings', wait, { subtitle: 'Global controls for platform operators.', className: 'edge-access-v2-runtime-section' });
  const settings = resource.value;
  if (!settings) return SectionUI.renderOperatorSection('Runtime settings', SectionUI.renderEmptyState('Runtime settings unavailable', 'Retry loading the runtime configuration.', 'settings'), { subtitle: 'Global controls for platform operators.', className: 'edge-access-v2-runtime-section' });
...
export function renderV2Runtime(state: EdgeAccessV2ShellState): string {
