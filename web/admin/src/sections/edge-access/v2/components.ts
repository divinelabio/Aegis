...
import type { IdentityProvider, ServiceIdentity } from './identity-contract.js';
import type {
  EdgeAccessV2Destination,
  EdgeAccessV2IdentityProviderKind,
  EdgeAccessV2IdentityProviderPreset,
  EdgeAccessV2IdentityTab,
  EdgeAccessV2ShellState
} from './types.js';
...
const configurationNavigation: readonly NavigationItem[] = [
...
function identityAction(label: string, action: string, attributes = '', primary = false): string {
...
function identityLoadingOrError(loading: boolean, error: string | null, retryTab: EdgeAccessV2IdentityTab): string | null {
  if (loading) return SectionUI.renderLoading('Loading identity configuration...');
  if (error) return `<div class="section-error" role="alert"><div class="error-message">${escapeHTML(error)}</div>${identityAction('Retry', 'retry-identity', `data-edge-access-v2-identity-tab="${retryTab}"`)}</div>`;
  return null;
}

function providerStatus(provider: IdentityProvider): string {
  const status = provider.validation.status;
  const label: Record<string, string> = { valid: 'Valid', warning: 'Warning', failed: 'Failed', not_validated: 'Not validated' };
  return SectionUI.renderStatusPill(label[status] || 'Not validated', statusTone(status));
}

interface ProviderTemplate {
  id: EdgeAccessV2IdentityProviderPreset;
  name: string;
  description: string;
  type: 'oidc' | 'jwt';
...
function providerBrand(provider: Pick<IdentityProvider, 'name' | 'issuer' | 'type'>): EdgeAccessV2IdentityProviderPreset {
  const source = `${provider.name} ${provider.issuer}`.toLowerCase();
  if (provider.type === 'jwt') {
    if (source.includes('cognito') || source.includes('amazonaws')) return 'cognito-jwt';
    if (source.includes('keycloak')) return 'keycloak-jwt';
    if (source.includes('microsoft') || source.includes('entra') || source.includes('login.microsoftonline')) return 'entra-jwt';
    if (source.includes('okta')) return 'okta-jwt';
    if (source.includes('auth0')) return 'auth0-jwt';
    return 'jwt';
  }
  if (source.includes('microsoft') || source.includes('entra') || source.includes('login.microsoftonline')) return 'entra';
  if (source.includes('okta')) return 'okta';
  if (source.includes('auth0')) return 'auth0';
  if (source.includes('google') || source.includes('accounts.google')) return 'google';
  if (source.includes('keycloak')) return 'keycloak';
  return 'oidc';
}

function providerMark(brand: EdgeAccessV2IdentityProviderPreset, label: string): string {
  const content: Record<EdgeAccessV2IdentityProviderPreset, string> = {
...
function setupField(label: string, name: string, value = '', options: { type?: string; required?: boolean; placeholder?: string; wide?: boolean; textarea?: boolean; readonly?: boolean } = {}): string {
...
function renderProviderTypeSelector(selectedType?: EdgeAccessV2IdentityProviderKind): string {
...
function renderProviderSetup(state: EdgeAccessV2ShellState): string {
  ...
  if (editing && state.identity.providers.loading) return `<div class="edge-access-v2-create-shell edge-access-v2-provider-setup-shell">${renderCreateBack('Identity sources', 'Back to identity sources', 'provider-back')}${SectionUI.renderLoading('Loading provider configuration...')}</div>`;
  if (editing && !provider) return `<div class="edge-access-v2-create-shell edge-access-v2-provider-setup-shell">${renderCreateBack('Identity sources', 'Back to identity sources', 'provider-back')}${SectionUI.renderOperatorSection('Provider unavailable', SectionUI.renderEmptyState('Provider not found', 'Return to Identity sources and choose an available provider.', 'info'))}</div>`;
...
function serviceIdentityStatus(identity: ServiceIdentity): string {
...
function machineIdentityMark(identity: ServiceIdentity): string {
...
function renderIdentityInventory(state: EdgeAccessV2ShellState): string {
...
function renderIdentity(state: EdgeAccessV2ShellState): string {
  if (state.route.identityProviderMode !== 'list') return renderProviderSetup(state);
  return renderIdentityInventory(state);
}

function renderDestination(state: EdgeAccessV2ShellState): string {
...
export function renderEdgeAccessV2Shell(state: EdgeAccessV2ShellState): string {
