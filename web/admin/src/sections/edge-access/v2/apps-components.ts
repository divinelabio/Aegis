...
import type { AppCatalogItem, AppRecentDecision } from './apps-contract.js';
...
function action(label: string, name: string, attributes = '', primary = false, leadingIcon = ''): string {
...
function appStatus(item: AppCatalogItem): string {
  const state = item.protected_app.access_health.state;
  const label: Record<string, string> = { draft: 'Draft', deny_only: 'Deny-only', active: 'Active', update_available: 'Update available', disabled: 'Disabled', error: 'Error' };
  return SectionUI.renderStatusPill(label[state] || state, statusTone(state === 'deny_only' ? 'warning' : state));
}

function addresses(item: AppCatalogItem): string {
...
function accessSummary(item: AppCatalogItem): string {
...
function appIdentity(item: AppCatalogItem): string {
...
function loadingOrError(loading: boolean, error: string | null, retry: string): string | null {
  if (loading) return SectionUI.renderLoading('Loading Apps...');
  if (error) return `<div class="section-error" role="alert"><div class="error-message">${escapeHTML(error)}</div>${action('Retry', retry)}</div>`;
  return null;
}

function renderAppList(state: EdgeAccessV2ShellState): string {
  ...
  const rows = apps.map(item => {
    ...
    const rowActions = [
      ...
      ...(app.deployment_state === 'draft'
        ...
        : []),
      ...(app.deployment_state === 'disabled'
        ? [{ label: 'Delete application', attrs: `data-edge-access-v2-action="app-delete" ${appAttrs}`, tone: 'danger' as const }]
        : [{ label: 'Disable application', attrs: `data-edge-access-v2-action="app-disable" ${appAttrs}`, tone: 'danger' as const }])
    ];
    return [
      appIdentity(item),
      addresses(item),
...
function routeOptions(state: EdgeAccessV2ShellState): string {
  if (state.apps.routes.loading) return '<option>Loading Routes…</option>';
  const routes = state.apps.routes.value || [];
  if (!routes.length) return '<option value="">No Routes available</option>';
...
function providersFor(state: EdgeAccessV2ShellState, type: 'oidc' | 'jwt') {
  return (state.identity.providers.value || []).filter(provider => provider.type === type);
}

function providerSelect(state: EdgeAccessV2ShellState, type: 'oidc' | 'jwt', name: string, label: string): string {
...
function providerAvailability(state: EdgeAccessV2ShellState, type: 'oidc' | 'jwt'): string {
  const resource = state.identity.providers;
  const providers = providersFor(state, type);
  if (resource.loading) return '<p class="edge-access-v2-choice-hint">Loading configured identity providers…</p>';
  if (resource.error) return `<p class="edge-access-v2-choice-hint">Identity providers could not be loaded. <button type="button" class="btn-link" data-edge-access-v2-action="retry-identity" data-edge-access-v2-identity-tab="providers">Retry</button></p>`;
  if (providers.length) return '<p class="edge-access-v2-choice-hint">Configured in Identity. You can change this later without changing the Route.</p>';
...
function renderNewAppExperience(state: EdgeAccessV2ShellState): string {
...
function renderOverview(state: EdgeAccessV2ShellState, item: AppCatalogItem, decisions: AppRecentDecision[] | null): string {
  const app = item.protected_app;
...
function renderPolicyAttachmentsWorkspace(state: EdgeAccessV2ShellState, item: AppCatalogItem): string {
  const app = item.protected_app;
  const appAttrs = `data-edge-access-v2-app-id="${escapeHTML(app.id)}"`;
...
function renderDetail(state: EdgeAccessV2ShellState, item: AppCatalogItem): string {
  if (state.route.appSubpage === 'access-policies') return renderPolicyAttachmentsWorkspace(state, item);
  const app = item.protected_app;
  const appAttrs = `data-edge-access-v2-app-id="${escapeHTML(app.id)}"`;
...
export function renderV2Apps(state: EdgeAccessV2ShellState): string {
  if (state.route.creatingApp) return renderNewAppExperience(state);
  if (state.route.appID) {
    const resource = state.apps.detail; const pending = loadingOrError(resource.loading, resource.error, 'app-detail-retry');
    if (pending) {
    ...
    if (resource.value) return renderDetail(state, resource.value);
    return SectionUI.renderResourceWorkspace({
