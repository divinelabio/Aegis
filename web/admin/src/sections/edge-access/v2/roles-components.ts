...
import type { EdgeAccessV2ResourceState, EdgeAccessV2ShellState } from './types.js';
...
function accessSummary(role: AccessRole): string {
...
export function renderManagedRolePicker(
  resource: EdgeAccessV2ResourceState<AccessRole[]>,
  selected: string[],
  options: { name?: string; label?: string; excludeID?: string; emptyMessage?: string } = {}
): string {
...
function renderRoleForm(state: EdgeAccessV2ShellState): string {
...
function renderRoleInventory(state: EdgeAccessV2ShellState): string {
...
export function renderV2Roles(state: EdgeAccessV2ShellState): string {
