...
import type { AntibotConfigState, AntibotWhitelistConfig } from './antibots-config-shared.js';

type RenderSectionHeaderFn = (
    icon: string,
    title: string,
    desc: string,
...
type ExceptionField = {
    key: keyof AntibotWhitelistConfig;
    label: string;       // short label for stat row
    title: string;
    modalLabel: string;
    icon: string;
    placeholder: string;
    accent: string;      // success | primary | warning | secondary | danger
};
...
const EXCEPTION_FIELDS: ExceptionField[] = [
    {
        key: 'ips',
        label: 'IPs',
        title: 'IP / CIDR',
...
function exceptionList(whitelist: AntibotWhitelistConfig, key: keyof AntibotWhitelistConfig): string[] {
...
function exceptionEntries(whitelist: AntibotWhitelistConfig): ExceptionEntry[] {
    return EXCEPTION_FIELDS.flatMap((field) =>
        exceptionList(whitelist, field.key).map((value) => ({ key: field.key, value, field }))
    );
}
...
function renderExceptionRow(entry: ExceptionEntry, index: number): string {
...
function renderExceptionsTable(whitelist: AntibotWhitelistConfig): string {
...
function renderAddExceptionModal(open: boolean): string {
    if (!open) return '';
...
function renderExceptionsMetricStrip(whitelist: AntibotWhitelistConfig): string {
...
export function renderAntibotExceptionsTab(
