...
import type {
    AppSecurityEditionID,
    AppSecurityEditionInfo,
    AppSecurityViewModel,
    HTTPSecurityCapabilities
} from './types.js';

type HTTPSecurityConfigRecord = Record<string, unknown>;

const EDITION_LABELS: Record<AppSecurityEditionID, string> = {
    community: 'Community',
...
function sectionValue(config: HTTPSecurityConfigRecord, key: string): unknown {
    return cloneHTTPSecurityValue(config[key] ?? {});
}

function optionalSectionValue(
    config: HTTPSecurityConfigRecord,
...
function normalizeEdition(edition: unknown): AppSecurityEditionInfo {
...
function normalizeEditionID(value: unknown): AppSecurityEditionID {
...
function assignIfDefined(target: HTTPSecurityConfigRecord, key: string, value: unknown | undefined): void {
    if (value !== undefined) target[key] = cloneHTTPSecurityValue(value);
}

export function createHTTPSecurityViewModel(
    config: HTTPSecurityConfigRecord | null | undefined,
    capabilities: HTTPSecurityCapabilities | null | undefined,
    securityTxtContact: string,
    edition: unknown = 'unknown'
): AppSecurityViewModel {
    const source = cloneHTTPSecurityValue(config || {}) as HTTPSecurityConfigRecord;
    const mergedCapabilities: HTTPSecurityCapabilities = {
...
export function buildHTTPSecurityConfigFromViewModel(model: AppSecurityViewModel): HTTPSecurityConfigRecord {
    const config: HTTPSecurityConfigRecord = {};
...
export function buildHTTPSecuritySavePayloadFromViewModel(model: AppSecurityViewModel): HTTPSecurityConfigRecord {
