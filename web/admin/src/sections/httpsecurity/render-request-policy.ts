...
export function renderHTTPSecurityRequestPolicy(runtime: HTTPSecurityRenderRuntime): string {
    const view = runtime;
        ...
        const modeCard = (active: any, icon: any, title: any, desc: any, meta: any, attrs: any) => `
        ...
        const compactModeOption = (active: any, title: any, desc: any, attrs: any) => `
        ...
        const customEditorHeader = (title: any, desc: any, meta: any) => `
        ...
        const moduleSwitch = (key: any, enabled: any, title: any) => `<label class="section-switch section-policy-toggle-switch" title="${enabled ? 'Disable' : 'Enable'} ${title}"><input type="checkbox" ${enabled ? 'checked' : ''} data-httpsec-path="${key}.enabled" data-httpsec-value-type="checkbox" data-httpsec-render="true"><span class="switch-slider"></span></label>`;
        const methodTiles = () => `<div class="httpsec-check-grid httpsec-method-grid">${allMethods.map((method: any) => `<label class="httpsec-check-tile ${selectedMethods.includes(method) ? 'is-active' : ''}"><input type="checkbox" value="${method}" ${selectedMethods.includes(method) ? 'checked' : ''} data-httpsec-method-toggle="${method}"><span>${method}</span></label>`).join('')}</div>`;
        const contentTiles = () => `<div class="httpsec-check-grid httpsec-method-grid">${['POST', 'PUT', 'PATCH', 'DELETE'].map((method: any) => `<label class="httpsec-check-tile ${(ctv.required_on || []).includes(method) ? 'is-active' : ''}"><input type="checkbox" value="${method}" ${(ctv.required_on || []).includes(method) ? 'checked' : ''} data-httpsec-array-toggle="content_type_validator.required_on"><span>${method}</span></label>`).join('')}</div>`;
        const domainContent = `
            ...
            ${hv.enabled === true ? `<div class="httpsec-compact-editor httpsec-custom-editor">
              ...
              <div class="section-inline-form"><input type="text" id="host-add-input" placeholder="example.com" class="section-input text-mono" data-httpsec-enter-action="add-tag-from-input" data-httpsec-path="host_validator.allowed_hosts"><button type="button" class="btn btn-outline btn-sm" data-httpsec-action="add-tag-from-input" data-httpsec-path="host_validator.allowed_hosts" data-httpsec-input-id="host-add-input">Add</button></div>
                <div class="httpsec-extension-grid">${hosts.map((host: any) => `<button class="httpsec-extension-chip" type="button" data-httpsec-action="remove-tag" data-httpsec-path="host_validator.allowed_hosts" data-httpsec-value="${encodeHTTPSecurityValue(host)}"><span>${escapeHTTPSecurityHtml(host)}</span><span class="httpsec-extension-remove" aria-hidden="true">&times;</span></button>`).join('') || '<div class="httpsec-empty-note">No domains listed yet.</div>'}</div>
            </div>` : ''}
        `;
        const methodContent = `
        ...
        const requestAreaRow = (id: string, title: string, description: string, icon: string, enabled: boolean) => SectionUI.renderOperatorControlRow({
            title,
            description,
            icon: view.icons[icon] || view.icons.shield,
            enabled,
        ...
        const requestPages: Record<string, { content: string }> = {
            host_validator: {
