...
import { renderUpgradeBanner } from '../../core/features.js';
...
import {
    HTTPSEC_BOUNDARY_PROFILES,
    HTTPSEC_ADVANCED_CONFIG_ROOTS,
    HTTPSEC_CONTENT_TYPE_PROFILES,
    HTTPSEC_COOKIE_PROFILES,
    HTTPSEC_EXTENSION_GROUPS,
    HTTPSEC_FALLBACK_CAPABILITIES,
    HTTPSEC_METHOD_PROFILES,
    HTTPSEC_PAYLOAD_INSPECTION_PROFILES,
    HTTPSEC_PAYLOAD_PROFILES,
    HTTPSEC_RESPONSE_PROFILES,
    HTTPSEC_ROOT_CAPABILITY,
    HTTPSEC_UPLOAD_PROFILES,
    HTTP_SECURITY_EFFECTIVE_STATE_ENDPOINT
} from './constants.js';
import {
    getHTTPSecurityCapability,
    isHTTPSecurityConfigurable,
    normalizeHTTPSecurityCapability,
    normalizeHTTPSecurityExtension
} from './normalize.js';
import {
    buildHTTPSecurityConfigFromViewModel,
    buildHTTPSecuritySavePayloadFromViewModel,
    createHTTPSecurityViewModel
} from './view-model.js';
import {
    applyHTTPSecurityCSPPreset,
    applyHTTPSecurityPermissionsPreset,
    isValidHTTPSecurityExtension,
    isValidHTTPSecurityHeaderName,
    parseHTTPSecurityCSP,
    parseHTTPSecurityPermissionsPolicy,
    serializeHTTPSecurityCSP,
    serializeHTTPSecurityPermissionsPolicy,
...
import type { AppSecurityViewModel } from './types.js';
let httpSecurityBindingsInitialized = false;
let httpSecurityBoundRoot: HTMLElement | null = null;
let httpSecurityBindingDisposers: Array<() => void> = [];
type AnyRecord = Record<string, any>;
type FieldElement = HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
const HTTPSEC_INITIAL_STATE = createHTTPSecurityInitialState() as AnyRecord;
function encodeHTTPSecurityValue(value: unknown): string {
    return encodeURIComponent(String(value));
}
function decodeHTTPSecurityValue(value: string | undefined): string {
    return value ? decodeURIComponent(value) : '';
}
function httpsecSizeToMb(value: unknown): number {
...
function httpsecSizeToBytes(value: unknown): number {
    return httpsecSizeToMb(value) * 1024 * 1024;
}
function parseHTTPSecurityFieldValue(element: FieldElement): unknown {
    ...
    if (valueType === 'csv') {
        return element.value.split(',').map((item: string) => item.trim()).filter(Boolean);
    }
    return element.value;
}
function setHTTPSecurityFieldFromElement(element: Element): boolean {
...
function ensureHTTPSecurityBindings() {
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLElement>(document, 'click', '[data-httpsec-action]', (_event: any, target: any) => {
        ...
        const actionHandlers: Record<string, () => boolean> = {
            ...
            'add-tag-from-input': () => {
                ...
                if (!(input instanceof HTMLInputElement))
                    return false;
                const values = input.value.split(/[\s,]+/).map((value: string) => value.trim()).filter(Boolean);
                const updated = values.some((value: string) => HTTPSecurityConfig.addTag(path, value) !== false);
                input.value = '';
                return updated;
            },
            'toggle-array-item': () => {
            ...
            'open-request-policy-page': () => {
                ...
                if (!['host_validator', 'method_enforcer', 'request_size_guard', 'content_type_validator', 'header_manager'].includes(page))
                    return false;
                HTTPSecurityConfig.requestPolicyPage = page as any;
                HTTPSecurityConfig.render();
                return false;
            },
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLInputElement | HTMLSelectElement>(document, 'change', '[data-httpsec-action="update-header-row"], [data-httpsec-action="set-permissions-policy-feature"]', (_event: any, target: any) => {
        const action = target.dataset.httpsecAction;
        let mutated = false;
        if (action === 'update-header-row' && target instanceof HTMLInputElement) {
            mutated = HTTPSecurityConfig.updateHeaderRow(
                target.dataset.httpsecHeaderPath || '',
                decodeHTTPSecurityValue(target.dataset.httpsecOldName || ''),
                target.dataset.httpsecHeaderField || '',
                target.value
            ) !== false;
        }
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<FieldElement>(document, 'change', '[data-httpsec-path]', (_event: any, target: any) => {
        if (setHTTPSecurityFieldFromElement(target)) {
            SectionUI.markSaveActionBarDirty('data-httpsec-action');
        }
    }));
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLSelectElement>(document, 'change', 'select[data-httpsec-profile-select="response"]', (_event: any, target: any) => {
        if (HTTPSecurityConfig.applyResponseHeaderProfile(target.value) !== false) {
            SectionUI.markSaveActionBarDirty('data-httpsec-action');
        }
    }));
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '[data-httpsec-method-toggle]', (_event: any, target: any) => {
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '[data-httpsec-array-toggle]', (_event: any, target: any) => {
        if (!(target instanceof HTMLInputElement))
            return;
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '[data-httpsec-file-type-toggle]', (_event: any, target: any) => {
    ...
    httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<FieldElement>(document, 'keydown', '[data-httpsec-enter-action="add-tag-from-input"]', (event: any, target: any) => {
        ...
        if (!path)
            return;
        const values = String(target.value || '').split(/[\s,]+/).map((value: string) => value.trim()).filter(Boolean);
        if (!values.some((value: string) => HTTPSecurityConfig.addTag(path, value) !== false))
            return;
        target.value = '';
        SectionUI.markSaveActionBarDirty('data-httpsec-action');
	...
	httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLElement>(document, 'keydown', '[data-httpsec-action="switch-tab"][role="button"], [data-httpsec-action="open-request-policy-page"][role="button"], [data-httpsec-action="open-response-protection-page"][role="button"]', (event: any, target: any) => {
	...
	httpSecurityBindingDisposers.push(AdminEvents.delegateEvent<HTMLInputElement>(document, 'keydown', '#httpsec-securitytxt-contact', (event: any) => {
		if (!(event instanceof KeyboardEvent) || event.key !== 'Enter')
			return;
		event.preventDefault();
		void HTTPSecurityConfig.saveSecurityTxt();
...
const HTTPSecurityConfig: AnyRecord = {
    ...
    fileTypePolicyModal: null as { path: string; title: string; countLabel: string } | null,
    payloadExpertPage: false,
    payloadInspectionCustomSelected: false,
    requestPolicyPage: null as 'host_validator' | 'method_enforcer' | 'request_size_guard' | 'content_type_validator' | 'header_manager' | null,
    responseProtectionPage: null as 'browser' | 'privacy' | 'cookies' | 'headers' | null,
    icons: {
    ...
    refreshViewModel(): AppSecurityViewModel {
    ...
    async saveConfig() {
        ...
        if (validation.errors.length) {
			this.validationIssues = validation.errors.map((message: string) => ({ path: '', message }));
        ...
        try {
			...
			this.validationIssues = details.map((detail: AnyRecord) => ({
    ...
    async saveSecurityTxt() {
        ...
        try {
            ...
            this.refreshViewModel();
			showSectionToast(
				contact === '' ? 'Security.txt publication removed' : published ? 'Security.txt published and verified' : 'Contact saved, but the public file could not be verified',
				contact === '' || published ? 'success' : 'warning'
			);
            this.render();
			return true;
        }
	...
	isValidSecurityTxtContact(contact: string) {
	...
	normalizeSecurityTxtContact(contact: string) {
		const value = String(contact || '').trim();
		if (!value)
			return '';
    ...
    canMutatePath(_path: any) {
        return true;
    },
    updateField(path: any, value: any) {
	...
	clearValidationIssue(path: string) {
		const normalized = String(path || '');
		this.validationIssues = (this.validationIssues || []).filter((issue: AnyRecord) => {
		...
		AdminDOM.queryAll('#httpsecurity-config-content [data-httpsec-path]').forEach((element: any) => {
			if (element instanceof HTMLElement && element.dataset.httpsecPath === normalized)
				element.removeAttribute('aria-invalid');
		});
	},
    getField(path: any, fallback: any) {
        const parts = path.split('.');
        let obj = this.config;
    ...
    render() {
        ...
        const openAdvancedDetails = AdminDOM.queryAll('#httpsecurity-config-content .httpsec-advanced-details')
            .map((details: any, index: any) => details instanceof HTMLDetailsElement && details.open ? index : -1)
            .filter((index: any) => index >= 0);
        const visibleTabs = this.tabs;

        // If current tab was hidden, reset to first visible
        if (!visibleTabs.find((t: any) => t.id === this.currentTab)) {
            this.currentTab = visibleTabs[0]?.id ?? 'overview';
        }
		const configuredEnabled = this.config?.enabled === true;

        const consoleMetaMap: Record<string, { title: string; kicker: string; subtitle: string }> = {
            overview: {
                title: 'Overview',
        ...
        AdminDOM.queryAll('#httpsecurity-config-content .httpsec-advanced-details').forEach((details: any, index: any) => {
	...
	tabForValidationPath(path: string) {
	...
	focusValidation(path: string) {
		...
		requestAnimationFrame(() => {
			...
			const exact = fields.find((element: any) => element instanceof HTMLElement && element.dataset.httpsecPath === normalized);
			const root = normalized.split('.')[0];
			const fallback = fields.find((element: any) => element instanceof HTMLElement && String(element.dataset.httpsecPath || '').startsWith(`${root}.`));
	...
	applyValidationMarkers() {
		const issuePaths: Set<string> = new Set((this.validationIssues || []).map((issue: AnyRecord) => String(issue?.path || '')).filter(Boolean));
		AdminDOM.queryAll('#httpsecurity-config-content [data-httpsec-path]').forEach((element: any) => {
			if (!(element instanceof HTMLElement))
				return;
			const path = String(element.dataset.httpsecPath || '');
			const invalid = Array.from(issuePaths).some((issuePath: string) => issuePath === path || issuePath.startsWith(`${path}.`) || path.startsWith(`${issuePath}.`));
			if (invalid)
    ...
    renderTabContent() {
        switch (this.currentTab) {
            ...
            case 'request_policy': return renderHTTPSecurityRequestPolicy(this as any);
            case 'payload_protection': return renderHTTPSecurityPayloadProtection(this as any);
            case 'upload_security': return renderHTTPSecurityUploadSecurity(this as any);
            case 'response_protection': return renderHTTPSecurityResponseProtection(this as any);
            default: return '';
        }
    },
    // ========== OVERVIEW ==========
    ...
    renderOverviewStat(label: any, value: any, color: any) {
    ...
    renderModuleCards(modules: any) {
        return modules.map((module: any) => this.renderModuleCard(module)).join('');
    },
    renderModuleCard(mod: any) {
        const row = Array.isArray(this.effectiveState?.functions)
            ? this.effectiveState.functions.find((item: any) => item?.id === mod.key)
            : null;
    ...
    removeTag(path: any, value: any) {
        if (!this.canMutatePath(path))
            return false;
        const arr = this.getField(path, []).filter((x: any) => x !== value);
        this.updateField(path, arr);
        this.render();
        return true;
    },
    toggleArrayItem(path: any, value: any) {
        if (!this.canMutatePath(path))
            return false;
        const arr = this.getField(path, []);
    ...
    setArrayItem(path: any, value: any, enabled: any) {
        if (!path || !value)
            return false;
        if (!this.canMutatePath(path))
            return false;
        ...
        const next = enabled ? [...new Set([...arr, normalizedValue])] : arr.filter((item: any) => item !== normalizedValue);
        this.updateField(path, next);
        this.render();
        return true;
    },
    setMethodEnabled(method: any, enabled: any) {
        if (!method)
            return false;
        const mode = (this.config.method_enforcer || {}).mode || 'blocklist';
        const field = mode === 'blocklist' ? 'method_enforcer.blocked_methods' : 'method_enforcer.allowed_methods';
    ...
    getRequestMethodProfile(me: any) {
        const mode = me.mode || 'blocklist';
        const blocked = me.blocked_methods || [];
        const allowed = me.allowed_methods || [];
        const same = (left: any, right: any) => left.length === right.length && right.every((item: any) => left.includes(item));
    ...
    getContentTypeProfile(ctv: any) {
        const required = ctv.required_on || [];
        const same = (right: any) => required.length === right.length && right.every((item: any) => required.includes(item));
    ...
    getRequestBoundaryProfile(rsg: any) {
        ...
        const matches = (preset: any) => Object.keys(values).every((key) => values[key] === preset[key]);
    ...
    getPayloadInspectionProfile() {
        ...
        const matches = (profile: any) => Number(body.max_findings || 100) === profile.maxFindings
    ...
    applyRequestMethodProfile(profile: any) {
    ...
    applyContentTypeProfile(profile: any) {
    ...
    applyRequestBoundaryPreset(preset: any) {
    ...
    applyPayloadProfile(profile: any) {
    ...
    applyPayloadInspectionProfile(profileID: any) {
    ...
    applyUploadProfile(profile: any) {
        if (profile === 'custom') {
    ...
    applyResponseHeaderProfile(profile: any) {
        if (profile === 'custom') {
    ...
    applyCookieProfile(profile: any) {
        if (profile === 'custom') {
    ...
    addHeaderRow(path: any, name: any, value: any) {
        if (!this.canMutatePath(path))
            return false;
        ...
        const duplicate = Object.keys(map).find((key: any) => key.toLowerCase() === cleanName.toLowerCase());
        if (duplicate) {
    ...
    updateHeaderRow(path: any, oldName: any, field: any, value: any) {
        if (!this.canMutatePath(path))
            return false;
        const map = { ...(this.getField(path, {}) || {}) };
        ...
        if (field === 'name') {
            ...
            const duplicate = Object.keys(map).find((key: any) => key.toLowerCase() === nextName.toLowerCase() && key !== previous);
            if (duplicate) {
                showSectionToast('That header is already configured.', 'warning');
                return false;
            }
    ...
    removeHeaderRow(path: any, name: any) {
        if (!this.canMutatePath(path))
            return false;
        const map = { ...(this.getField(path, {}) || {}) };
    ...
    addCSPSource(directive: any, source: any) {
        if (!this.canMutatePath('security_headers.csp'))
            return false;
    ...
    removeCSPSource(value: any) {
        ...
        if (!directive || !source)
            return false;
        const map = parseHTTPSecurityCSP(this.config.security_headers?.csp || '');
        map[directive] = (map[directive] || []).filter((item: any) => item !== source);
        this.updateField('security_headers.csp', serializeHTTPSecurityCSP(map));
        this.render();
        return true;
    },
    setPermissionsPolicyFeature(feature: any, value: any) {
    ...
    applyPermissionsPolicyPreset(preset: any) {
        if (!this.canMutatePath('security_headers.permissions_policy'))
            return false;
    ...
    addExtensionGroup(path: any, groupID: any) {
        if (!this.canMutatePath(path))
            return false;
...
const HTTPSecurityConfigState: AnyRecord = {
    get currentTab() { return HTTPSecurityConfig.currentTab; },
    set currentTab(value: any) { HTTPSecurityConfig.currentTab = value; },
    get config() { return HTTPSecurityConfig.config; },
    set config(value: any) { HTTPSecurityConfig.config = value; },
    get stats() { return HTTPSecurityConfig.stats; },
    set stats(value: any) { HTTPSecurityConfig.stats = value; },
    get capabilities() { return HTTPSecurityConfig.capabilities; },
    set capabilities(value: any) { HTTPSecurityConfig.capabilities = value; },
    get effectiveState() { return HTTPSecurityConfig.effectiveState; },
    set effectiveState(value: any) { HTTPSecurityConfig.effectiveState = value; },
    get functions() { return HTTPSecurityConfig.functions; },
    set functions(value: any) { HTTPSecurityConfig.functions = value; },
    get tabs() { return HTTPSecurityConfig.tabs; },
    set tabs(value: any) { HTTPSecurityConfig.tabs = value; },
    get headerManagerTab() { return HTTPSecurityConfig.headerManagerTab; },
    set headerManagerTab(value: any) { HTTPSecurityConfig.headerManagerTab = value; },
    get icons() { return HTTPSecurityConfig.icons; },
    set icons(value: any) { HTTPSecurityConfig.icons = value; }
};
const HTTPSecurityConfigService: AnyRecord = {
...
const HTTPSecurityConfigView: AnyRecord = {
    ...
    renderOverviewStat: (label: any, value: any, color: any) => HTTPSecurityConfig.renderOverviewStat(label, value, color),
    renderModuleCard: (mod: any) => HTTPSecurityConfig.renderModuleCard(mod),
...
const HTTPSecurityConfigController: AnyRecord = {
    dispose: () => disposeHTTPSecurityBindings(),
    updateField: (path: any, value: any) => HTTPSecurityConfig.updateField(path, value),
    getField: (path: any, fallback: any) => HTTPSecurityConfig.getField(path, fallback),
    switchTab: (id: any) => HTTPSecurityConfig.switchTab(id),
    setHeaderTab: (tab: any) => HTTPSecurityConfig.setHeaderTab(tab),
    addTag: (path: any, value: any) => HTTPSecurityConfig.addTag(path, value),
    removeTag: (path: any, value: any) => HTTPSecurityConfig.removeTag(path, value),
    toggleArrayItem: (path: any, value: any) => HTTPSecurityConfig.toggleArrayItem(path, value),
    toggleMethodPill: (method: any) => HTTPSecurityConfig.toggleMethodPill(method),
    promptAddHost: () => HTTPSecurityConfig.promptAddHost(),
    parseHeaderMap: (path: any, text: any) => HTTPSecurityConfig.updateField(path, parseHTTPSecurityHeaderMap(text))
};
...
export const HTTPSecurityConfigFacade: any = new Proxy({}, {
