...
import { TrafficTrustedException } from './types.js';

export function renderPriorityTab(
    trustedExceptions: any[] | undefined,
        ...
        <div class="flow-priority-console">
            <section class="flow-priority-inventory">
                ...
                <div class="flow-priority-list">
                    ${exceptions.length ? exceptions.map((exception: any) => {
                        const targets = exception.targets || [];
                        const target = targets.join(', ');
                        const isCidr = targets.some((value: string) => value.includes('/'));
