...
interface TLSConfigState {
  ...
  cache_path?: string;
  renew_before_days?: number | null;
  renewal?: TLSRenewalStatus;
  warnings?: string[];
  recommendations?: string[];
  automation_summary?: string;
  suggested_domains?: string[];
  listener_mode?: 'http' | 'https';
  restart_required?: boolean;
}

interface TLSMutationResponse {
  status: string;
  restart_required?: boolean;
  warnings?: string[];
}
...
function encodeSslValue(value: string): string {
  return encodeURIComponent(value);
}

function decodeSslValue(value: string | undefined): string {
...
export async function loadSSLPage(): Promise<void> {
  ensureSSLBindings();
  ...
  try {
    const [certsRes, configRes] = await Promise.all([
      api.get<CertificateRecord[]>('certificates'),
      api.get<TLSConfigState>('certificates/config')
    ]);
...
function renderSSLPage(container: HTMLElement): void {
  if (!sslState.tlsConfig.domains) sslState.tlsConfig.domains = [];
  const content = `
...
function renderDomainManager(): string {
...
function renderCertificatesList(): string {
...
function sslIcon(name: 'automation' | 'certificate' | 'upload'): string {
  if (name === 'automation') {
...
function ensureSSLBindings(): void {
  if (sslBindingsInitialized) return;
  sslBindingsInitialized = true;

  AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '#auto-tls-toggle', (_event, target) => {
    toggleAutoTLS(target.checked);
  });
  AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '#tls-enabled-toggle', (_event, target) => {
    sslState.tlsConfig.enabled = target.checked;
    markTLSConfigDirty();
    const container = AdminDOM.getById('ssl-container');
    if (container) renderSSLPage(container);
  });
  AdminEvents.delegateEvent<HTMLInputElement>(document, 'input', '#acme-email', (_event, target) => {
    sslState.tlsConfig.email = target.value;
    markTLSConfigDirty();
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '[data-ssl-action]', (_event, target) => {
    ...
    switch (action) {
      ...
      case 'save-tls-config':
        if (!sslConfigDirty || sslSaveInFlight) break;
      ...
      case 'reset-tls-config':
        if (sslConfigDirty && !sslSaveInFlight) void loadSSLPage();
        break;
      ...
      case 'upload-certificate':
        ...
        break;
      case 'delete-cert':
        if (certId) void deleteCert(certId);
        break;
      default:
        break;
    }
  ...
  AdminEvents.delegateEvent<HTMLInputElement>(document, 'keydown', '#domain-input', event => {
    if (event instanceof KeyboardEvent) handleDomainInput(event);
  });
}

function toggleAutoTLS(checked: boolean): void {
  const email = AdminDOM.getInput('acme-email');
  if (email) sslState.tlsConfig.email = email.value;
...
function handleDomainInput(event: KeyboardEvent): void {
  if (event.key !== 'Enter') return;
  event.preventDefault();
  addDomainFromInput();
}

function addDomainFromInput(): void {
  const input = AdminDOM.getInput('domain-input');
  if (!input) return;
  if (addDomain(input.value)) input.value = '';
}

function addDomain(value: string): boolean {
...
function markTLSConfigDirty(): void {
  sslConfigDirty = true;
  SectionUI.markSaveActionBarDirty('data-ssl-action');
}

async function saveTLSConfig(): Promise<void> {
  const emailEl = AdminDOM.getInput('acme-email');
  if (emailEl) sslState.tlsConfig.email = emailEl.value.trim();
  ...
  const result = await api.requestResult<TLSMutationResponse>('certificates/config', 'PUT', sslState.tlsConfig);
  if (result.error) {
  ...
  sslShowToast(
    result.data?.restart_required ? 'Configuration saved. Restart Aegis to apply it.' : 'TLS configuration applied.',
    result.data?.restart_required ? 'warning' : 'success'
  );
  await loadSSLPage();
}

export function showUploadCertModal(): void {
  const content = `
...
async function uploadCertificate(): Promise<void> {
  ...
  try {
    ...
    const responseData = await response.json().catch(() => ({} as TLSMutationResponse & { error?: string }));
    if (response.ok && (responseData.status === 'activated' || responseData.status === 'staged')) {
      ...
      sslShowToast(
        `${responseData.restart_required ? 'Certificate staged. Restart Aegis to activate it.' : 'Certificate validated and activated.'}${warningSuffix}`,
        responseData.restart_required || responseData.warnings?.length ? 'warning' : 'success'
      );
      await loadSSLPage();
      return;
    }
...
async function confirmDeleteCert(id: string): Promise<void> {
  SectionUI.closeModal();
  const result = await api.requestResult<TLSMutationResponse>('certificates', 'DELETE', { id });
  if (result.error) {
    sslShowToast(result.error.message, 'error');
    return;
  }
  ...
  sslShowToast(
    result.data.warnings?.length ? result.data.warnings.join(' ') : 'Inactive certificate deleted',
...
function certificateStatusLabel(cert: CertificateRecord): string {
  if (cert.status === 'not_yet_valid') return 'Not yet valid';
  if (cert.status === 'expired' || cert.valid === false) return 'Expired';
  if (cert.status === 'expiring') return 'Expiring';
  if (cert.active) return 'Active';
  if (cert.auto_renew) return 'Auto-renewed';
  return 'Valid';
}

function sslShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}
