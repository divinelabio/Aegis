...
import { loadAppsForRouting, type AppV2 } from './sections/edge-access/v2/apps-contract.js';
...
const routesState = {
  routesData: [] as RouteRecord[],
  protectedApps: [] as AppV2[],
  protectedAppsByRoute: new Map<string, AppV2>(),
  protectedAppsAvailable: true,
  originGroups: [] as OriginGroup[],
  editingRoute: null as RouteRecord | null,
  eventsBound: false
};

export async function loadRoutesTable(): Promise<void> {
  bindRoutesEvents();
  ...
  try {
    ...
    const [data, protectedApps, originGroups] = await Promise.all([
      api.get<RouteRecord[] | RouteListResponse>('routes'),
      hasAccessControl
        ...
        : Promise.resolve([] as AppV2[]),
      api.get<OriginGroup[]>('upstreams').catch(error => {
        console.warn('Failed to load Origins for Routing', error);
        return [] as OriginGroup[];
      })
    ]);
    if (!data) {
...
function routePaths(route: RouteRecord): string[] {
  return route.match?.paths?.length ? route.match.paths : (route.paths || []);
}

function routeBackends(route: RouteRecord): RouteBackend[] {
  return route.action?.backends?.length ? route.action.backends : (route.backends || []);
}

function routeForWrite(route: RouteRecord | null): RouteRecord {
  if (!route) return {};
  return {
    id: route.id,
    name: route.name,
...
function routeStripPrefix(route: RouteRecord): string {
...
function routeInlineStrategy(route: RouteRecord | null): string {
  if (!route) return 'round_robin';
  if (route.action?.strategy && inlineStrategies.has(route.action.strategy)) return route.action.strategy;
  if (route.load_balance && inlineStrategies.has(route.load_balance)) return route.load_balance;
...
function routeNamedUpstream(route: RouteRecord | null): string {
  const upstream = route?.action?.upstream || '';
  return inlineStrategies.has(upstream) ? '' : upstream;
}

function formatMatcherMap(value?: Record<string, string[]>): string {
  return value && Object.keys(value).length > 0 ? JSON.stringify(value, null, 2) : '';
}

type RouteDeliveryMode = 'origin' | 'inline' | 'default';

function routeDeliveryMode(route: RouteRecord | null): RouteDeliveryMode {
  if (routeNamedUpstream(route)) return 'origin';
  if (route && routeBackends(route).length > 0) return 'inline';
  return route ? 'default' : (routesState.originGroups.length > 0 ? 'origin' : 'inline');
}

function renderOriginOptions(selectedOrigin: string): string {
...
type RouteAccessTone = 'success' | 'warning' | 'danger' | 'neutral' | 'primary';

interface RouteAccessInfo {
  label: 'Public' | 'Protected' | 'Disabled' | 'Misconfigured' | 'Unknown';
  tone: RouteAccessTone;
  app: AppV2 | null;
  description: string;
}

function routeProtectedApp(route: RouteRecord): AppV2 | null {
  if (!route.id) return null;
  return routesState.protectedAppsByRoute.get(route.id) || null;
}

function routeAccessInfo(route: RouteRecord): RouteAccessInfo {
...
function renderRoutingEnterpriseRows(): string[][] {
  const hasAccessControl = api.hasFeature(FEATURES.ACCESS_CONTROL);
  return routesState.routesData.map((route, index) => {
    ...
    const items = [
      ...
      { label: 'Delete route', attrs: `data-action="routes-delete" data-route-id="${routeID}"`, tone: 'danger' as const }
    ];
...
function renderRoutesTable(container: HTMLElement): void {
  const hasAccessControl = api.hasFeature(FEATURES.ACCESS_CONTROL);
...
function renderRouteEditor(container: HTMLElement, route: RouteRecord | null = null): void {
    ...
    const hasAdvancedSettings = Boolean(
      methods || sourceCIDRs || headerMatchers || queryMatchers ||
    ...
    const selected = (value: string): string => loadBalance === value ? ' selected' : '';
...
function syncRouteEditorControls(): void {
  const deliveryMode = AdminDOM.selectValue('route-delivery-mode', 'origin') as RouteDeliveryMode;
  ...
  if (originDelivery) originDelivery.hidden = deliveryMode !== 'origin';
  if (defaultDelivery) defaultDelivery.hidden = deliveryMode !== 'default';
  if (inlineDelivery) inlineDelivery.hidden = deliveryMode !== 'inline';
  if (fallbackDelivery) fallbackDelivery.hidden = deliveryMode !== 'origin';
  ...
  if (stripField) stripField.hidden = pathTransform !== 'strip';
  if (rewriteField) rewriteField.hidden = pathTransform !== 'rewrite';
}

function routeIcon(name: 'route' | 'add'): string {
  if (name === 'add') {
...
function bindRoutesEvents(): void {
  if (routesState.eventsBound) return;
  routesState.eventsBound = true;

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-open-editor"]', event => {
    event.preventDefault();
    const container = AdminDOM.getById('routes-container');
    if (container) renderRouteEditor(container);
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-edit"]', (event, target) => {
    event.preventDefault();
    const routeId = target.dataset.routeId;
    if (routeId) editRoute(routeId);
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-delete"]', (event, target) => {
    event.preventDefault();
    const routeId = target.dataset.routeId;
    if (routeId) void deleteRoute(routeId);
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-protect-access"]', (event, target) => {
    event.preventDefault();
    if (!api.hasFeature(FEATURES.ACCESS_CONTROL)) return;
    const routeId = target.dataset.routeId;
    if (routeId) void openEdgeAccessForRoute(routeId);
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-close-editor"]', event => {
    event.preventDefault();
    closeRouteEditor();
  });

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '#routes-container [data-action="routes-toggle"]', (event, target) => {
    event.preventDefault();
    const routeId = target.dataset.routeId;
    if (!routeId) return;
  ...
  AdminEvents.delegateEvent<HTMLSelectElement>(document, 'change', '#routes-container #route-delivery-mode, #routes-container #route-path-transform', () => {
    syncRouteEditorControls();
  });

  AdminEvents.delegateEvent<HTMLFormElement>(document, 'submit', '#routes-container #route-form', event => {
    void saveRoute(event as SubmitEvent);
  });
...
function parseMatcherMap(value: string, label: string): Record<string, string[]> {
  ...
  for (const [name, allowed] of Object.entries(parsed as Record<string, unknown>)) {
...
function parseRouteBackends(value: string): RouteBackend[] {
  return value
    .split('\n')
    .map(line => line.trim())
    .filter(Boolean)
...
async function saveRoute(event: SubmitEvent): Promise<void> {
  event.preventDefault();
...
async function toggleRoute(id: string, enabled: boolean): Promise<void> {
  const route = routesState.routesData.find(routeItem => routeItem.id === id);
  if (!route) return;
...
function renderRouteAccessContext(route: RouteRecord): string {
  const access = routeAccessInfo(route);
...
async function openEdgeAccessForRoute(routeId: string): Promise<void> {
  if (!api.hasFeature(FEATURES.ACCESS_CONTROL)) return;
  const runtimeRouter = (window as unknown as { router?: { navigate?: (target: string) => void } }).router;
...
async function deleteRoute(id: string): Promise<void> {
  SectionUI.openConfirmModal(
    'Delete Route',
    `Are you sure you want to delete this route (<strong>${escapeHTML(id)}</strong>)?<br>Rules are processed in priority order. Linked Edge Access routes cannot be deleted.`,
    'Delete',
    'var(--danger)',
    () => {
...
async function confirmDeleteRoute(id: string): Promise<void> {
...
function routesShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}
