...
export const router: RouterApi = {
  ...
  views: {
    ...
    access_sessions: `
        ...
        `,
    bot_management: featurePageView(
      'Bot Management',
      'Manage bot scoring, challenges, known bots, and fingerprinting during the unified migration.',
      'antibots_config'
    ),
    ddos_protection: featurePageView(
      'DDoS Protection',
      'Operate rate limits, traffic throttling, and IP controls in Traffic Control.',
      'traffic_config'
    ),
    api_shield: featurePageView(
      'API Shield',
      'Centralize API schema, BOLA, and GraphQL controls while preserving API Security configuration.',
      'apisecurity_config'
    ),
    waf_config: `
            <div class="view view-operator">
  ...
  navigate(target: string, sourceLink?: Element | null, options: { replace?: boolean } = {}): void {
    ...
    const viewGroupMap: Record<string, { group: string; containerId: string }> = {
    ...
    if (target === 'security_analytics') void initSecurityAnalytics();
    ...
    if (target === 'waf') void loadPoliciesTable();
    ...
    if (target === 'module_reputation') void ReputationConfig.init();
    const httpsecTabs: Record<string, string> = {
      httpsecurity_config: 'overview',
    ...
    const apisecTabs: Record<string, string> = {
      apisecurity_config: 'overview',
      api_schema: 'contract',
      api_owasp: 'authorization',
    ...
    const antibotTabs: Record<string, string> = {
      antibots_config: 'overview',
      bot_classification: 'classification',
    ...
    const accessTabs: Record<string, string> = {
      access_config: 'apps',
      access_apps: 'apps',
    ...
    if (target === 'modules') {
      ...
      if (container) container.innerHTML = String(ModuleDashboard.render() || '');
    }
    if (target === 'module_captcha') void CaptchaConfig.init();
    if (target === 'module_challenge') void ChallengeConfig.init();

    if (target === 'routes') void loadRoutesTable();
    if (target === 'proxy_settings') void loadProxySettingsPage();
    if (target === 'settings') void loadSettingsPage();
    if (target === 'logs') void loadThreatLogsPage();
    if (target === 'upstream') void loadUpstreamsPage();
    if (target === 'ssl') void loadSSLPage();
    if (target === 'users') void loadUsersPage();
    if (target === 'profile') void loadProfilePage();
    if (target === 'audit') void loadAuditLogsPage();
  }
};

(globalThis as unknown as Record<string, unknown>).router = router;
