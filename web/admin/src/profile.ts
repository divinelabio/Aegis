...
const profileState = {
  mfaWizardStep: 0,
  mfaSecretData: null as MFASecretData | null
};

function bindProfileInteractions(): void {
  if ((bindProfileInteractions as { initialized?: boolean }).initialized) return;
  (bindProfileInteractions as { initialized?: boolean }).initialized = true;
...
export async function loadProfilePage(): Promise<void> {
...
async function loadProfileDetails(): Promise<ProfileDetails | null> {
  try {
...
function renderIcon(name: 'shield' | 'check' | 'alert'): string {
  if (name === 'check') {
...
async function startMFAFlow(): Promise<void> {
  profileState.mfaWizardStep = -1;
  renderMFAWizard();

  try {
    const result = await api.requestResult<MFASecretData>('mfa/setup', 'POST');
    if (result.error || !result.data) throw new Error(result.error?.message || 'Failed to generate secret');
...
function checkAutoSubmit(event: KeyboardEvent): void {
  if (event.key === 'Enter') {
    void enableMFA();
  }
}
...
async function enableMFA(): Promise<void> {
  const input = AdminDOM.getInput('mfa-verify-code');
  const button = AdminDOM.getById<HTMLButtonElement>('btn-enable-mfa');
  ...
  const result = await api.requestResult<{ reauth_required?: boolean }>('mfa/enable', 'POST', { code });
  if (!result.error) {
...
async function disableMFA(): Promise<void> {
  if (!window.confirm('Disable login verification for this account?')) return;
  ...
  const result = await api.requestResult<{ reauth_required?: boolean }>('mfa/disable', 'POST', { code });
  if (result.error) {
...
async function updateProfilePassword(): Promise<void> {
  ...
  const result = await api.requestResult<{ reauth_required?: boolean }>('profile', 'PUT', { password: pass });
  if (result.error) {
    profileShowToast(result.error.message, 'error');
    return;
  }
...
function profileGetRoleName(id?: number | string): string {
...
function profileShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}
