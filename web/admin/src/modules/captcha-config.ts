...
export interface CaptchaModuleConfig {
  ...
  secret_key_migration_required?: boolean;
  score_threshold?: number;
  [key: string]: unknown;
}

interface CaptchaModuleApiResponse {
  error?: string;
  message?: string;
  detail?: string;
  [key: string]: unknown;
...
type CaptchaProviderId = 'recaptcha_v2' | 'recaptcha_v3' | 'turnstile' | 'hcaptcha' | 'mtcaptcha' | 'friendlycaptcha';

interface CaptchaProviderOption {
  id: CaptchaProviderId;
  name: string;
  desc: string;
}

function captchaCfgIsRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

function createDefaultCaptchaConfig(): CaptchaModuleConfig {
  return {
    enabled: false,
...
function asCaptchaModuleConfig(value: unknown): CaptchaModuleConfig {
  if (!captchaCfgIsRecord(value)) return createDefaultCaptchaConfig();
  return { ...createDefaultCaptchaConfig(), ...value };
}

async function captchaCfgParseJson<T>(response: Response): Promise<T | null> {
  try {
...
function captchaCfgErrorMessage(data: CaptchaModuleApiResponse | null, fallback: string): string {
...
function captchaCfgEscapeAttribute(value: unknown): string {
  return String(value ?? '')
    .replace(/&/g, '&amp;')
    .replace(/"/g, '&quot;')
    .replace(/</g, '&lt;')
...
const captchaProviders: CaptchaProviderOption[] = [
...
const captchaProviderLogos: Record<CaptchaProviderId, string> = {
  recaptcha_v2: `
...
export const CaptchaConfig = {
  config: createDefaultCaptchaConfig() as CaptchaModuleConfig,
  eventsBound: false,
  ...
  bindEvents(): void {
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#captcha-config-content [data-action="captcha-select-provider"]',
      (_event, target) => {
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#captcha-config-content [data-action="captcha-save"]',
      event => {
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#captcha-config-content [data-action="captcha-reset"]',
      event => {
        event.preventDefault();
    ...
    AdminEvents.delegateEvent<HTMLInputElement>(
      document,
      'input',
      '#captcha-config-content [data-action="captcha-update-text"]',
      (_event, target) => {
    ...
    AdminEvents.delegateEvent<HTMLInputElement>(
      document,
      'change',
      '#captcha-config-content [data-action="captcha-toggle-enabled"]',
      (_event, target) => {
        this.updateField('enabled', target.checked);
        const status = AdminDOM.query<HTMLElement>('#captcha-config-content [data-captcha-section-status]');
        if (status) {
    ...
    AdminEvents.delegateEvent<HTMLInputElement>(
      document,
      'input',
      '#captcha-config-content [data-action="captcha-update-score"]',
      (_event, target) => {
  ...
  async loadConfig(): Promise<void> {
    try {
      ...
      const data = await captchaCfgParseJson<CaptchaModuleConfig & CaptchaModuleApiResponse>(response);
      if (!response.ok) {
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      const data = await captchaCfgParseJson<CaptchaModuleApiResponse>(response);
      if (!response.ok) {
