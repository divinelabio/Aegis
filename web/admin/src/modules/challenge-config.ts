...
import type { CaptchaModuleConfig } from './captcha-config.js';
import { notify } from '../core/notify.js';
import { SectionUI } from '../sections/ui-components.js';

type InteractiveVerificationMethod = 'hold' | 'click' | 'slide' | 'sequence' | 'match';

const INTERACTIVE_VERIFICATION_METHODS: ReadonlyArray<{
  id: InteractiveVerificationMethod;
  label: string;
}> = [
...
interface ChallengeModuleConfig {
  enabled?: boolean;
  theme?: string;
  custom_title?: string;
  ...
  interactive_style?: InteractiveVerificationMethod;
...
interface ChallengeModuleApiResponse {
  error?: string;
  message?: string;
  detail?: string;
  [key: string]: unknown;
...
function challengeCfgIsRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

function createDefaultChallengeConfig(): ChallengeModuleConfig {
  return {
    enabled: true,
    theme: 'dark',
...
function createDefaultCaptchaConfigForChallenge(): CaptchaModuleConfig {
  return {
    enabled: false,
...
function asChallengeModuleConfig(value: unknown): ChallengeModuleConfig {
  if (!challengeCfgIsRecord(value)) return createDefaultChallengeConfig();
  return {
...
function asChallengeCaptchaConfig(value: unknown): CaptchaModuleConfig {
  if (!challengeCfgIsRecord(value)) return createDefaultCaptchaConfigForChallenge();
  return { ...createDefaultCaptchaConfigForChallenge(), ...value };
}

async function challengeCfgParseJson<T>(response: Response): Promise<T | null> {
  try {
...
function challengeCfgErrorMessage(data: ChallengeModuleApiResponse | null, fallback: string): string {
...
function challengeCfgEscapeHTML(value: unknown): string {
  return String(value ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function normalizeInteractiveVerificationMethod(value: unknown): InteractiveVerificationMethod {
  return value === 'click' || value === 'slide' || value === 'sequence' || value === 'match' ? value : 'hold';
}

function shuffleChallengePreviewValues<T>(values: readonly T[]): T[] {
...
function createSequencePreview(): { sequence: number[]; choices: number[] } {
...
function createSymbolPreview(): { target: string; choices: string[] } {
...
function challengeShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}

export const ChallengeConfig = {
  config: createDefaultChallengeConfig() as ChallengeModuleConfig,
  captchaConfig: null as CaptchaModuleConfig | null,
  eventsBound: false,
  ...
  bindEvents(): void {
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#challenge-config-content [data-action="challenge-set-theme"]',
      (event, target) => {
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#challenge-config-content [data-action="challenge-save"]',
      event => {
        event.preventDefault();
        void this.save();
      }
    ...
    AdminEvents.delegateEvent<HTMLElement>(
      document,
      'click',
      '#challenge-config-content [data-action="challenge-reset"]',
      event => {
        event.preventDefault();
    ...
    AdminEvents.delegateEvent<HTMLInputElement>(
      document,
      'input',
      '#challenge-config-content [data-action="challenge-update-preview"]',
      (_event, target) => {
    ...
    AdminEvents.delegateEvent<HTMLTextAreaElement>(
      document,
      'input',
      '#challenge-config-content textarea[data-action="challenge-update-preview"]',
      (_event, target) => {
        const field = target.dataset.field;
        if (!field) return;
        this.updateField(field, target.value);
    ...
    AdminEvents.delegateEvent<HTMLSelectElement>(
      document,
      'change',
      '#challenge-config-content [data-action="challenge-update-mode"]',
      (_event, target) => {
    ...
    AdminEvents.delegateEvent<HTMLInputElement | HTMLSelectElement>(
      document,
      'change',
      '#challenge-config-content [data-action="challenge-update-field"]',
      (_event, target) => {
        const field = target.dataset.field;
        if (!field) return;
    ...
    AdminEvents.delegateEvent<HTMLInputElement | HTMLSelectElement>(
      document,
      'change',
      '#challenge-config-content [data-action="challenge-update-cookie-ttl"]',
      (_event, target) => {
  ...
  async loadConfig(): Promise<void> {
    try {
      ...
      const [challengeData, captchaData] = await Promise.all([
        challengeCfgParseJson<ChallengeModuleConfig & ChallengeModuleApiResponse>(challengeResponse),
        challengeCfgParseJson<CaptchaModuleConfig & ChallengeModuleApiResponse>(captchaResponse)
      ]);
  ...
  renderChallengePreview(
    config: ChallengeModuleConfig,
    titleThemeClass: string,
    msgThemeClass: string
  ): string {
    const mode = String(config.mode || 'managed');
  ...
  bindPreviewInteractions(): void {
    const preview = AdminDOM.getById('preview-container');
    if (!preview) return;

    const holdControl = preview.querySelector<HTMLButtonElement>('[data-preview-action="hold"]');
    if (holdControl) {
      const primary = holdControl.querySelector<HTMLElement>('[data-preview-primary]');
      const secondary = holdControl.querySelector<HTMLElement>('[data-preview-secondary]');
      let holdTimer = 0;
      ...
      const resetHold = (): void => {
        ...
        if (primary) primary.textContent = 'Hold to verify';
        if (secondary) secondary.textContent = 'Press and hold to continue';
      };
      ...
      const completeHold = (): void => {
        ...
        if (primary) primary.textContent = 'Verified';
        if (secondary) secondary.textContent = 'Preview complete';
      };

      const startHold = (): void => {
        resetHold();
        holdControl.classList.add('is-holding');
        if (secondary) secondary.textContent = 'Keep holding…';
        holdTimer = window.setTimeout(completeHold, 1100);
      };

      const cancelHold = (): void => {
        if (!holdControl.classList.contains('is-complete')) resetHold();
      };
    ...
    const clickControl = preview.querySelector<HTMLButtonElement>('[data-preview-action="click"]');
    if (clickControl) {
      const primary = clickControl.querySelector<HTMLElement>('[data-preview-primary]');
      const secondary = clickControl.querySelector<HTMLElement>('[data-preview-secondary]');
      clickControl.addEventListener('click', () => {
        ...
        if (primary) primary.textContent = complete ? 'Verified' : 'Verify you are human';
        if (secondary) secondary.textContent = complete ? 'Preview complete' : 'Click to continue';
      });
    }

    const slideControl = preview.querySelector<HTMLElement>('[data-preview-action="slide"]');
    if (slideControl) {
      const thumb = slideControl.querySelector<HTMLElement>('.preview-slide-thumb');
      const label = slideControl.querySelector<HTMLElement>('[data-preview-primary]');
      let dragging = false;
      let progress = 0;

      const paintSlide = (nextProgress: number): void => {
      ...
      const resetSlide = (): void => {
        ...
        if (label) label.textContent = 'Slide to verify';
        paintSlide(0);
      };

      const completeSlide = (): void => {
        dragging = false;
      ...
      const updateSlideFromPointer = (event: PointerEvent): void => {
      ...
      slideControl.addEventListener('pointerdown', event => {
        event.preventDefault();
        if (slideControl.classList.contains('is-complete')) resetSlide();
        dragging = true;
      ...
      slideControl.addEventListener('pointermove', event => {
        if (dragging) updateSlideFromPointer(event);
      });
      slideControl.addEventListener('pointerup', () => {
        if (!dragging) return;
        if (progress >= 88) completeSlide();
        else resetSlide();
      });
      slideControl.addEventListener('pointercancel', resetSlide);
      slideControl.addEventListener('keydown', event => {
        let nextProgress = progress;
        if (event.key === 'ArrowRight' || event.key === 'ArrowUp') nextProgress += 10;
        else if (event.key === 'ArrowLeft' || event.key === 'ArrowDown') nextProgress -= 10;
        ...
        if (nextProgress >= 88) completeSlide();
        else {
          if (label) label.textContent = 'Slide to verify';
    ...
    const sequenceControl = preview.querySelector<HTMLElement>('[data-preview-action="sequence"]');
    if (sequenceControl) {
      const sequence = String(sequenceControl.dataset.previewSequence || '')
        .split(',')
        .map(value => value.trim())
        .filter(Boolean);
      const label = sequenceControl.querySelector<HTMLElement>('.preview-method-label');
      const buttons = Array.from(sequenceControl.querySelectorAll<HTMLButtonElement>('[data-preview-value]'));
    ...
    const matchControl = preview.querySelector<HTMLElement>('[data-preview-action="match"]');
    if (matchControl) {
      const target = String(matchControl.dataset.previewTarget || '');
      const prompt = matchControl.querySelector<HTMLElement>('.preview-match-prompt');
      const buttons = Array.from(matchControl.querySelectorAll<HTMLButtonElement>('[data-preview-value]'));
      ...
      const resetMatch = (): void => {
        ...
        buttons.forEach(button => button.classList.remove('is-selected', 'is-wrong'));
        if (prompt) prompt.innerHTML = defaultPrompt;
      };

      buttons.forEach(button => {
        button.addEventListener('click', () => {
          if (matchControl.classList.contains('is-complete')) resetMatch();
          if (String(button.dataset.previewValue || '') === target) {
            button.classList.add('is-selected');
            matchControl.classList.add('is-complete');
            if (prompt) prompt.textContent = 'Verified';
            return;
          }

          matchControl.classList.add('is-wrong');
          button.classList.add('is-wrong');
          if (prompt) prompt.textContent = 'Try another symbol';
  ...
  applyModeDefaults(mode: string): void {
  ...
  normalizeForAvailableProviders(): void {
    if (this.isCaptchaConfigured()) return;
  ...
  updatePreview(): void {
    ...
    if (title) title.textContent = String(this.config.custom_title || 'Checking your browser...');
    if (message) message.textContent = String(this.config.custom_message || 'Please wait a moment while we verify your request.');
  },
  ...
  async save(): Promise<void> {
    try {
      ...
      const data = await challengeCfgParseJson<ChallengeModuleApiResponse>(response);
      if (!response.ok) {
