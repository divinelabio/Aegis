...
type SetupStorageStatus = {
  control: SetupStorageRuntime;
  analytics: SetupStorageRuntime;
};
...
type PendingStorageResponse = {
  status: 'pending';
  restart_required: true;
};

type SetupFirstAdminResponse = {
  status: 'created';
  id: string;
  username: string;
  role_id?: string;
...
let wizardState: SetupWizardState | null = null;
let activeStep = 0;
...
export async function maybeShowSetupWizard(options: { firstRunOnly?: boolean } = {}): Promise<boolean> {
  if (setupStatusRequested || wizardState) return Boolean(wizardState);
  setupStatusRequested = true;

  const status = await fetchJSON<SetupStatusResponse>('/api/setup/status');
  ...
  const bootstrap = await fetchJSON<SetupBootstrapResponse>('/api/setup/bootstrap');
...
async function requestPublicSetupResult<T>(
  endpoint: string,
  method: string,
  data?: unknown
  ...
  try {
    ...
    const payload = await parseResponseJSON<Record<string, unknown>>(response);
    if (!response.ok) {
      return {
        data: null,
        error: {
...
function setupRequestResult<T>(
  authenticatedEndpoint: string,
  firstRunEndpoint: string,
  method: string,
  data?: unknown
): Promise<ApiRequestResult<T>> {
  if (setupFirstRunPublicMode()) {
    return requestPublicSetupResult<T>(firstRunEndpoint, method, data);
  }
  return api.requestResult<T>(authenticatedEndpoint, method, data);
}
...
function setupErrorCode(payload: Record<string, unknown> | null): string | undefined {
...
function setupErrorMessage(payload: Record<string, unknown> | null, fallback: string): string {
  const error = payload?.error;
...
function ensureSetupBindings(): void {
  if (setupBindingsReady) return;
...
async function handleSetupAction(action: string): Promise<void> {
  if (!wizardState || wizardBusy) return;
...
function renderSetupWizard(): void {
  ...
  const primaryDisabled = primaryAction === 'next' && (
    (activeStep === 2 && !postgreSQLReadyForSetup())
  ...
  window.setTimeout(() => {
    const card = AdminDOM.query<HTMLElement>('.setup-wizard-card', root);
    const body = AdminDOM.query<HTMLElement>('.setup-wizard-body', root);
    card?.scrollIntoView({ block: 'start' });
    body?.scrollTo({ top: 0 });
    const focusTarget = AdminDOM.query<HTMLElement>('[data-setup-autofocus]', root)
      || AdminDOM.query<HTMLElement>('.setup-next', root);
    try {
...
function ensureSetupRoot(): HTMLElement {
...
function renderActiveStep(): string {
  switch (activeStep) {
    case 0:
...
function renderThemeStep(): string {
...
function renderThemeCard(value: AdminTheme, title: string, description: string, activeTheme: AdminTheme): string {
...
function renderDatabaseStep(): string {
...
function renderSetupUserRow(user: SetupUser, currentUserId: string): string {
...
function renderCreateAdminPanel(canCreate: boolean): string {
  return `
...
function renderCurrentPasswordPanel(): string {
  return `
...
async function activateSetupLicense(): Promise<void> {
  const input = document.getElementById('setup-license-key') as HTMLInputElement | null;
  const feedback = document.getElementById('setup-license-feedback') as HTMLElement | null;
  const button = document.getElementById('btn-setup-activate-license') as HTMLButtonElement | null;
  ...
  try {
    const res = await api.requestResult<{ license?: { effective_tier?: string; licensed_tier?: string; status?: string; expires_at?: string; offline_until?: string } }>('license/activate', 'POST', { key });
    if (res.error) {
...
function renderProFeature(title: string, description: string): string {
  return `
...
function renderStorageSettingsField(
  label: string,
  id: string,
  value: string,
  disabled = '',
...
function renderClickHouseWarningModal(): string {
  return `
...
function storageStatusText(status: SetupStorageRuntime, emptyMessage: string): string {
  if (status.restart_required) return 'Pending configuration is saved. Restart Aegis to apply it.';
  if (status.available) return 'Active connection is healthy.';
  if (status.mode === 'disabled') return 'Analytics is disabled.';
  if (status.configured && status.active) return 'Active connection is currently unavailable.';
  if (status.environment_locked) return 'Configuration is managed outside Aegis.';
  return emptyMessage;
}

async function testPostgreSQLConnection(): Promise<void> {
  const input = postgreSQLInput();
  if (!validatePostgreSQLInput(input, true)) return;
  const passwordInput = AdminDOM.getInput('setup-postgresql-password');
  setWizardBusy(true);
  Toast.show('Testing PostgreSQL connection...', 'info');
  try {
    const result = await setupRequestResult<PostgreSQLPreflightResponse>(
      'storage/control/postgresql/preflight',
      'setup/storage/control/postgresql/preflight',
      'POST',
      {
        host: input.host,
        port: input.port,
...
async function savePostgreSQLConfiguration(): Promise<void> {
  const input = postgreSQLInput();
  if (!validatePostgreSQLInput(input, false)) return;
  setWizardBusy(true);
  try {
    const result = await setupRequestResult<PendingStorageResponse>(
      'storage/control/postgresql/pending',
      'setup/storage/control/postgresql/pending',
      'POST',
      {
        host: input.host,
        port: input.port,
...
async function testClickHouseConnection(): Promise<void> {
  ...
  if (!validateClickHouseInput(input, true)) return;
  const passwordInput = AdminDOM.getInput('setup-clickhouse-password');
  setWizardBusy(true);
  Toast.show('Testing ClickHouse connection...', 'info');
  try {
    const result = await setupRequestResult<PostgreSQLPreflightResponse>(
      'storage/analytics/clickhouse/preflight',
      'setup/storage/analytics/clickhouse/preflight',
      'POST',
      {
        host: input.host,
        port: input.port,
...
async function saveClickHouseConfiguration(): Promise<void> {
  const input = clickHouseInput();
  if (!validateClickHouseInput(input, false)) return;
  setWizardBusy(true);
  try {
    const result = await setupRequestResult<PendingStorageResponse>(
      'storage/analytics/clickhouse/pending',
      'setup/storage/analytics/clickhouse/pending',
      'POST',
      {
        mode: input.mode,
        host: input.host,
...
async function createAdminAccount(): Promise<void> {
  ...
  try {
    ...
    const result = firstRun
      ? await requestPublicSetupResult<SetupFirstAdminResponse>('setup/first-admin', 'POST', { username, email, password })
      : await api.requestResult<SetupFirstAdminResponse>('users', 'POST', {
        username,
        email,
        password,
        role_id: role.id
...
async function updateCurrentAdminPassword(): Promise<void> {
  ...
  setWizardBusy(true);
  try {
    const result = await api.requestResult<{ reauth_required?: boolean }>('profile', 'PUT', { password });
    if (result.error) {
...
async function finishSetupWizard(): Promise<void> {
  setWizardBusy(true);
  try {
    const result = await api.requestResult<SetupCompleteResponse>('setup/complete', 'POST', {});
...
async function refreshSetupRuntimeState(): Promise<void> {
  if (!wizardState) return;
  const [status, bootstrap] = await Promise.all([
    fetchJSON<SetupStatusResponse>('/api/setup/status'),
    fetchJSON<SetupBootstrapResponse>('/api/setup/bootstrap')
  ]);
  if (status) wizardState.status = status;
  if (bootstrap?.config) wizardState.config = bootstrap.config;
  if (bootstrap?.roles) wizardState.roles = bootstrap.roles;
  if (bootstrap?.modules) wizardState.modules = bootstrap.modules;
  if (bootstrap?.users) wizardState.users = bootstrap.users;
}

async function refreshSetupUsers(): Promise<void> {
  if (!wizardState) return;
  const bootstrap = await fetchJSON<SetupBootstrapResponse>('/api/setup/bootstrap');
  if (bootstrap?.users) {
...
function activateSetupShell(): void {
  document.body.classList.add('setup-wizard-active');
}

function deactivateSetupShell(): void {
  document.body.classList.remove('setup-wizard-active');
}

function advanceSetupStep(): void {
  activeStep = Math.min(steps.length, activeStep + 1);
  renderSetupWizard();
}

function postgreSQLReadyForSetup(status = wizardState?.status.storage.control): boolean {
  if (!status) return false;
  if (status.environment_locked) return status.configured;
  if (status.configured && status.available) return true;
  return postgreSQLVerifiedSignature !== '' && postgreSQLVerifiedSignature === currentPostgreSQLSignature();
}

function postgreSQLGateText(status = wizardState?.status.storage.control): string {
  if (!status) return 'Test PostgreSQL before continuing.';
  if (status.environment_locked && status.configured) return 'PostgreSQL is managed outside Aegis.';
  if (status.configured && status.available) return 'PostgreSQL is connected. You can continue.';
  if (postgreSQLReadyForSetup(status)) return 'PostgreSQL connection tested. You can continue.';
  return 'Test PostgreSQL successfully before continuing.';
}

function updateDatabaseGateUI(): void {
  if (activeStep !== 2) return;
  const root = AdminDOM.getById('setup-wizard-root');
  if (!root) return;
  const nextButton = AdminDOM.query<HTMLButtonElement>('.setup-next[data-setup-action="next"]', root);
  if (nextButton) {
...
function adminReadyForSetup(state = wizardState): boolean {
  return hasExistingAdminAccounts(state);
}

function updateAdminGateUI(): void {
...
function setupFirstRunPublicMode(state = wizardState): boolean {
  return Boolean(state?.status.setup_required && state.status.user_count === 0 && !api.currentUser);
}

function shouldPromptForMissingClickHouse(): boolean {
  if (activeStep !== 2 || clickHouseSkipConfirmed || !wizardState) return false;
  const status = wizardState.status.storage.analytics;
  if (status.environment_locked || status.configured) return false;
  return !validSecretRef(AdminDOM.inputValue('setup-clickhouse-secret').trim());
}

function desiredClickHouseMode(): 'optional' | 'required' {
  const mode = wizardState?.config.storage?.analytics?.mode;
  return mode === 'required' ? 'required' : 'optional';
}

function hasExistingAdminAccounts(state = wizardState): boolean {
  if (!state) return false;
  return state.users.length > 0 || state.status.user_count > 0;
}

function isCurrentSetupUser(user: SetupUser, currentUserId: string): boolean {
...
function setWizardBusy(isBusy: boolean): void {
  wizardBusy = isBusy;
  const root = AdminDOM.getById('setup-wizard-root');
  if (!root) return;
  root.classList.toggle('is-busy', isBusy);
  AdminDOM.queryAll<HTMLButtonElement>('button', root).forEach(button => {
...
function currentPostgreSQLSignature(): string {
  const input = postgreSQLInput();
  return postgreSQLConnectionSignature(input);
}

function postgreSQLConnectionSignature(input: Pick<ReturnType<typeof postgreSQLInput>, 'host' | 'port' | 'database' | 'username' | 'sslMode'>): string {
  return [
    input.host,
    String(input.port),
    input.database,
...
function validatePostgreSQLInput(input: ReturnType<typeof postgreSQLInput>, requirePassword: boolean): boolean {
...
function validateClickHouseInput(input: ReturnType<typeof clickHouseInput>, requirePassword: boolean): boolean {
  if (!input.host || !validPort(input.port) || !input.database || !input.username) {
...
function normalizedSecretRef(value: unknown): string {
  return typeof value === 'string' && value !== '[REDACTED]' ? value : '';
}

function findSuperAdminRole(): SetupRole | null {
...
function applySetupTheme(theme: AdminTheme): void {
  if (wizardState) wizardState.theme = theme;
  ...
  const switchInput = AdminDOM.getById<HTMLInputElement>('admin-theme-switch');
...
function requireWizardState(): SetupWizardState {
  if (!wizardState) throw new Error('Setup wizard state is not available');
  return wizardState;
}
