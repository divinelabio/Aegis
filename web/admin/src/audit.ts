...
const auditPageSizes = [20, 50, 100, 200] as const;
const auditState = {
	bound: false,
	pageSize: 20,
	pageIndex: 0,
...
export async function loadAuditLogsPage(): Promise<void> {
...
async function fetchAuditPage(cursor: string, targetPage: number): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Audit log request failed with status ${response.status}`);
    ...
    container.innerHTML = SectionUI.renderOperatorFrame({
      ...
      content: SectionUI.renderEmptyState(
        'Audit logs unavailable',
        'The latest administrative activity could not be loaded. Refresh to try again.',
        'alertCircle'
      ),
...
function bindAuditInteractions(): void {
	if (auditState.bound) return;
	auditState.bound = true;
	AdminEvents.delegateEvent<HTMLElement>(document, 'click', '[data-audit-action]', (event, target) => {
		event.preventDefault();
		const action = target.dataset.auditAction || '';
		if (action === 'next') void loadNextAuditPage();
		if (action === 'previous') showPreviousAuditPage();
		if (action === 'clear-filters') {
			auditState.filters = { action: '', actor: '', from: '', to: '' };
			void loadAuditLogsPage();
		}
		if (action === 'export') void exportAuditLogs();
	});
	AdminEvents.delegateEvent<HTMLFormElement>(document, 'submit', 'form[data-audit-filter-form]', (event, form) => {
		event.preventDefault();
	...
	AdminEvents.delegateEvent<HTMLSelectElement>(document, 'change', '[data-audit-page-size]', (_event, target) => {
		const pageSize = Number(target.value);
		if (!auditPageSizes.includes(pageSize as typeof auditPageSizes[number])) return;
		auditState.pageSize = pageSize;
		void loadAuditLogsPage();
	});
}

async function loadNextAuditPage(): Promise<void> {
	if (!auditState.nextCursor) return;
	await fetchAuditPage(auditState.nextCursor, auditState.pageIndex + 1);
}

function showPreviousAuditPage(): void {
	if (auditState.pageIndex <= 0) return;
	const page = auditState.pages[auditState.pageIndex - 1];
	if (!page) return;
	auditState.pageIndex -= 1;
	auditState.nextCursor = page.meta?.next_cursor || '';
	const container = AdminDOM.getById('audit-logs-container');
	if (container) renderAuditTable(container, page.logs, page.meta);
}

function buildAuditQuery(cursor = ''): URLSearchParams {
	const query = new URLSearchParams();
	query.set('limit', String(auditState.pageSize));
	if (cursor) query.set('cursor', cursor);
	if (auditState.filters.action) query.set('action', auditState.filters.action);
	if (auditState.filters.actor) query.set('actor', auditState.filters.actor);
...
function toAuditISOTime(value: string): string {
	if (!value) return '';
...
async function exportAuditLogs(): Promise<void> {
	...
	try {
		...
		notify(
			response.headers.get('X-Aegis-Export-Truncated') === 'true'
...
function renderAuditTable(container: HTMLElement, logs: AuditLogEntry[], meta?: AuditLogsMeta): void {
...
function renderAuditRow(log: AuditLogEntry): string[] {
...
function describeAuditActivity(log: AuditLogEntry): { title: string; detail: string } {
...
function isOpaqueAuditResource(resource: string): boolean {
  return /^[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i.test(resource);
}

function renderAuditFilters(): string {
	return `
...
function renderAuditPagination(meta?: AuditLogsMeta): string {
