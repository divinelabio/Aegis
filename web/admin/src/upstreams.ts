...
interface UpstreamTargetRuntimeStatus {
  url: string;
  healthy: boolean;
...
interface UpstreamGroupRuntimeStatus {
  name: string;
  targets: UpstreamTargetRuntimeStatus[];
}

const upstreamsState = {
  upstreamGroups: [] as UpstreamGroup[],
  runtimeStatus: [] as UpstreamGroupRuntimeStatus[],
  runtimeStatusByTarget: new Map<string, UpstreamTargetRuntimeStatus>(),
  runtimeStatusAvailable: true,
  editingGroup: null as UpstreamGroup | null
};
...
function decodeGroupName(value: string | undefined): string {
...
function formatStrategy(strategy = 'round_robin'): string {
...
function parseExpectedStatuses(value: string): number[] | null {
  if (!value.trim()) return [];
  const statuses = value.split(',').map(item => Number(item.trim()));
  if (statuses.some(status => !Number.isInteger(status) || status < 100 || status > 599)) return null;
  return Array.from(new Set(statuses));
}

export async function loadUpstreamsPage(): Promise<void> {
  ...
  try {
    const [data, runtimeStatus] = await Promise.all([
      api.get<UpstreamGroup[]>('upstreams'),
...
function renderUpstreams(container: HTMLElement): void {
...
async function loadUpstreamRuntimeStatus(): Promise<{ data: UpstreamGroupRuntimeStatus[]; available: boolean }> {
  try {
    const response = await fetch('/api/upstreams/status', { headers: { Accept: 'application/json' } });
    if (response.status === 404) return { data: [], available: false };
    if (!response.ok) {
      console.warn(`Origins runtime status unavailable: HTTP ${response.status}`);
      return { data: [], available: false };
    }
    const data = await response.json() as unknown;
    return { data: Array.isArray(data) ? data as UpstreamGroupRuntimeStatus[] : [], available: true };
  } catch (error) {
...
function findRuntimeTarget(groupName: string, targetURL: string): UpstreamTargetRuntimeStatus | undefined {
  return upstreamsState.runtimeStatusByTarget.get(runtimeTargetKey(groupName, targetURL));
}

function indexRuntimeStatus(groups: UpstreamGroupRuntimeStatus[]): Map<string, UpstreamTargetRuntimeStatus> {
  const indexed = new Map<string, UpstreamTargetRuntimeStatus>();
...
function runtimeTargetKey(groupName: string, targetURL: string): string {
  return `${groupName}\u0000${targetURL}`;
}

function renderTarget(groupName: string, target: UpstreamTarget): string {
...
function renderGroupRow(group: UpstreamGroup): string[] {
  ...
  return [
    ...
    SectionUI.renderStatusPill(
      healthCheck.enabled && !upstreamsState.runtimeStatusAvailable
        ? 'Configured'
        : healthCheck.enabled
        ? `${targets.filter(target => findRuntimeTarget(group.name, target.url)?.healthy).length}/${targets.length} healthy`
        : 'Disabled',
      healthCheck.enabled && upstreamsState.runtimeStatusAvailable && targets.some(target => !findRuntimeTarget(group.name, target.url)?.healthy) ? 'danger' : healthCheck.enabled && upstreamsState.runtimeStatusAvailable ? 'success' : 'neutral'
    ),
...
function ensureUpstreamsBindings(): void {
  if (upstreamsBindingsInitialized) return;
  upstreamsBindingsInitialized = true;

  AdminEvents.delegateEvent<HTMLElement>(document, 'click', '[data-upstreams-action]', (_event, target) => {
    ...
    switch (action) {
      ...
      case 'save-group':
        ...
        break;
      case 'edit-group':
        if (groupName) editGroup(groupName);
        break;
      case 'delete-group':
        if (groupName) void deleteGroup(groupName);
        break;
      case 'add-target-row':
        addTargetRow(target);
        break;
  ...
  AdminEvents.delegateEvent<HTMLInputElement>(document, 'change', '#origin-healthcheck, #origin-sticky', () => {
    syncOriginEditorControls();
  });

  AdminEvents.delegateEvent<HTMLFormElement>(document, 'submit', '#origin-group-form', event => {
    event.preventDefault();
    void saveGroup();
  });
}

export function openOriginEditor(): void {
  openGroupEditor();
}

function openGroupEditor(group: UpstreamGroup | null = null): void {
  const container = AdminDOM.getById('upstream-container');
  if (container) renderOriginEditor(container, group);
}
...
function renderOriginTargetRow(target: UpstreamTarget = { url: '', weight: 1 }): string {
  return `
...
function renderOriginEditor(container: HTMLElement, group: UpstreamGroup | null = null): void {
  ...
  const selected = (value: string): string => strategy === value ? ' selected' : '';
...
function syncOriginEditorControls(): void {
  ...
  if (healthSettings) healthSettings.hidden = !AdminDOM.checkboxValue('origin-healthcheck', false);
  if (stickySettings) stickySettings.hidden = !AdminDOM.checkboxValue('origin-sticky', false);
}
...
async function saveGroup(): Promise<void> {
  ...
  AdminDOM.queryAll<HTMLElement>('#origin-targets-container .target-row').forEach(row => {
    const url = (AdminDOM.query<HTMLInputElement>('.target-url', row)?.value || '').trim();
    const weight = Number.parseInt(AdminDOM.query<HTMLInputElement>('.target-weight', row)?.value || '', 10) || 1;
    if (url) targets.push({ url, weight });
  });
...
async function deleteGroup(name: string): Promise<void> {
  SectionUI.openConfirmModal(
    'Delete Origin Pool',
    `Are you sure you want to delete origin pool "${escapeHTML(name)}"?`,
    'Delete',
    'var(--danger)',
    () => {
...
async function confirmDeleteGroup(name: string): Promise<void> {
...
function upstreamsShowToast(message: string, type: ToastType = 'info'): void {
  notify(message, type);
}
