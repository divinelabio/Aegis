...
interface FingerprintConfig {
  ...
  config_expires_at: number;
  nonce_expires_at: number;
  collect_canvas: boolean;
  collect_webgl: boolean;
  collect_audio: boolean;
  collect_fonts: boolean;
  collect_behavior: boolean;
  check_headless: boolean;
  token_ttl?: number;
}

interface RuntimeSignals {
  webdriver: boolean;
  webdriver_lie: boolean;
  chrome_runtime_missing: boolean;
  notification_inconsistent: boolean;
  permissions_inconsistent: boolean;
  outer_window_zero: boolean;
  touch_inconsistent: boolean;
}
...
interface SignalPayload {
  ...
  session_id: string;
  session_nonce: string;
  fp_canvas_hash?: string;
  fp_webgl_hash?: string;
  webgl_renderer?: string;
  fp_fonts_hash?: string;
  fp_audio_hash?: string;
  headless?: RuntimeSignals;
...
const BehaviorTracker = (() => {
  ...
  function getVector(): BehaviorVector {
...
async function fetchConfig(): Promise<FingerprintConfig> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (isValidFingerprintConfig(config)) return config;
    }
  } catch (_error) {
  } finally {
...
function isValidFingerprintConfig(value: unknown): value is FingerprintConfig {
  if (!value || typeof value !== 'object') return false;
  const config = value as Partial<FingerprintConfig>;
...
async function collectSignals(config: FingerprintConfig): Promise<SignalPayload> {
...
async function transmitPayload(payload: SignalPayload): Promise<void> {
...
async function collectRuntimeSignals(): Promise<RuntimeSignals> {
  ...
  return {
    ...
    chrome_runtime_missing: /Chrome|Edg/.test(navigator.userAgent) && !(window as Window & { chrome?: { runtime?: unknown } }).chrome?.runtime,
...
function readCanvasFingerprint(): string {
  try {
...
function readWebGLFingerprint(): { fingerprint: string; renderer: string } {
  try {
    const canvas = document.createElement('canvas');
    const gl = (canvas.getContext('webgl') || canvas.getContext('experimental-webgl')) as WebGLRenderingContext | null;
    if (!gl) return { fingerprint: 'no_webgl', renderer: 'no_webgl' };
...
function readFontFingerprint(): string {
...
async function readAudioFingerprint(): Promise<string> {
  try {
    const AudioContextCtor = window.OfflineAudioContext || (window as Window & { webkitOfflineAudioContext?: typeof OfflineAudioContext }).webkitOfflineAudioContext;
    if (!AudioContextCtor) return 'no_audio';
...
async function readPermissionsState(): Promise<string> {
  try {
