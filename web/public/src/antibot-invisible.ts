...
async function runInvisibleChallenge(): Promise<void> {
  const config = window.__AEGIS_INVISIBLE__;
  if (!config?.salt) return;
  ...
  const payload: InvisiblePayload = {
...
async function solveProof(salt: string, difficulty: number): Promise<string> {
...
function buildFormData(payload: InvisiblePayload): FormData {
